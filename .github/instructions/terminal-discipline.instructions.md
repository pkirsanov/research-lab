...
**REQUIRED:** Run product checks from the repository root using the exact Node
commands declared by the repository or active spec. The baseline project check
is:
...
Scenario-specific validators and live checks use their exact committed script
paths and arguments. Do not substitute a different command or silently reduce
