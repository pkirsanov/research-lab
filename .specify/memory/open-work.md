...
| res-g136-acceptance-record-backfill | 21 of 23 certified-done packets carry no `## Human Acceptance Record`, so Gate G136 blocks any terminal re-transition | residue | specs/*/uservalidation.md, specs/_bugs/*/uservalidation.md, specs/*/bugs/*/uservalidation.md | open | operator | Decide per packet whether to author a Human Acceptance Record or to record an explicit grandfather exemption for certifications that predate Gate G136 — no exemption path exists today, since `.github/bubbles/scripts/acceptance-authority-lib.sh` contains no match for grandfather, legacy, predates, certifiedAt, or historical. Measured 2026-08-24 in the primary checkout: 23 packets hold `status=done` with `certification.status=done`; only `specs/025-company-multi-horizon-intelligence-lab` and `specs/027-company-scoped-owner-deep-links` pass G136, both certified 2026-08-24 with `acceptedBy: operator` and `method: human-interactive`; the other 21 fail with `PD12-NO-RECORD` (confirmed by running the guard on `specs/001-causal-rotation-intelligence`, which exits blocked on `failedGateIds: [G136]` despite 21/21 boxes checked). Start with the three whose acceptance checklists are still substantially unchecked while the packet is certified done — `specs/010-company-fundamentals-and-brief-lab` (5 checked, 20 unchecked), `specs/002-distributed-tool-briefs-and-history` (18 checked, 10 unchecked), `specs/_bugs/BUG-007-decision-attention-contract-drift` (4 checked, 2 unchecked). Only the operator can discharge this: G136 states that checking a box on the author's behalf would fabricate the acceptance it requires, so no agent may author a record, tick an acceptance box, or change a packet status | 2026-08-24 | 2026-08-24 |
