...
Published via GitHub Pages (GitHub Actions) from the repository root on every
push to `main`. The deployed address is the repository's standard GitHub Pages
URL — see **Settings → Pages** for the live link.
...
| [`Global Rotation Lab`](global-rotation-lab.html) | Country and region ETF leadership across South Korea, Singapore, Philippines, Germany, Japan, Taiwan, India, China, the UK, Australia, Canada, Brazil and Mexico. Simple mode is a steerable allocation queue; Power exposes benchmark-relative 21/63/126-day momentum, 20/50/200 trend, realized risk, local-close timing and diversification. Country score and rank are **equity-only** — raw FX cannot move them. Currency effect is a separate product: two-leg USD leadership and three-leg local/translation decomposition keep their own returns, coverage and clocks, and an absent FX leg leaves USD leadership intact rather than assuming zero. Uses [`global-rotation-universe.json`](global-rotation-universe.json), shared cache first, and automatically appends only stale/missing bars. | [`notes/global-rotation-lab.md`](notes/global-rotation-lab.md) |
| [`FX Regime & Currency Vehicle Lab`](fx-regime-relative-value-lab.html) | Map a currency view to an eligible **listed vehicle** rather than to spot. Simple states the owner decision; Power exposes the per-vehicle fit anatomy across FXY, FXE, UUP, UDN, USDU, CEW and YCS with direction, basket, wrapper, liquidity/cost and daily-reset boundaries kept distinct. Every committed vehicle observation is currently rights-unclear, so the route reports an exact `Unavailable` disposition per member with `RIGHTS_UNCLEAR`, selects nothing, and never substitutes a fund. Uses [`fx-regime-universe.json`](fx-regime-universe.json) and [`fx-vehicle-universe.json`](fx-vehicle-universe.json). | [`notes/fx-regime-relative-value-lab.md`](notes/fx-regime-relative-value-lab.md) |
...
| [`📐 Technical Analysis Decision Lab`](technical-analysis-decision-lab.html) | One coherent technical-analysis **decision framework** (not another indicator dashboard): for a source-qualified stock it moves **primary context → regime → location → trigger → expectancy** and returns a plain-language setup state (`NO EDGE` · `WATCH` · `ARMED` · `TRIGGERED` · `INVALIDATED` · `EXPIRED`, plus first-class `MIXED` / `UNAVAILABLE`). A **five-gate** workflow (context, location, confirmation, validation, risk) never lets a setup trigger by averaging indicator votes; SMA/EMA/MACD/RSI/stochastic/Bollinger cluster by mathematical family so correlated indicators can't manufacture confidence. Specialist models (structure, trend, price-volume/Wyckoff, auction/value, patterns, mean-reversion, relative strength, options positioning, psychology/risk) each expose supporting/contradicting/unstable/unavailable evidence. Confidence is evidence quality, **never** a win probability; wicks aren't proven manipulation; volume splits are OHLCV proxies; dealer gamma is a convention-dependent scenario. Simple and Power share one result. Configure via [`technical-analysis-decision-universe.json`](technical-analysis-decision-universe.json). Educational only — not investment advice. | [`notes/technical-analysis-decision-lab.md`](notes/technical-analysis-decision-lab.md) |
| [`🧭 Portfolio Survival & Allocation Lab`](portfolio-survival-allocation-lab.html) | A six-tab **local** workspace that asks whether a portfolio survives what its owner actually needs it to survive — a mandate with real dates and real cash needs. Risk X-Ray, bootstrap **path survival**, stress-conditional **dependence**, six **allocation** methods with sensitivity ranges and an explicit **Black-Litterman** editor, and a **research dossier** that keeps in-sample, walk-forward and cost-adjusted results as three separate figures. Every analytic **fails closed** with a named reason, and the replacement comparison refuses to call two securities substantially identical **in either direction** — that is a legal and tax determination. Holdings, mandate, scenarios and dossiers **never leave this browser**. Educational only — not investment advice. | [`notes/portfolio-survival-allocation-lab.md`](notes/portfolio-survival-allocation-lab.md) |
| [`Research Agenda Lab`](research-agenda-lab.html) | Recurring public-market research with an immutable current generation, explicit unavailable and stale states, durable actor/flow/transmission models, dated history, five local scenario assumptions, public proxy sensitivity, and falsifiable reversal conditions. An unavailable current review never borrows a posture, probability, or range from the historical dossier. | [`notes/research-agenda-lab.md`](notes/research-agenda-lab.md) |
