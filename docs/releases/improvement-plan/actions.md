...
| A07 | **P3 — ROW CORRECTED 2026-08-13** | `/bubbles.validate` next; then the current Feature 006 owner | ~~Scope 5 remains In Progress~~ **Stale when written.** Re-measured at `af89f280`: Scope 5 is **Done** with 22/22 DoD items checked and inline executed evidence, all 16 Test Plan rows pass, and **all 5 scopes are Done**. `state.json` names `nextRequiredOwner: bubbles.validate`. The remaining obligation is validate-owned certification, not implementation | `/bubbles.validate` performs the certification review. `certification.*` was deliberately left untouched by the implement phase and must not be self-promoted | Feature 006 reaches a terminal status through validate certification; scheduled/headless consumption of the registered owner read is a separate follow-on, and browser cache is never treated as scheduled evidence | Respect the current owner. Shared `brief-refresh.mjs` work is now clear of A01/A03 |
