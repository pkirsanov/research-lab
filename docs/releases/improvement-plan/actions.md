...
| A14 | **P2 — OPERATOR-PRIORITIZED ASAP; OPTIONAL AND NON-GATING** | `/bubbles.workflow` routes the assigned Feature 028 implementation owner | Feature 028 remains `not_started`, planning-only, uncertified, and not delivered. Feature 011 is its sole technical dependency and is `done` and certified. A04, A06, A09, and A11 remain unresolved release-order work, but the operator has authorized A14 to proceed ahead of them. | Dispatch Feature 028 implementation from SCOPE-028-01. Before changing shared `RLVOL` or volatility-workspace files, perform ordinary compatibility review against their current consumers and coordinate active ownership. A04, A06, A09, and A11 do not gate pickup and do not trigger Feature 028 planning invalidation. | The existing Feature 011 volatility tool gains an opt-in Power diagnostic with no new root page or `tools.json` row, no second provider fetch, deterministic browser/Node parity, withheld $H$ for weak or insufficient evidence, invariant Feature 011 decision outputs, and passing Feature 028 tests and validation before any delivery claim. The 750 ms Node reference and separate system-Chrome checks remain planned obligations, not executed results. | Optional and not a phase exit criterion. Preserve Feature 011 certification and behavior. Coordinate shared-file edits through normal compatibility review; do not claim Feature 028 restores the existing route's unavailable `file://` configuration path. |
...
7. ~~Resolve Feature 028 representation, conflict, performance, grid, and file-origin policy~~ **DECIDED by the
    operator for A14:**
    - Embed the immutable diagnostic in an additive decision projection. Give it its own deterministic
       `diagnosticId` while preserving the Feature 011 base `decisionId` as the parent identity. Do not create a
       second formula or store owner. The implementation design owner retains responsibility for the final
       versioned representation.
    - Use the existing conflict vocabulary: `blocking: false` with optional `kind: "model-assumption"`,
       `diagnosticId`, and `deepLink`. The current contract has no severity field, so this decision does not invent
       one.
    - Make Node 20 on `ubuntu-latest` the binding 750 ms formula reference and record runner, Node, platform, and
       architecture facts with the result. Keep `system-chrome` with two workers as separate browser smoke and
       first-paint coverage. Neither check has run for Feature 028 yet.
    - Fix the Feature 028 grids at $q\in\{0.5,1.0,1.5,2.0\}$ and lags
       $\Delta\in\{1,2,4,8,16,32\}$. Any configurability requires a later versioned contract and owner decision.
    - Promise only that Feature 028 introduces no additional file-origin regression. The existing Feature 011
       route already reaches configuration-unavailable under `file://`; Feature 028 does not restore it. Repairing
       that route requires a separate product decision. This is an honest compatibility boundary, not a P10
       waiver.
      - Feature 011 is the sole technical dependency. It is delivered and certified. A04, A06, A09, and A11 are
         release-order constraints only. They provide no roughness, structure-function, zeta, Hurst, or additive
         `RLVOL` decision-schema capability required by Feature 028.
      - The operator authorizes A14 ahead of unresolved A04, A06, A09, and A11. This reprioritization does not mark
         any of those actions delivered and does not make Feature 028 a phase exit criterion.
      - Shared `RLVOL` and volatility-workspace edits require ordinary compatibility review against current
         consumers. A09 or A11 delivery does not invalidate Feature 028 planning and does not require a special
         analyst/design/planning revalidation cycle.
      Feature 028 remains optional, non-gating, `not_started`, uncertified, and not delivered until its own
      implementation, tests, validation, and acceptance establish otherwise.
