...
- **2026-06-30 power-controls pass:** fixed Power-mode optimizer controls so changing objective, risk aversion, max positions, max weight, horizon/scenario/convex settings auto-runs the optimizer and reselects checked holdings/weights across the full supplier universe. Added feasibility handling for impossible max-position/max-weight combinations (e.g., 3 positions at max 10% cannot sum to 100%).
