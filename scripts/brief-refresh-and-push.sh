...
DERIVED_FILES=(brief-history.recent.jsonl market-brief.scorecard.json market-brief.owner-reads.json market-brief.attention-scorecard.json causal-rotation.snapshot.json)
...
# The attention scorecard and deterministic causal snapshot ride every selection. Neither carries
# brief-pair parity, and both are written before the final narrative outcome is known.
if [ "$NARRATIVE_OK" = "1" ]; then
  SELECTED_FILES=("${DATA_FILES[@]}" "$PAYLOAD" "$CONFIG" market-brief.owner-reads.json market-brief.attention-scorecard.json causal-rotation.snapshot.json data)
  [ -d "$AGENDA_DIR" ] && SELECTED_FILES+=("$AGENDA_DIR")
  SELECTION="matching-pair"
elif [ "$RETAINED_TIER_B_OK" = "1" ]; then
  SELECTED_FILES=("${DATA_FILES[@]}" market-brief.owner-reads.json market-brief.attention-scorecard.json causal-rotation.snapshot.json data)
  SELECTION="same-target-data-only"
else
  # market-brief.owner-reads.json is a DERIVED_FILES / OWNED_PATHS member (see the pre-fetch dirty
  # check above), regenerated by Tier A on every run regardless of narrative outcome. Omitting it
  # here — unlike the matching-pair and same-target-data-only branches above, which both already
  # carry it — left it perpetually dirty after any run that fell back to raw-data-only, which then
  # made THIS SAME dirty check refuse the very next invocation with no narrative attempted at all.
  # Bonsai/OMLX runs are exactly the ones most likely to land here on a given cycle, so this bug
  # was silently bricking the local narrative path after any single non-convergent attempt.
  SELECTED_FILES=(market-brief.owner-reads.json market-brief.attention-scorecard.json causal-rotation.snapshot.json data)
