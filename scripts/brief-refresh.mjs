 ...
 * 1m/3m momentum + RRG-lite state), and APPENDS one snapshot to brief-history.jsonl —
...
async function yahooRows(sym, range = '6mo', interval = '1d') {
  try {
...
async function main() {
  ...
  const benchMom1m = momentumPct(bench, 21), benchMom3m = momentumPct(bench, 63);
  ...
  for (const s of (cfg.track?.sectors || [])) {
    ...
    sectors[s] = { rsMom1m: round(m1 - benchMom1m, 2), rsMom3m: round(m3 - benchMom3m, 2), rrgState: rrgLite(m1, benchMom1m) };
  }
  const names = {};
  for (const it of (wl.items || [])) {
    const rows = await yahooRows(it.ticker); if (!rows) continue;
    names[it.ticker] = { px: round(rows[rows.length - 1].c, 2), mom5: round(momentumPct(rows, 5)), mom21: round(momentumPct(rows, 21)), mom63: round(momentumPct(rows, 63)) };
  }
