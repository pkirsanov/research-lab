...
| 2026-07-29 | GAP-F5 — **RESOLVED**. Two required tests carried a silent-pass bailout: `try { ({ chromium } = await loadPlaywright()); } catch (e) { t.skip('Playwright runtime unavailable'); return; }` in `tests/distributed-briefs.static.integration.mjs:26` and `tests/distributed-briefs.ui-canary.mjs:18`. Audit AUD-9 qualified this as latent because Playwright currently resolves — but if any CI image ever omits it, **both required tests report PASS having executed zero assertions**. That is precisely the bailout-return pattern this repository's own policy forbids in required tests. | fixed-in-session | Both bailouts replaced with a loud failure: `catch (e) { throw new Error('Playwright runtime is REQUIRED for this test and failed to load: ' + e.message); }`. Adversarial proof simulating an unavailable runtime — OLD: `skipped=true`, a required test passes with ZERO assertions executed; NEW: `threw=true`, `Playwright runtime is REQUIRED for this test and failed to load: Cannot find module "playwright"`. Verified with bare exit codes: `t.skip` bailouts remaining across all 002-owned tests = 0; the repository's own silent-pass detector `regression-quality-guard.sh` reports **0 violation(s), 0 warning(s)** over both files, exit 0; and both tests still pass normally with Playwright present (1 pass / 0 fail each, exit 0). Both files are 002-owned (named in scope 10) and were clean of concurrent edits. |
| 2026-07-29 | FW-02 — **OBSERVED NON-DETERMINISM in `state-transition-guard.sh`.** One run reported a third block, `DoD item [x] asserts an EXECUTION outcome but its evidence block '#tp-10-18-deployed-pages-e2e-post-cutover' contains no command output (prose-only)`. The finding is provably wrong: the anchor resolves uniquely to line 139, fence parity before it is even (8 fences) so `in_fence=false`, the fence-aware `end_line` is 180, and the resulting block carries `**Executed:** YES`, `**Command:** …`, `**Exit Code:** 0` and two fences — every token in the guard's own signature regex `` ```\|Exit Code:\|^[[:space:]]*\$ \|Executed:\|Command: ``. Scope 10 was byte-clean and unchanged (no diff since `3605cf42`, no working-tree modification). Four consecutive re-runs then returned **2 blocks with zero tp-10-18 findings**, so the verdict was transient, not a real regression. | routed | Mechanism NOT proven, stated as a hypothesis: `state-transition-guard.sh` references fixed, non-unique scratch paths — `/tmp/bubbles-agent-ownership-lint.`, `/tmp/bubbles-assurance-cert-check.`, `/tmp/bubbles-workflow-grants-lint.` — alongside only 5 `mktemp` uses, while several agent sessions were running framework guards concurrently in this workspace. Fixed shared scratch paths are a known cross-session collision hazard and the framework maintainers are already investigating exactly this in the `bubbles` repo. Recorded because a gate that can emit a **non-reproducible blocking verdict** under concurrent use is a reliability defect in its own right — it invites agents to "fix" phantom findings, and a genuinely transient PASS would be far worse than a transient FAIL. Owner: `bubbles` framework maintainers. Not actioned here: the guard is a framework-managed install artifact, and the framework repo is `ahead 21` with its own live validation runs. |
