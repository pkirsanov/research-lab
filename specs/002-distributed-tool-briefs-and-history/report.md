...
| 2026-07-29 | RG-06 — BUG-003 is only PARTIALLY repaired. The 2026-07-27 fix landed on `sector-research-lab` alone; `options-flow-feed-lab` still fails the Scope-10 all-page ui-canary (`page.waitForSelector: Timeout 30000ms exceeded` waiting for `[data-rlbrief-mount][data-rlbrief-ready="1"]`; 22 of 23 pages pass, exit `1`). The mechanism is identical to the original: Feature 012 scope 15 (commit `5c83d9d7`) provider-wired the page, so `rlapp.js` yields `ownerModes: ["power"]` and the `data-simple-target` mount never reaches ready. `volatility-sizing-lab.html` and `options-structure-lab.html` carry the same three `__rlOwnerStateProvider` registrations alongside a `data-simple-target` mount and are exposed to the same defect. | bug-filed | `specs/012-market-action-center-and-guided-tools/bugs/BUG-003-shell-brief-panel-adoption-hides-feature-002-mount/` — still `in_progress` and owned by feature 012; Feature 002 does not modify those pages. Evidence: [scopes/10-shared-ui-and-pages-acceptance/report.md](scopes/10-shared-ui-and-pages-acceptance/report.md#test-evidence) (2026-07-29 re-verification block, full canary failure text). |
