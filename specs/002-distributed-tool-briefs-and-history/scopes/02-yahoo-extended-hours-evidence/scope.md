...
**Allowed file families:** `market-brief.config.json` source/evidence policy blocks; new calendar/evidence scripts; `data/calendars/xnys/calendar.json`; bounded generated `data/session-bars/**`; Scope 02 fixtures/tests; additive imports of Scope 01 contracts.

**Excluded surfaces:** owning model formulas/interpretations, BLS/report/consensus/reaction logic, registry briefing metadata, tool reads/briefs/final authoring, current/history pointers, scheduler Git publication, shared renderer/pages, legacy history, package/dependency files, other specs, and unrelated dirty/untracked paths.
...
| Baseline Canary: shared foundation contracts | functional | SCN-002-017, SCN-002-028 | Existing complete repository selftest | `node scripts/selftest.mjs` | Yes | Red: shared source changes alter existing owner/daily behavior; Green: the unchanged repository baseline passes. |

### Definition of Done - Tiered Validation

Core outcomes:

- [x] The committed XNYS projection, exact source/request/use policies, Yahoo normalizer, official-close anchor, extended-hours aggregate, comparable-volume result, and read-only source smoke implement the design contracts with no alternate-source or weekday fallback. — Evidence: [report.md](report.md#tp-02-01-unit-source-policy-allowlist-scn-002-028) (TP-02-01 unit + TP-02-04 calendar `--check` OK 365 rows + TP-02-05 live smoke + TP-02-09 selftest 572/0; per-row index: [report.md](report.md#test-evidence-one-block-per-test-plan-row)); code byte-identical to HEAD `e8328b7`.
- [x] SCN-002-017 and SCN-002-028 preserve cutoff, provenance, freshness, adjustment, official-versus-indicative, exact-bucket, thin/unavailable/disputed, and missing-versus-zero truth in machine and user-facing summaries. — Evidence: [report.md](report.md#tp-02-02-functional-captured-yahoo-normalization-scn-002-017) (functional TP-02-02/TP-02-03 + source.e2e TP-02-06/TP-02-07 GREEN; scenario contract summary: [report.md](report.md#scenario-contract-evidence)).
- [x] Consumer and Shared Infrastructure Impact Sweeps are complete; daily-bar/current consumers remain compatible, independent canaries pass, and the narrow rollback is proven. — Evidence: [report.md](report.md#lint-and-quality) (independent canaries: selftest 572/0, brief-cache PASS, brief-payload PASS); sweep narrative: [report.md](report.md#consumer-and-shared-infrastructure-sweep); the adapter is additive (no existing caller repointed) and `node scripts/selftest.mjs` = 572/0 confirms the daily-fetch + Market Brief canaries unchanged.
- [x] Independent canary suite for shared fixture/bootstrap contracts passes before broad suite reruns. — Evidence: [report.md](report.md#lint-and-quality) (`node scripts/selftest.mjs` = 572/0, `node scripts/validate-brief-cache.mjs` = PASS over 354 files, and `node scripts/validate-brief-payload.mjs market-brief.payload.json` = PASS all run with the additive calendar/source-policy/acquisition modules in place, before any broad suite rerun).
- [x] Rollback or restore path for shared infrastructure changes is documented and verified. — Evidence: [report.md](report.md#consumer-and-shared-infrastructure-sweep) (the change boundary is additive-only — new scripts, tests, fixtures, and one additive `marketSessionEvidence` config block — so narrow rollback is removal of those additive files/block; the green baseline canaries above confirm the previous daily-only consumer path already runs independently of the Scope 02 additions, and no committed evidence/history object is mutated).
...
- [x] Change Boundary is respected and zero excluded file families were changed — every enumerated excluded surface (owning model formulas/interpretations, BLS/report/consensus/reaction logic, registry briefing metadata, tool reads/briefs/final authoring, current/history pointers, scheduler Git publication, shared renderer/pages, legacy history, package/dependency files, other specs) is byte-identical. — Evidence: [report.md](report.md#byte-identity-audit-claim-source-executed) (`git status --porcelain` empty; six Scope 02 code files byte-identical to HEAD).
...
- [x] [TP-02-04] Integration evidence passes for the canonical calendar `--check` command without repository writes. — `--check OK` 365 rows / 251 open, exit 0; [report.md](report.md#tp-02-04-and-tp-02-05-integration-evidence).
- [x] [TP-02-05] Integration evidence passes for the live no-write source smoke without fixed-value claims. — `structural=unavailable market-closed weekend`, `no-write verified`, `OK`, exit 0; [report.md](report.md#tp-02-04-and-tp-02-05-integration-evidence).
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-002-017 pass with the exact title. [TP-02-06] — `Regression: SCN-002-017 …` GREEN in source.e2e / combined suite, exit 0.
...
- [x] Broader E2E regression suite passes for the combined foundation and source graph. [TP-02-08] — `node --test tests/market-session-evidence.foundation.e2e.mjs tests/market-session-evidence.source.e2e.mjs` = 6/6, exit 0.
