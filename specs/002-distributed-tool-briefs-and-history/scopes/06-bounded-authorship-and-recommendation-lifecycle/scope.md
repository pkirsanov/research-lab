...
**Allowed file families:** `rlcontracts.js` author/brief/recommendation contracts and reducers; new `scripts/brief-author.mjs`; tool-author pool/reuse hooks in `scripts/brief-refresh.mjs`; Scope 06 tests and external-author contract fixtures.

**Excluded surfaces:** source acquisition/owner formulas, registry metadata/read adapters, final authoring, storage/history/pointers, scheduler Git publication, shared renderer/pages, provider credential stores, dependency files, other specs, and unrelated dirty/untracked paths.
...
| Integration | integration | SCN-002-004, SCN-002-005 | `tests/distributed-briefs.authorship.integration.mjs` - `production pool resolves every registry source outcome with at most four active author processes` | `node --test tests/distributed-briefs.authorship.integration.mjs` | Yes | Red: barrier incomplete or concurrency exceeds four; Green: real frozen reads and process boundary yield exact complete outcomes. |
...
- [x] Every currently derived source has exactly one validated authored/carried/no-recommendation/coverage outcome under frozen input, explicit profile policy, and the complete source barrier. — Evidence: [report.md](report.md#tp-06-05-tp-06-06-functional-author-boundary-and-integration-source-pool) (TP-06-06 integration + TP-06-08 e2e: the pool resolves 22 outcome IDs exactly equal to `orderedSourceToolIds`, aggregator excluded, each brief re-validated by `validateToolBrief`).
- [x] Compaction, four-worker concurrency, attempts, timeouts, token/cost ceilings, exact reuse, duplicate handling, privacy, safe telemetry, and refusal behavior implement the declared contracts without omission or implicit values. — Evidence: [report.md](report.md#tp-06-07-stress-concurrency-retry-and-run-token-ceilings) (TP-06-02 compaction, TP-06-05 boundary, TP-06-06/07 pool concurrency + `B002-BUDGET` ceilings; `resolveBriefReuse` zero-call carry; caller-supplied budgets, no default values).
- [x] SCN-002-005 keeps repeated malformed and over-budget author work deterministic: unchanged content is carried by reference with zero author calls, changed sources use at most four concurrent calls and the initial attempt plus two retries per source, a required-input or run-ceiling breach fails before any participant is omitted or publication begins, and telemetry records safe counts, clocks, usage, and categories without prompts, private values, or rejected prose. — Evidence: [report.md](report.md#tp-06-07-stress-concurrency-retry-and-run-token-ceilings) (TP-06-09 e2e `Regression: SCN-002-005 unchanged and duplicate work creates no author prose event or cost churn` RED→GREEN proves zero-call carry and idempotent event IDs; TP-06-06 integration observed peak concurrency 4 with one call per source and zero retries; TP-06-07 stress "every retried source resolves in the initial attempt plus one retry", "peak concurrency 4 stays within the four-worker ceiling", and run output/attempt/input ceilings each refuse `B002-BUDGET run-ceiling-exceeded`; TP-06-02 `mandatory-material-exceeds-cap` refuses over-cap before invocation; TP-06-05 timeout, oversize, malformed, unsafe, mismatch, and duplicate responses are each bounded, redacted, and rejected).
- [x] Recommendation identities, observations, lifecycle/outcome/correction events, compatible/shared-origin/conflict groups, confidence floor, and immutable prior terms satisfy SCN-002-006. — Evidence: [report.md](report.md#tp-06-03-unit-lifecycle-transition-identity-and-immutability-red-before-green) (TP-06-03 lifecycle transitions + immutable prior terms, TP-06-04 minimum-retained grouping and visible conflicts, TP-06-10 e2e).
...
- [x] Change Boundary is respected and zero excluded file families were changed — every enumerated excluded surface (source acquisition/owner formulas, registry metadata/read adapters, final authoring, storage/history/pointers, scheduler Git publication, shared renderer/pages, provider credential stores, dependency files, other specs) is byte-identical. — Evidence: [report.md](report.md#lint-and-quality) (`git status --porcelain` shows only the eleven in-scope paths; diff isolated to the declared surface, RED restores verified byte-exact by SHA-256).

Test evidence items, one per Test Plan row:

- [x] [TP-06-01] Unit evidence passes for owner-evidence-bound brief validation after its recorded red stage. — Evidence: [report.md](report.md#tp-06-01-unit-owner-evidence-bound-brief-validation-red-before-green) (RED cited-evidence check disabled → ghost brief validated, exit 1; restored byte-exact; GREEN `recommendation-cited-evidence-absent`).
- [x] [TP-06-02] Unit evidence passes for exact-cap deterministic compaction after its recorded red stage. — Evidence: [report.md](report.md#tp-06-02-unit-exact-cap-deterministic-compaction-red-before-green) (RED reversed priority sort → `['fact-low',…]` vs `['fact-high',…]`, exit 1; restored; GREEN exact-cap + `B002-BUDGET` over-cap).
- [x] [TP-06-03] Unit evidence passes for lifecycle transition identity and immutability after its recorded red stage. — Evidence: [report.md](report.md#tp-06-03-unit-lifecycle-transition-identity-and-immutability-red-before-green) (RED modified→reaffirmed forced → `['reaffirmed']` vs `['modified']`, exit 1; restored; GREEN transition graph + retained prior terms).
- [x] [TP-06-04] Unit evidence passes for compatible/shared-origin/conflict grouping after its recorded red stage. — Evidence: [report.md](report.md#tp-06-04-unit-compatibleshared-originconflict-grouping-red-before-green) (RED `Math.min`→`Math.max` → `64 !== 50`, exit 1; restored; GREEN minimum-retained + shared-origin-once + visible conflict).
- [x] [TP-06-05] Functional evidence passes for the true external author boundary and every adversarial response class. — Evidence: [report.md](report.md#tp-06-05-tp-06-06-functional-author-boundary-and-integration-source-pool) (`node --test …author-boundary.functional.mjs` 1/1 exit 0; real child; timeout/oversize/malformed/unsafe/mismatch/duplicate each bounded + rejected).
- [x] [TP-06-06] Integration evidence passes for the complete 22-source four-worker pool. — Evidence: [report.md](report.md#tp-06-05-tp-06-06-functional-author-boundary-and-integration-source-pool) (`node --test …authorship.integration.mjs` 1/1 exit 0; 22 validated outcomes, observed peak concurrency ≤ 4, one call per source).
- [x] [TP-06-07] Stress evidence passes for declared concurrency, retry, and run token ceilings. — Evidence: [report.md](report.md#tp-06-07-stress-concurrency-retry-and-run-token-ceilings) (`node tests/…authorship.stress.mjs` 16/0 exit 0; subset retry within ceiling; output/attempt/input ceilings each refuse `B002-BUDGET`).
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-002-004 pass with the exact title. [TP-06-08] — Evidence: [report.md](report.md#tp-06-08-tp-06-09-tp-06-10-e2e-regression-scn-002-004-and-scn-002-005-and-scn-002-006-red-before-green) (RED evidence-subset check disabled → tampered brief validated, exit 1; restored; GREEN `Regression: SCN-002-004 every registry source read reaches one truthful validated brief outcome`).
- [x] [TP-06-09] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-002-005 pass with the exact title. — Evidence: [report.md](report.md#tp-06-08-tp-06-09-tp-06-10-e2e-regression-scn-002-004-and-scn-002-005-and-scn-002-006-red-before-green) (RED nonce in eventId → differing event IDs, exit 1; restored; GREEN `Regression: SCN-002-005 unchanged and duplicate work creates no author prose event or cost churn`).
- [x] [TP-06-10] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-002-006 pass with the exact title. — Evidence: [report.md](report.md#tp-06-08-tp-06-09-tp-06-10-e2e-regression-scn-002-004-and-scn-002-005-and-scn-002-006-red-before-green) (RED `Math.min`→`Math.max` → `64 !== 50`, exit 1; restored; GREEN `Regression: SCN-002-006 recommendation lifecycle preserves prior terms merges origins and exposes conflicts`).
- [x] Broader E2E regression suite passes for registry-owner-authorship behavior. [TP-06-11] — Evidence: [report.md](report.md#tp-06-11-tp-06-12-broader-e2e-regression-and-baseline-selftest) (full distributed-briefs `node --test` surface `tests 25 / pass 25 / fail 0`, exit 0).
- [x] [TP-06-12] Baseline functional evidence passes for `node scripts/selftest.mjs` after focused checks are green. — Evidence: [report.md](report.md#tp-06-11-tp-06-12-broader-e2e-regression-and-baseline-selftest) (`Research-Lab self-test: 616 passed, 0 failed`, exit 0; +10 from the 606 baseline).
