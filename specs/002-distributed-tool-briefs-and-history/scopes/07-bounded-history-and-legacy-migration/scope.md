...
**Allowed file families:** new storage/history/index/pointer/migration/whole-graph validator modules; generated `briefs/**` fixtures/artifacts; compatibility projection generation; Scope 07 tests; read-only access to actual `brief-history.jsonl` and Git evidence.

**Excluded surfaces:** direct edit or append to `brief-history.jsonl`, source acquisition, owner formulas/reads, author/reducer behavior, final narrative policy, scheduler commit/push integration, UI renderer/pages, dependency files, other specs, and unrelated dirty/untracked paths.
...
TP-07-06 is a standalone direct-Node load script, not a `node:test` suite. Its family runner expands `tests/distributed-briefs*.load.mjs`, invokes every match with plain `node`, and fails on the first nonzero script. Reachability discovery must preserve that runner class rather than treating the family as a `node --test` argument.
...
| Load | load | SCN-002-007, SCN-002-008 | `tests/distributed-briefs.history.load.mjs` - `Load: 31-day four-window history stays bounded to 124 authoritative references` | `for test_file in tests/distributed-briefs*.load.mjs; do node "$test_file" \|\| exit 1; done` | Yes | Red: partition/index/body/file caps or idempotency breach; Green: measured production projection remains within every declared budget. |
...
- [x] SCN-002-007 reads one tool current record and one relevant monthly partition: one small current pointer plus one complete tool brief answer current state, the compact index plus at most one per-tool monthly partition answer recent history, and no unrelated source, final narrative, legacy global history, or unselected partition is required. — Evidence: [report.md](report.md#tp-07-01-unit-smallest-partition-history-selection-red-before-green) (RED with the `selectHistory` tool-key filter disabled returned two partitions where one was expected; GREEN resolves exactly one tool+month partition and a closed refusal for an absent selector) and [report.md](report.md#tp-07-07-regression-e2e-scn-002-007-focused-resolution-red-before-green) (RED `exactly one monthly partition answers recent history  3 !== 1`; GREEN resolves current plus focused history through the pointer, one read object, and one monthly partition, reading no unrelated tool object, no final narrative, no evidence bundle, and no global-history stream).
- [x] SCN-002-008 appends tool final run recommendation and evidence history atomically: unchanged narrative content is referenced rather than copied, duplicate event IDs, changed prefixes, sealed edits, malformed rows, index disagreement, or mixed-run refs refuse the publish set, and rebuilding indexes from authoritative rows reproduces the same canonical metadata without changing history. — Evidence: [report.md](report.md#tp-07-02-unit-byte-deterministic-idempotent-projectionindex-red-before-green) (RED nonce broke byte determinism; GREEN yields byte-identical objects, index fingerprint, and pointers, and `regenerateIndexes` reproduces the same fingerprint and partition metadata), [report.md](report.md#tp-07-03-functional-append-onlyindex-corruption-fails-closed-red-before-green) (each of `duplicate-event`, `prefix-mutation`, `sealed-partition-edit`, `malformed-row`, and `index-mismatch` fails closed with `B002-HISTORY` and no pointer candidate), and [report.md](report.md#tp-07-08-regression-e2e-scn-002-008-append-only-authority-red-before-green) (RED dropped prior prefix; GREEN proves byte-identical duplicate projection, on-disk index rebuild with both runs' rows, and pointer-swap rollback preserving both append-only rows).
- [x] SCN-002-009 migrates the immutable actual brief-history corpus: all actual source rows and every timestamp and window are accounted for exactly once, exact duplicate content is stored once with every occurrence retained, unproven per-tool briefs, recommendations, and final narratives are explicitly unavailable rather than reconstructed, and the legacy file remains byte-identical. — Evidence: [report.md](report.md#tp-07-05-integration-actual-corpus-migration-parity-red-before-green) (RED `one occurrence per actual row  52 !== 55`; GREEN maps the derived row count one-to-one, preserves every raw-row fingerprint and window/time coverage, retains every duplicate occurrence, marks tool-brief/recommendation/final explicitly unavailable, and leaves the file byte-identical), [report.md](report.md#tp-07-09-regression-e2e-scn-002-009-immutable-byte-migration-red-before-green) (RED `occurrence-count-mismatch  rows=55 occurrences=52`; GREEN migrates every actual row with deterministic legacy keys, exact source-row fingerprints, explicit unavailable gaps, and no narrative body), and [report.md](report.md#tp-07-10-integration-actual-corpus-no-write-migration-command-green) (`bytesUnchanged: true`, identical before/after SHA-256, `explicitUnavailable` 55/55/55, `git status --porcelain brief-history.jsonl` empty).
