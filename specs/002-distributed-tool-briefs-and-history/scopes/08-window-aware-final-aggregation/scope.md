...
**Allowed file families:** final compaction/validation/group consumption in `rlcontracts.js`; final request path in `scripts/brief-author.mjs`; final barrier orchestration in `scripts/brief-refresh.mjs`; `FinalBrief/v1`; Scope 08 tests/external-author contract fixtures.

**Excluded surfaces:** source/owner formulas, registry/read/tool-author/lifecycle behavior, storage projection except consuming current refs, scheduler Git publication, UI rendering, dependency files, other specs, and unrelated dirty/untracked paths.
...
| Stress | stress | SCN-002-025 | `tests/distributed-briefs.final-budget.stress.mjs` - `Final budget boundary refuses honestly and never truncates mandatory material under sweep` | `node --test tests/distributed-briefs.final-budget.stress.mjs` | No | Red: at some intermediate budget the compactor sheds a source envelope, group, or run-context field to fit the cap, degrades silently instead of refusing, or becomes non-monotone; Green: sweeping the whole accept/refuse boundary across three scenario shapes in both windows keeps every mandatory field byte-identical to the unconstrained result, sheds only `optionalFacts`, and refuses only with `B002-BUDGET` / `final-mandatory-material-exceeds-cap`. |

### Definition of Done - Tiered Validation

Core outcomes:

- [x] One final author starts only after every derived source read/brief outcome validates and receives a bounded complete registry/owner/evidence/lifecycle/group/window envelope with no raw history or recursive Market Brief input. — Evidence: [report.md](report.md#red-before-green-scn-002-025-final-compaction-retains-every-source-owner-ref-window-field) (TP-08-04 barrier + SCN-002-025 RED→GREEN; real evidence)
- [x] Final validation accounts for the current 23/22 canary and runtime-derived IDs, preserves origins/conflicts/exclusions/clocks/provenance, and rejects unsupported actions or confidence inflation. — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (TP-08-03 functional + TP-08-05 integration; real evidence)
- [x] Pre-market, morning, pre-close, and after-hours outputs satisfy SCN-002-025; low-noise owner/persistence/corroboration policy satisfies SCN-002-027 with educational/privacy/proxy labels intact. — Evidence: [report.md](report.md#four-window-policy-matrix-rlcontractsjswindowpolicy-derived-validated-by-tp-08-06) (four-window matrix + TP-08-06/07; real evidence)
- [x] SCN-002-027 prevents an unusual extended-hours or report observation from becoming an unsupported action: with no eligible owning interpretation and no structural-persistence or independent-corroboration condition, the observation remains context or no-action carrying its provenance and suppression reason, action/attention/confidence counts do not increase, shared evidence counts once, any owner/provider conflict remains visible, and the output is never labeled fund flow, official extended-hours close, personalized advice, or execution instruction. — Evidence: [report.md](report.md#red-before-green-scn-002-027-low-noise-gate-owner-persistencecorroboration) (TP-08-02 unit RED at `unit:100` on the `action` versus `context` discriminator → GREEN `SCN-002-027: low-noise gate requires owner plus structural persistence or independent corroboration`; TP-08-05 integration keeps owner disputes, thin baselines, and shared source origins as context or conflict; TP-08-07 e2e GREEN `Regression: SCN-002-027 unsupported unusual evidence remains educational context with zero action-slot impact`; confidence-averaging scan found only the inverse comment, group merges use minimum-retained confidence).
- [x] Consumer and Shared Infrastructure Impact Sweeps, independent final-policy canaries, rollback, and the declared Change Boundary are complete with unrelated dirty paths unchanged and unstaged. — Evidence: [report.md](report.md#consumer-and-shared-infrastructure-impact-sweep) (Consumer + Shared-Infra sweeps: none found; real evidence)
- [x] Change Boundary is respected and zero excluded file families were changed — every enumerated excluded surface (source/owner formulas, registry/read/tool-author/lifecycle behavior, storage projection beyond consuming current refs, scheduler Git publication, UI rendering, dependency files, other specs) is byte-identical. — Evidence: [report.md](report.md#code-diff-evidence) (`git status --porcelain` shows only the Scope 08 surface; the `shasum -a 256` byte-identity audit after both RED-before-GREEN cycles records the four production files restored byte-exact; "No source/owner formula, registry/read/tool-author/lifecycle module, storage projection, scheduler, or UI file was changed" and the shared-infrastructure files "reported zero modification").

Test evidence items, one per Test Plan row:

- [x] [TP-08-01] Unit evidence passes for complete exact-cap final compaction after its recorded red stage. — Evidence: [report.md](report.md#red-before-green-scn-002-025-final-compaction-retains-every-source-owner-ref-window-field) (SCN-002-025 RED unit:64 → GREEN; real evidence)
- [x] [TP-08-02] Unit evidence passes for owner/structural/persistence/corroboration low-noise promotion after its recorded red stage. — Evidence: [report.md](report.md#red-before-green-scn-002-027-low-noise-gate-owner-persistencecorroboration) (SCN-002-027 RED unit:100 → GREEN; real evidence)
- [x] [TP-08-03] Functional evidence passes for every external final-author adversarial response. — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (final-author.functional GREEN; real evidence)
- [x] [TP-08-04] Integration evidence passes for the registry-derived final barrier, with 23 participants/22 sources retained only as the current-repository canary. — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (final.integration barrier GREEN; real evidence)
- [x] [TP-08-05] Integration evidence passes for dispute/thin/shared-origin context treatment. — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (final.integration dispute/thin GREEN; real evidence)
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-002-025 pass with the exact title. [TP-08-06] — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (final.e2e four-window regression GREEN; real evidence)
- [x] [TP-08-07] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-002-027 pass with the exact title. — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (final.e2e low-noise regression GREEN; real evidence)
- [x] Broader E2E regression suite passes for author-lifecycle-history-final behavior. [TP-08-08] — Evidence: [report.md](report.md#tp-08-08-tp-08-09-broader-author-lifecycle-history-final-e2e-regression-and-baseline-repository-selftest-green) (authorship+history+final 7/7; real evidence)
- [x] [TP-08-09] Baseline functional evidence passes for `node scripts/selftest.mjs` after focused checks are green. — Evidence: [report.md](report.md#tp-08-08-tp-08-09-broader-author-lifecycle-history-final-e2e-regression-and-baseline-repository-selftest-green) (selftest 633/0; real evidence)
- [x] [TP-08-10] Stress evidence passes for the final input/output/run budget boundary: sweeping the whole accept/refuse range across three scenario shapes in both authoring windows proves the budget is enforced without truncating participant, source-envelope, group, conflict, or provenance material, refuses only with the documented `B002-BUDGET` / `final-mandatory-material-exceeds-cap` contract error, stays monotone, and is byte-stable under repetition. — Evidence: [report.md](report.md#tp-08-10-final-budget-boundary-stress-2026-07-29) (2 passed / 0 failed exit 0; 6/6 injected faults caught by the adversarial proof; real evidence)

Build quality gate:

- [x] Exact Node checks, completeness/budget/privacy/unsafe-output/internal-mock/self-validation scans, consumer search, artifact validation, diff isolation, and full output are recorded in this scope report with zero warning or undeclared mutation. — Evidence: [report.md](report.md#focused-scope-08-suite-tp-08-01-tp-08-07-all-four-final-test-files-green) (node --test + selftest 633/0, consumer/shared-infra sweeps, sha256 byte-identity diff isolation; real evidence)
