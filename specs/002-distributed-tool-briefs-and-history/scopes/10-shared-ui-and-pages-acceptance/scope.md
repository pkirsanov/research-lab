...
| CPI dispute/revision (SCN-002-023/024) | Disputed or revised evidence | Expand provider/revision rows | Distinct provider values with synthesis blocked; original and revision rows/times both visible | `Regression: report conflicts stay separate and revisions append without rewriting the original` |
...
**Allowed file families:** `rlbrief.js`, narrow `rlapp.js` mount bridge, declarative anchors/mode-target metadata on all registered HTML pages, static test server/runtime, complete generated compatibility outputs, Scope 10 tests, and product docs later owned by `bubbles.docs` (not this plan/implementation scope owner).

**Excluded surfaces:** owner formulas/controls/canvases/local state, page-specific renderer branches, navigation redesign, broad CSS restyling, source/author/history/scheduler logic, service worker/client persistence, credentials/private data, dependency files, other specs, and unrelated dirty/untracked paths.
...
- [ ] SCN-002-015 keeps shared brief surfaces usable and safe on every registered page: controls remain labeled, focusable, ordered, and at least 44 CSS pixels without overlap, clipping, or horizontal overflow; authored markup renders literally while only validated registry or HTTPS citation links navigate; the compatibility projections and current pointer select the same run; and registry membership alone supplies the mount without a page-ID switch or hand-maintained list. — UNCHECKED, honest gap. Three of the four clauses are proven by [report.md](report.md#test-evidence): spec case 12 `…ory UI is accessible safe and stable at desktop mobile and zoom`, spec case 13 `…y source receives the shared mount with no page-specific branch`, TP-10-03 ui-canary across all 23 registered pages, and TP-10-01 renderer-unit escaped-text/safe-link assertions. The pointer-coherence clause ("the compatibility projections and current pointer select the same run") has NO positive proof in this scope's report: TP-10-21 `node scripts/validate-distributed-briefs.mjs --root .` reports only `ok:true` and reconciles vacuously because no current pointer is published in the repository root (the identical command recorded in Scope 09's report prints `"present": false, "reason": "no-current-pointer-published"` and `"compatibilityProjection": { "pointerBound": false }`), and TP-10-03 records the mount as inert (`idle`, zero briefs network requests) before the cutover. This item stays unchecked until a published pointer makes the clause positively verifiable; routing to the report/state owner.
- [x] Consumer and Shared Infrastructure Impact Sweeps, independent all-page boot/control canaries, rollback, and the declared Change Boundary are complete with unrelated dirty paths unchanged and unstaged. — Evidence: [report.md](report.md#test-evidence) (ui-canary all-page boot/control canary + consumer-trace 1/1; no prior-scope files touched; +2-line anchors; rollback is additive mount/bridge removal).
- [x] Change Boundary is respected and zero excluded file families were changed — every enumerated excluded surface (owner formulas/controls/canvases/local state, page-specific renderer branches, navigation redesign, broad CSS restyling, source/author/history/scheduler logic, service worker/client persistence, credentials/private data, dependency files, other specs) is byte-identical. — Evidence: [report.md](report.md#regression-remediation-evidence-brief-view-reveal-restore-2026-07-27) (`git diff --stat` = `2 files changed, 14 insertions(+), 1 deletion(-)`); [report.md](report.md#lint-and-quality) ("No prior-scope files touched (working-tree changes confined to the Scope-10 surface); each of the 23 HTML edits is +2 lines.").
