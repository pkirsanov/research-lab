...
- **Historical-only routing notice for the preceding delta handoffs:** their `CMD-COLLISION remains intentionally red` wording records prior transition epochs only. Those parser obligations and the v14 scoped-authority successor are implemented history and do not route current work. Current acceptance routes only `F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001` to `bubbles.test`, followed by independently evidenced Scope 1 batches, without weakening any inherited check or unlocking Scope 2.
...
- **Historical disposition:** strict v3 parser adoption, the former `TR-BUG-002-IMPLEMENT-DIAGNOSTICS-01` prerequisite, the Scope 1 scoreability/reader foundation, and the v14 scoped-evidence transition are complete historical records; none routes current work. Current execution routes `F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001` to `bubbles.test`, then permits separately committed evidence batches under the v15 transition contract. Planning does not relabel any owner receipt or prior green run as planning evidence.
...
- **Current route and lock:** after this planning handoff, `bubbles.test` owns `F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001`, followed by independently evidenced Scope 1 batches. This planning change records no evidence, checks no item, performs no status or certification transition, and makes no completion claim. Scope 1 remains In Progress and Scope 2 remains locked and Not Started.
...
| CMD-COLLISION | `node --test tests/feature-004-dirty-tree-collision.test.mjs` | Unchanged three-case acceptance command with a 600-second execution ceiling. V12 and every predecessor remain immutable history; v13 and v14 remain fully parsed historical authority. Acceptance requires the one marker-pinned v15 cardinality-only successor, exact v14 planning/inline-evidence grammar and field validation, one-or-more independently evidenced transitions in required mode, zero changes in `allowNoTransition` mode, exact transitioned-key audit output, duplicate-proof rejection, bounded foreign-inventory equality, Feature 017 exclusion, path-limited staging, and every inherited v14/v13 eligibility, representation, lineage, performance, case, assertion, order, and failure check without removal or weakening. |
...
The committed Scope 1 source and tests now contain the scoreability/reader foundation and the v14 report/parser successor, but the v14 singular transition validator permits only one newly checked item per evidence commit. `bubbles.test` therefore owns `F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001` next, then owns the independently evidenced TP-01-23 through TP-01-31 and protected provider/Bond/Causal batches. Planning records no execution result and does not reuse a prior green run as evidence. Scope 1 cannot become Done, and Scope 2 cannot start, until every strengthened Scope 1 item has current test-owned evidence and `bubbles.validate` legitimately closes Scope 1. The prior v9, foundation, v14, and provider-only routes remain historical records; none is the current implementation target.
...
| TP-01-22 | GRILL-004-09 / F004-V13-CAPTURE-SHAPE-001 / F004-V14-SCOPED-EVIDENCE-AUTHORITY-001 / F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001 | Collision and evidence-history preservation | functional | `tests/feature-004-dirty-tree-collision.test.mjs` / all three existing top-level test cases | CMD-COLLISION | No | Preserve and parse every v12/v13/v14 byte and exact historical two-LF separators; adopt one v15 cardinality-only successor; accept two valid independently evidenced item transitions; reject zero required transitions, mixed-invalid batches, duplicate keys or cross-item evidence hashes, malformed zero/eight-space evidence indentation, every evidence-field forgery, prior-evidence or item-projection mutation, status/certification edits, foreign persistence, separator drift, and incomplete staging; retain the unchanged command, 600-second ceiling, three cases, and every inherited assertion, order, and failure condition. |
