...
| CMD-COLLISION | `node --test tests/feature-004-dirty-tree-collision.test.mjs` | Read-only validation of the immutable original baseline, independent `rldata.js` supersession, exact five-hash disposition, historical one-path delta, final settled-owner delta, every inherited current/index/hunk identity, untracked validator prefix, and conditional just-in-time config checkpoint |
...
| TP-01-22 | GRILL-004-09 | Collision preservation | functional | `tests/feature-004-dirty-tree-collision.test.mjs` / `Feature 004 preserves every pre-existing dirty hunk` | CMD-COLLISION | No | Original baseline, independent `rldata.js` supersession, and five-hash disposition remain exact; immutable delta history first overlays only `scripts/selftest.mjs` hunk 7 from `0f9739b064bc90a02c3baf5a1014442b8f566ad9f88dd3528c1103a462c55e1b` to `ba4b911411a53fe83c6d9c99cce505f28b9cb0d38c88eae22eabb578f59e7c80`, then exactly one settled block extends delta raw hash `334cae6ba3d95ad3837971ee3a402a68ffb46df23f490a31104d94cd73ea0e4b` and overlays only hunk 7 to `15ff8c7662995bbc7e977c2ea57bb95c5ac64d494a43f4bdc1d64ee81e42f943` under exact current identity, Feature 006 marker inventory, owner state/report/tool-log evidence, unrelated 491/1 failure, and inherited fail-closed equality |
...
- [ ] The report collision checkpoints preserve the immutable original record, accept only owner-proven replacements under complete current identities, and the Change Boundary contains zero unexplained excluded-file changes.
    ...
    > **What was attempted:** Planning reconstructed all five original hunk bodies, matched named owner packets and executable evidence, and wrote the closed additive checkpoint.
    > **What was observed:** The current collision parser recognizes only the earlier `rldata.js` checkpoint and still rejects the five newly reviewed hashes.
    > **Why this is uncertain:** The test-owned parser has not yet enforced the new closed schema and complete path identities.
    > **What would resolve this:** `bubbles.test` implements the exact parser handoff and executes CMD-COLLISION successfully without changing source or weakening any fail-closed branch.
...
- [ ] TP-01-22 collision test enforces the immutable baseline, both additive checkpoints, both complete shared-path identities, and every remaining fail-closed contract without staging or destructive Git action.
    > **Uncertainty Declaration**
    > **What was attempted:** Planning supplied the exact five-record schema and parser requirements while preserving the foreign-owned test.
    > **What was observed:** No passing CMD-COLLISION execution exists for the new checkpoint because the parser remains test-owned and unchanged.
    > **Why this is uncertain:** A planning checkpoint cannot establish that the foreign test parses and rejects the new schema correctly.
    > **What would resolve this:** `bubbles.test` adds adversarial parser coverage, runs CMD-COLLISION, and records the actual output under its own phase.
...
**Collision prerequisite:** all recorded baselines for `tools.json`, `index.html`, `rlnav.js`, `market-brief.html`, `notes/market-brief.md`, `README.md`, `notes/README.md`, `scripts/selftest.mjs`, and the untracked `scripts/validate-brief-payload.mjs` must match immediately before editing. `market-brief.config.json` requires a fresh just-in-time checkpoint because its reported concurrent mutation is not represented by the planning-time clean hash. Any mismatch blocks the path until the concurrent bytes are re-baselined and reviewed.
...
| TP-04-20 | All Scope 4 | Collision preservation | functional | `tests/feature-004-dirty-tree-collision.test.mjs` / `Feature 004 preserves every pre-existing dirty hunk` | CMD-COLLISION | No | Original baseline, independent `rldata.js` supersession, and five-hash disposition remain exact; immutable delta history first overlays only `scripts/selftest.mjs` hunk 7 from `0f9739b064bc90a02c3baf5a1014442b8f566ad9f88dd3528c1103a462c55e1b` to `ba4b911411a53fe83c6d9c99cce505f28b9cb0d38c88eae22eabb578f59e7c80`, then exactly one settled block extends delta raw hash `334cae6ba3d95ad3837971ee3a402a68ffb46df23f490a31104d94cd73ea0e4b` and overlays only hunk 7 to `15ff8c7662995bbc7e977c2ea57bb95c5ac64d494a43f4bdc1d64ee81e42f943` under exact current identity, Feature 006 marker inventory, owner state/report/tool-log evidence, unrelated 491/1 failure, inherited fail-closed equality, and the unchanged volatile-config checkpoint rule |
...
- [ ] TP-04-20 proves every pre-existing dirty hunk survives and the volatile config was checkpointed immediately before its edit.
