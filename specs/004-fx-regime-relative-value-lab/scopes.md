...
- **Historical-only routing notice for the preceding delta handoffs:** their `CMD-COLLISION remains intentionally red` wording records prior transition epochs only. Those parser obligations plus the v14 scoped-authority and v15 multi-item successors are implemented history and do not route current work. Current acceptance routes only `F004-V16-HERMETIC-HISTORICAL-V13-001` to `bubbles.test`, without weakening any inherited check or unlocking Scope 2.
...
- **Historical disposition:** strict v3 parser adoption, the former `TR-BUG-002-IMPLEMENT-DIAGNOSTICS-01` prerequisite, the Scope 1 scoreability/reader foundation, the v14 scoped-evidence transition, and the v15 multi-item transition are complete historical records; none routes current work. Current execution routes only `F004-V16-HERMETIC-HISTORICAL-V13-001` to `bubbles.test`. Planning does not relabel any owner receipt or prior green run as planning evidence.
...
### Feature 004 V15 Multi-Item Evidence Transition Successor (`F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001`)

- **Narrow purpose and supersession:** v15 changes only post-v15 execution-transition cardinality. The v14 singular rule remains exact historical truth: its `allowedTransition` permits one matching unchecked item to become checked with correctly phase-owned executed evidence, and its validator requires transition count one unless `allowNoTransition` is active. V15 supersedes only that cardinality rule for transitions after v15 adoption; it does not reinterpret any transition accepted under v14.
- **Immutable v14 authority reuse:** the complete v14 report block, raw pin, two-LF suffix, parser branch, scoped authority paths, planning projection, execution item keys, evidence grammar, evidence-field validation, bounded capture rule, staged allowlist, and every inherited v13/predecessor check remain byte-for-byte parsed inputs. V15 does not recapture owner, planning, source, test, foreign, or repository authority; redefine a v14 path; relax a v14 field; or omit a v14 validation. The additive v15 report block contains only the cardinality-successor contract and its predecessor link.
- **Commit-relative baseline without authority recapture:** every candidate execution transition compares the exact committed `HEAD:specs/004-fx-regime-relative-value-lab/scopes.md` projection at transaction start with the candidate worktree/index `scopes.md` projection. The v15 implementation transaction uses `allowNoTransition` and may stage only report/parser; an evidence transaction uses required mode and may stage only `scopes.md`. After each evidence commit, that committed `scopes.md` becomes the immutable parent baseline for the next batch. State routing and certification never share the evidence commit. V15 stores no replacement planning/source authority snapshot in its report block.
- **Required multi-item transition:** one evidence commit may transition one or more matching unchecked items to checked. Every transitioned item retains the exact key `scopeId + itemOrdinal + uncheckedItemTextSha256`; keys are unique; transition count equals the exact number of newly checked items; required mode rejects zero; no hardcoded upper bound exists; and the validator returns the exact transitioned-key list in planning order for audit. `allowNoTransition` remains a separate mode and requires exactly zero checkbox or evidence changes.
- **Independent evidence validation:** every newly checked item appends at least one syntactically valid evidence block. Every appended block is independently validated for closed phase, exact nonempty command, zero exit code, `executed` claim source, at least ten literal raw-output lines, exact raw-output SHA-256, and exact block SHA-256. The evidence grammar remains the v14 four-space form for every header, fence, and raw-output line. Zero-space and eight-space evidence forms are malformed and must not be removed from the planning projection or admitted as evidence.
- **No cloned proof by default:** evidence-block hashes must be unique across distinct transitioned item keys. V15 defines no reference-relationship schema, so duplicate evidence hashes across different item keys are rejected. A later explicit policy successor would be required to permit a relationship proving that one exact command directly establishes two item claims. Unique evidence per item is the v15 path.
- **Immutable transition surface:** existing checked items and all existing evidence remain immutable and prefix-preserved. Unchecked items cannot gain evidence. No execution transition may change item text, item order, item cardinality, item ordinal, scope ID, planning projection, Test Plan projection, scope status, top-level status, planning-owned routing, or certification. A mixed batch is atomic: one invalid item or evidence block rejects the entire transition and returns no accepted transitioned-key list.
- **Adversarial closure:** TP-01-22 must reject required mode with zero transitions; a mixed valid/invalid batch; a duplicate item key; a duplicate evidence hash across item keys; checked-without-evidence; evidence under an unchecked item; phase forgery; wrong command, exit code, claim source, raw output, raw-output hash, or block hash; prior evidence removal, replacement, or reorder; item text, order, cardinality, ordinal, or planning-projection mutation; status or certification edits; a persisted foreign path; any v14 byte mutation; any v14-to-v15 two-LF separator mutation; an incomplete or overbroad staged allowlist; and zero-space or eight-space evidence indentation. It must accept two independently evidenced valid transitions and return both exact item keys.
- **Final report successor:** `bubbles.test` appends exactly one `feature004-dirty-collision-multi-item-evidence-v15` report block after the immutable v14 block and exact two-LF separator. V15 becomes report-final with exactly two LF bytes, carries its own marker-inclusive raw SHA-256 pin, and preserves the unchanged three-case CMD-COLLISION command, 600-second ceiling, and every inherited v14/v13 assertion, case, order, and failure condition.
- **Owner and commit sequence:** (1) `bubbles.plan` commits only `scopes.md`, `test-plan.json`, and planning-owned `state.json.execution` routing after exact cached-path verification; (2) `bubbles.test` appends v15 and implements its parser successor, then commits only `report.md` and `tests/feature-004-dirty-tree-collision.test.mjs`; (3) subsequent `bubbles.test` evidence batches may transition multiple independently evidenced items and commit only `scopes.md`; (4) test-owned execution state and validate-owned certification remain separate state transactions. No step may stage a foreign or undeclared path.
- **Historical disposition and lock:** the v15 report and parser successor are immutable implemented history and no longer route current work. V16 may consume them only as exact predecessor input. This planning change records no evidence, checks no item, performs no status or certification transition, and makes no completion claim. Scope 1 remains In Progress and Scope 2 remains locked and Not Started.

### Feature 004 V16 Hermetic Historical V13 Successor (`F004-V16-HERMETIC-HISTORICAL-V13-001`)

- **Finding and bounded cause:** `F004-SCOPE01-COMMITTED-COLLISION-DEPENDENCY-001` is the only routed successor. The post-v14 adapter `feature004V14RunInheritedV13AdversarialCases()` currently reaches the live-current `runPostCommitV13AdversarialCases()` path, so an absent uncommitted foreign artifact can fail committed-only acceptance. V16 changes only that post-v14 historical replay. It does not alter direct pre-v14 v13 current validation, any current v14 authority check, or any product behavior.
- **One additive report-final successor:** `bubbles.test` appends exactly one `feature004-dirty-collision-hermetic-historical-v13-v16` block after the immutable report-final v15 block and the exact existing two-LF separator. V16 ends with exactly two LF bytes and adds exactly one `HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256` pin plus one closed parser branch. Every v15, v14, v13, v12, and predecessor byte, pin, schema, field, order, separator, hash, length, selector, normalization rule, and meaning remains immutable. V16 captures no current owner, planning, source, test, foreign, repository, or certification authority.
- **Authority split:** v14 remains the sole current-file authority after v14 adoption. Direct pre-v14 v13 validation against the then-current files remains immutable historical behavior but is never invoked as post-v14 committed-only acceptance. The v16 adapter replaces only the post-v14 inherited v13 adversarial replay with a hermetic replay over the already pinned v13 payload and recorded immutable sources. A current declared Scope 1 authority path still flows through v14 and must still reject on mutation.
- **Exact historical payload validation:** before any replay, v16 recomputes and validates the exact v13 marker-inclusive raw pin, envelope hash and length, compressed hash and length, decoded hash and length, canonical JSON bytes, closed top-level and nested schemas, ordered fields and records, internal required/foreign matrix, complete inventory integrity, predecessor links, protected-clean closure, report-prefix authority, semantic selectors, pin-family normalization, and every existing closed-schema mutation. The required/foreign matrix and inventory checks named here are the v13 payload's own internal structures and remain mandatory. Missing, extra, duplicate, reordered, broadened, or changed data rejects before adversarial execution. Re-assertion of a v12-epoch identity over records derived from this payload is separately bounded by the epoch rule below and covers the Feature-004-owned required set only.
- **Opaque foreign commitments:** every v13 dirty or untracked foreign record is an opaque pinned historical commitment. V16 validates every captured field and every internal cross-link inside the immutable payload without opening, hashing, statting, resolving, or otherwise reading the current path. Historical clean Git-tree records may be reconstructed only from the exact commit, tree entry, object identity, and bytes already named by the immutable v13 sources. Historical stripped-parser records may be reconstructed only from their recorded immutable parser source and normalization contract. No historical dirty or untracked record may be reconstructed from live bytes, and no current file can satisfy or replace a missing historical commitment. Historical v12 authority for a foreign record is derived from the recorded `v12.protectedAuthorityClosure`, following the established v14 derivation already used by the historical v12 observation and the hermetic v12 parser reconstruction. No foreign record is re-identified against a v12 record commitment.
- **V12 epoch boundary and reachable authority (amended):** the preceding v16 contract text required the hermetic builder to re-assert every immutable v12 identity commitment for all records derived from the pinned v13 payload, including the aggregate v12 record-matrix and inventory hashes. That requirement is unsatisfiable for any foreign path whose owner committed between the v12 and v13 epochs. `specs/004-fx-regime-relative-value-lab/design.md` proves it. The v12 epoch recorded that path (foreign record index 2, owner `bubbles.design`) as a dirty `foreign-specialist-artifact`. The path was then committed at `ba8758fa`, which descends from the v12 capture head `37de4610`, and its worktree is clean today. Its status, head object, last commit, worktree hash, and hunk set therefore all changed legitimately, and its observed identity `23c977c9` cannot equal the required v12 commitment `c36453d3`. No field conversion of a v13-epoch record can regenerate a v12-epoch identity. The epoch model already retired this comparison: the inherited validator is re-invoked with `verifyCurrent = false`, the v13 history banner reports `liveComparison=false`, and the framework already treats a superseded epoch's live-current comparison as forbidden rather than optional. V16 therefore keeps the v12 identity re-assertion for the Feature-004-owned required records, which stay reachable through the recorded hermetic parser reconstruction, and drops that re-assertion for foreign records together with the aggregate v12 record-matrix and inventory hash re-assertion inside the hermetic builder. This removes no coverage. The v12 record-commitment assertion and each of its existing rejects stay in force at the v12 epoch inside the v12 validation and adversarial path, and v16 drops only the duplicate cross-epoch re-invocation. This narrowing grants no permission to open, hash, stat, resolve, or otherwise read a current foreign path, and every foreign v13 record remains an opaque pinned historical commitment whose captured fields and internal cross-links stay fully validated inside the immutable payload.
- **Inherited adversarial and equivalence closure:** retain all nine planned v13 promotion adversaries in synthetic fixture repositories or synthetic immutable records, including the eligible untracked and unstaged exact descendant accepts, representation and staged-state rejects, lineage and `L:path` rejects, independent subset acceptance, and all content/path/type/mode/index-flag/selector/authority/inventory rejects. Preserve path-copy versus deep-clone byte equivalence, ordered labels and count, canonical source immutability, changed-ancestor copy and untouched-container identity, single `settledPaths` reuse, parser-pin mutations, and every v12/predecessor adversary without removal, reorder, weakening, or timeout relaxation.
- **V16 adversarial closure:** the existing TP-01-22 row must additionally prove that an absent current foreign path passes; arbitrary current bytes at a historically named path cannot influence replay; a changed, missing, reordered, or duplicated opaque commitment rejects; substituting live bytes for any historical dirty or untracked commitment rejects; and mutating a v14-declared current Scope 1 path still rejects through v14. A focused diagnostic may run `node --test --test-name-pattern='Feature 004 collision disposition parser fails closed on malformed records' tests/feature-004-dirty-tree-collision.test.mjs`, but it cannot replace the unchanged three-case acceptance command.
- **Acceptance and count invariants:** extend existing TP-01-22 only. Keep exactly five scopes, 33 `SCN-004-*` scenarios, 94 Test Plan rows, the same three top-level collision cases, exact command `node --test tests/feature-004-dirty-tree-collision.test.mjs`, and the 600-second ceiling. Preserve every inherited assertion, case, title, order, failure condition, and v15 multi-item contract. No new scenario, Test Plan row, checklist item, count, or acceptance substitute is authorized.
- **Ownership, commit, and lock:** planning may change and commit only `scopes.md`, `test-plan.json`, and planning-owned `state.json.execution` with `git commit --only` after exact path verification. `bubbles.test` may change and commit only `report.md` and `tests/feature-004-dirty-tree-collision.test.mjs`. Planning explicitly authorizes `bubbles.test` to update the single `HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256` pin and the corresponding v16 report block claims, including `v12Authority` and `providerResults`, so the recorded block text matches this amended contract. Planning also explicitly authorizes `bubbles.test` to re-pin any recorded v14 or v16 planning-authority commit prefix, identity, or hash for `scopes.md`, `test-plan.json`, and `state.json` that changes solely because of this planning commit, which supersedes the previously pinned planning commit `1e3abdd9`. Both authorizations are exhaustive. They cover no other pin, block, marker, path, count, or claim, and they authorize no evidence, checkbox, status, or certification transition. No parser implementation, report append, evidence, checkbox, status, certification, Scope 2, foreign artifact, or unrelated dirty/staged byte belongs to this planning transaction. Scope 1 remains In Progress and Scope 2 remains locked and Not Started.
...
| CMD-COLLISION | `node --test tests/feature-004-dirty-tree-collision.test.mjs` | Unchanged three-case acceptance command with a 600-second execution ceiling. V12 and every predecessor remain immutable history; direct pre-v14 v13 current validation remains immutable history; v14 remains the sole current-file authority; and v15 remains the immutable multi-item successor. Acceptance additionally requires one marker-pinned v16 branch that validates every raw/enveloped/compressed/decoded v13 commitment from recorded immutable sources, never current historical dirty/untracked paths, while preserving exact v14 current-path rejection, v15 transition behavior, bounded foreign-inventory equality, Feature 017 exclusion, path-limited staging, and every inherited eligibility, representation, lineage, equivalence, pin-mutation, case, assertion, order, and failure check without removal or weakening. |
...
| TP-01-22 | GRILL-004-09 / F004-V13-CAPTURE-SHAPE-001 / F004-V14-SCOPED-EVIDENCE-AUTHORITY-001 / F004-V15-MULTI-ITEM-EVIDENCE-TRANSITION-001 / F004-V16-HERMETIC-HISTORICAL-V13-001 | Collision and evidence-history preservation | functional | `tests/feature-004-dirty-tree-collision.test.mjs` / all three existing top-level test cases | CMD-COLLISION | No | Preserve and parse every v12/v13/v14/v15 byte and exact historical two-LF separators; add one report-final v16 hermetic historical-v13 successor without recapturing current authority; validate exact raw/envelope/compressed/decoded commitments, internal matrix/inventory/cross-links, predecessor and protected closure, selectors, normalization, closed schemas, nine promotion adversaries, path-copy/deep-clone equivalence, source immutability, parser-pin mutations, and all predecessor adversaries; prove absent or arbitrary current foreign paths cannot influence history; reject opaque-commitment mutation or live-byte substitution; retain v14 current Scope 1 path rejection, the unchanged command, 600-second ceiling, three cases, and every inherited assertion, order, and failure condition; re-assert v12 identity commitments only for the Feature-004-owned required set, derive foreign v12 authority from the recorded protected authority closure, and assert no cross-epoch v12 identity, record-matrix, or inventory hash for foreign records inside the hermetic builder while the v12 epoch retains its own commitment assertion and rejects. |
...
- [x] The Scope 1 Consumer Impact Sweep is verified by TP-01-03, TP-01-21, and TP-01-22 across the RLFX vehicle/owner read, vehicle universe, `ToolControlBindingV1`, `FxBriefEligibilityV1`, Journey refresh, dedicated tests, v1 compatibility, navigation, breadcrumbs, redirects, API/generated clients, deep links, config, and downstream-scope consumers; zero stale first-party references remain.
    ...
    **Output:**
    ...
    > **Why this is uncertain:** The v15 report/parser successor and its adversarial cases have not been implemented or executed, so multi-item evidence batches are not yet admissible and no Scope 1 completion claim exists.
    > **What would resolve this:** `bubbles.test` appends the one v15 cardinality-only block, implements the strict multi-item validator and formatting adversaries, executes unchanged CMD-COLLISION under its 600-second ceiling, and only then records separately owned evidence batches without weakening inherited checks.
