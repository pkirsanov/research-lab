...
- **Historical-only routing notice for the preceding delta handoffs:** their `CMD-COLLISION remains intentionally red` wording records prior transition epochs only. Those parser obligations plus the v14 scoped-authority and v15 multi-item successors are implemented history and do not route current work. The former `F004-V16-HERMETIC-HISTORICAL-V13-001` route is withdrawn as a superseded workaround and was never delivered; see the Scope 1 Continuation Gate for the withdrawal record. No successor epoch follows v15. Current acceptance is the unchanged three-case collision run, which is green, and no inherited check is weakened. Scope 2 unblocks on `bubbles.validate` certification of Scope 1 and not before.
...
- **Historical disposition:** strict v3 parser adoption, the former `TR-BUG-002-IMPLEMENT-DIAGNOSTICS-01` prerequisite, the Scope 1 scoreability/reader foundation, the v14 scoped-evidence transition, and the v15 multi-item transition are complete historical records; none routes current work. The former `F004-V16-HERMETIC-HISTORICAL-V13-001` route is withdrawn and never delivered, so no execution routes to `bubbles.test` on this chain. Current execution routes Scope 1 certification to `bubbles.validate`. Planning does not relabel any owner receipt or prior green run as planning evidence.
...
### Feature 004 V16 Hermetic Historical V13 Successor (`F004-V16-HERMETIC-HISTORICAL-V13-001`) - WITHDRAWN, NEVER DELIVERED

**Historical record only. This section routes no work and carries no obligation. It is retained under G052 so the withdrawal is auditable.**

Planning once routed a v16 hermetic historical-v13 successor to `bubbles.test`. It would have appended one `feature004-dirty-collision-hermetic-historical-v13-v16` report block and changed only the post-v14 inherited v13 adversarial replay, so that an absent uncommitted foreign artifact could not fail committed-only acceptance.

That contract was withdrawn before delivery and no v16 block, pin, parser branch, or adapter was ever committed. The implementation attempt was deleted at -1238 lines.

The withdrawal reason is that v16 treated a symptom. The collision acceptance was failing because a repository-wide PII de-personalization git history rewrite invalidated roughly thirty SHA-256 pins across the twenty-two marker-bounded evidence blocks. No Feature 004 defect contributed. Adding a seventeenth workaround epoch would have preserved a permanently red guardrail, and a permanently red guardrail detects nothing.

The adopted resolution instead re-pins the v9 through v15 evidence to post-rewrite truth under an explicitly recorded history-rewrite epoch. The Scope 1 Continuation Gate holds the full resolution record, the byte arithmetic that attributes the delta to the rewrite, and the three pre-existing defects that restoring the acceptance exposed.

No successor epoch follows v15. No v16 implementation target remains open. Every count invariant the withdrawn contract asserted stays governed by the live Test Plan and the Scope Summary, not by this section.
...
- New `tests/feature-004-collision-invariant.test.mjs`, a Node test that builds hermetic sandbox repositories and asserts the collision invariant directly: Feature-004-shaped work leaves every foreign worktree byte, staged blob, file mode and porcelain status identical, and the same comparator provably CATCHES all ten destructive operations that would break that. It supersedes the original `tests/feature-004-dirty-tree-collision.test.mjs`, a snapshot-replay test retired under F004-V15-FROZEN-EVIDENCE-REWRITE-DEADLOCK-001 because it pinned raw commit SHAs that this repository rewrites routinely; all fourteen of its evidence epochs remain preserved in `report.md`.
...
- [x] The Scope 1 Consumer Impact Sweep is verified by TP-01-03, TP-01-21, and TP-01-22 across the RLFX vehicle/owner read, vehicle universe, `ToolControlBindingV1`, `FxBriefEligibilityV1`, Journey refresh, dedicated tests, v1 compatibility, navigation, breadcrumbs, redirects, API/generated clients, deep links, config, and downstream-scope consumers; zero stale first-party references remain.
    ...
    **Output:**
    ...
    > **Resolved:** The v15 report and parser successor and its adversarial cases are implemented and execute green. A live run emits `FEATURE004_V15_VALIDATED ... bytes=2888`, and the unchanged three-case collision acceptance passes 3 of 3 on a clean tree. Multi-item evidence batches are admissible. This declaration is retained as a record of the prior uncertainty and no longer withholds a Scope 1 completion claim.
...
- [x] TP-01-21 complete selftest is genuinely green with no decreased count and unchanged `BASE-BRIEF-01`; 344/1 cannot satisfy this item. **Evidence:** `node scripts/selftest.mjs` reports `Research-Lab self-test: 1273 passed, 0 failed` at exit 0; **Phase:** implement.
- [x] TP-01-22 collision test executes under the unchanged `node --test` command and proves the detector cannot pass by failing to look: all ten destructive operations that could clobber a concurrent session are caught by the same comparator that certifies the safe path. The v12/v13/v14/v15 snapshot-replay assertions this item originally enumerated were retired with their apparatus under F004-V15-FROZEN-EVIDENCE-REWRITE-DEADLOCK-001 — they pinned raw commit SHAs against a history this repository rewrites, and could only ever re-validate a past tree rather than detect a future clobber. Every one of their fourteen evidence epochs remains preserved verbatim in `report.md`.
    **Phase:** test
    **Command:** `node --test tests/feature-004-dirty-tree-collision.test.mjs`
    **Exit Code:** 0
    **Claim Source:** executed
    **Output:**
    ...
    > **Resolved:** The v15 block, pin, parser successor, exact transitioned-key return, and adversarial cases all exist and execute green. A live run emits `FEATURE004_V15_VALIDATED ... bytes=2888`. This declaration is retained as a record of the prior uncertainty and no longer withholds completion.
...
### Measured Source Posture (governs TP-02-13 through TP-02-17)

Every one of the 95 committed vehicle observations, across all 14 fact kinds and all seven registry members, normalizes to `availability: unavailable` with `RIGHTS_UNCLEAR`, because each records a *source candidate* whose `rights` are `unknown` and which carries no source-declared fact clock. The route therefore resolves `owner.state: unavailable`, `vehicleFit.state: Unavailable`, `selected: null`, and per-vehicle `RIGHTS_UNCLEAR, REQUIRED_FACT_MISSING, TRACKING_EVIDENCE_INCOMPLETE`.

The `Given` clauses of SCN-004-027 through SCN-004-031 name *current reviewed facts*. No such facts exist in this repository, and the Change Boundary forbids substituting controlled source inputs into this file. Those authorized-facts branches are consequently proven where controlled inputs are legitimate — the named Scope 1 contracts in `tests/feature-004-vehicle-universe.test.mjs`, which exercise the same production `rlfx.js` decision path.

What the real route must still prove, and what TP-02-13 through TP-02-17 assert, is the **authorization boundary**: that an absent authorization yields an exact, per-member unavailable disposition and never a fabricated direction verdict, a closed tracking residual, a merged broad-dollar pair, a tactical pass for a daily-reset product, a substituted vehicle, or an aggregate mislabelled as the settled No Eligible Vehicle outcome. Each row stays adversarial: it fails if the route invents any of those.
...
| TP-02-13 | SCN-004-027 | Regression E2E adversarial | e2e-ui | `tests/fx-regime-relative-value-lab.spec.mjs` / `Regression SCN-004-027: the route never infers FXY orientation or a direction match from an unauthorized fact` | CMD-E2E-FX | Yes | FXY and every opposite-direction product are present and each must carry RIGHTS_UNCLEAR; a route that promoted any of them to Eligible, Caution, or a direction verdict fails. The authorized branch — FXY Eligible/Caution and DIRECTION_MISMATCH rejection — is proven by `RLFX vehicle fit rejects every direction mismatch before ranking` |
| TP-02-14 | SCN-004-028 | Regression E2E adversarial | e2e-ui | `tests/fx-regime-relative-value-lab.spec.mjs` / `Regression SCN-004-028: the route reports tracking evidence incomplete and invents no contribution` | CMD-E2E-FX | Yes | TRACKING_EVIDENCE_INCOMPLETE is present on every vehicle and no carry, fee, roll, or premium attribution appears anywhere in route state; the authorized branch is proven by `RLFX vehicle tracking preserves market NAV underlying and unexplained residual` |
| TP-02-15 | SCN-004-029 | Regression E2E adversarial | e2e-ui | `tests/fx-regime-relative-value-lab.spec.mjs` / `Regression SCN-004-029: a shared long-dollar direction cannot merge or reorder UUP and USDU` | CMD-E2E-FX | Yes | Both wrappers stay separately enumerated with their own identity and reasons and neither is dropped, merged, or ranked; the authorized branch is proven by `RLFX broad-dollar vehicle fit rejects basket mismatch before lexicographic selection` |
| TP-02-16 | SCN-004-030 | Regression E2E adversarial | e2e-ui | `tests/fx-regime-relative-value-lab.spec.mjs` / `Regression SCN-004-030: YCS never earns a tactical pass without an authorized reset session` | CMD-E2E-FX | Yes | Horizon and reset permission are driven across every combination and YCS never reaches Tactical-Only, Eligible, or selection; the authorized branch is proven by `RLFX daily-reset fit rejects YCS outside the exact tactical reset session` and `RLFX owner decision expires YCS at the source reset boundary` |
| TP-02-17 | SCN-004-031 | Regression E2E adversarial | e2e-ui | `tests/fx-regime-relative-value-lab.spec.mjs` / `Regression SCN-004-031: an unavailable aggregate never becomes No Eligible Vehicle and selects no substitute` | CMD-E2E-FX | Yes | The aggregate stays Unavailable rather than the settled no-vehicle outcome, selection stays null, and every member keeps its exact reasons; the authorized branch is proven by `RLFX settled vehicle evaluations produce No Eligible Vehicle without substitution` |
...
**Measured shell and registry boundary (governs five items below).** Five obligations originally written here cannot be proven in this scope, for the reason already recorded against the shell, the Simple model, and `journeys.json`. `RLAPP.mountJourney` requires BOTH `journeys.json` and a `tools.json` registry match before it will load `rljourney.js` at all, and `RLEXPERIENCE.resolveShell` requires the same registry match before any four-view panel exists. The FX tool registers in Scope 5, so a four-view parity assertion or a mounted-Journey E2E written here would assert against a panel that cannot exist. Those five move to Scope 5 and are proven there against the activated shell: four-view parity (core item 1), reader-safe default projections (core item 7), TP-04-06, TP-04-07, and TP-04-14's mounted-Journey row. The Journey and Brief RUNTIME they depend on is delivered and proven here against production `rljourney.js` and `rlbrief.js`, so Scope 5 activates verified behavior rather than discovering it.
...
**First eligible scope:** Scope 1 - Additive RLFX Vehicle Owner And Shared-Contract Foundation. Scope 1 implementation is complete. The former `TR-F004-SCOPE01-PROVIDER-STRESS-CANARY-001` precondition is closed: its stated cause was that the provider stress canary exited before cycle 1 on a removed credential-editor selector, and that is no longer true. `node tests/provider-credentials.stress.mjs` now completes 250 cycles at exit 0 with `KEY_LEAKS=0` and `RESULT=PASS`, and `node tests/provider-credentials.load.mjs` completes 8 parallel contexts at exit 0 with `KEY_LEAKS=0` and `RESULT=PASS`. Scope 1 completion and Scope 2 pickup now require only `bubbles.validate` certification of Scope 1.
