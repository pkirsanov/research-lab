...
- **Ownership, commit, and lock:** planning may change and commit only `scopes.md`, `test-plan.json`, and planning-owned `state.json.execution` with `git commit --only` after exact path verification. `bubbles.test` may change and commit only `report.md` and `tests/feature-004-dirty-tree-collision.test.mjs`. No parser implementation, report append, evidence, checkbox, status, certification, Scope 2, foreign artifact, or unrelated dirty/staged byte belongs to this planning transaction. Scope 1 remains In Progress and Scope 2 remains locked and Not Started.
