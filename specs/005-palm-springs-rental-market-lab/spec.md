...
| Ocean Shores, Washington | `large-luxury-4plus` | `minBedrooms = 4`, `rentalType = entire-home`, plus a declared auditable luxury qualification | Sparse-population visibility, exact candidate/qualified/sample counts, waterfront/large-home evidence, legal-license overlap, acquisition sample, coastal downtime and cost assumptions |

> **Amendment (2026-07-21, owner-directed):** Ocean Shores' mandatory luxury boundary was switched from `large-luxury-5plus` (`minBedrooms = 5`) to `large-luxury-4plus` (`minBedrooms = 4`). Palm Springs retains `large-luxury-5plus` (`minBedrooms = 5`). The mandatory luxury segment id is now derived per-market (`LUXURY_SEGMENT_BY_MARKET` in `rlrental.js`), and each market's bedroom boundary is fixed by config and must equal the `Nplus` encoded in its id (validated by the config luxury-boundary check).

Each market has exactly one mandatory luxury segment (Palm Springs `large-luxury-5plus`; Ocean Shores `large-luxury-4plus`). A market may add an evidence-supported large-house segment with a different bedroom boundary only as an additional segment; it cannot remove or replace its mandatory luxury segment.
