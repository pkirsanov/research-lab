...
- In Simple, every bounded user assumption is adjustable by a paired slider whose companion number input remains the deterministic source of truth, and Simple presents the resulting deterministic economics as a compact model-result card shown after segment identity, evidence coverage, and the decision — never before them. Sliders drive the same single compute as Power and never mutate researched observations, evidence coverage, forecasts, legal facts, or source records.
