# User Validation: Technical Analysis Decision Lab

Planning authority: [spec.md](spec.md), [design.md](design.md), and the [scope index](scopes/_index.md). Execution evidence is recorded in the nine per-scope reports under [scopes/](scopes/).

## Journey Goal

A stock researcher can select one source-qualified instrument, explicit timeframe/session profile, comparison set, setup focus, and cost scenario, then understand whether a setup is `NO EDGE`, `MIXED`, `WATCH`, `ARMED`, `TRIGGERED`, `INVALIDATED`, `EXPIRED`, or `COMPLETED EVALUATION` and why, without correlated indicator inflation, hidden source assumptions, actor/motive claims, validation inheritance, or pressure to trade.

## Success Signal

Simple presents one source/vintage-faithful UnifiedRead with direction, regime, setup state, five gate outcomes, selected and competing candidates, trigger, invalidation, natural target path, gross/net asymmetry, support, contradiction, missing evidence, and the condition that changes the read. Power, mobile, accessible tables, export, deep link, and Market Brief owner read preserve the same identity and limitations while exposing formulas, levels, lifecycle, comparisons, option assumptions, validation, costs, process, and source audit.

## Planning Initialization

- [x] The acceptance baseline is anchored to 31 business scenarios, 107 functional requirements, 20 non-functional requirements, the complete UX contract, and the design's 65 page-local plus seven shared pure declarations.
- [x] The executable plan uses nine isolated scope directories, a `foundation:true` first scope, explicit foundation dependencies for every overlay/publisher/UI scope, and per-scope evidence files.
- [x] The validation contract uses Research Lab's build-free Node commands and the checkout-local Playwright `1.61.1` `system-chrome` project over a real ephemeral static server.
- [x] Feature 006 is treated as optional strict owner evidence; missing or incompatible `tdc-tool-read/v1` is unavailable and no Feature 006 completion claim is part of Feature 007 acceptance.
- [x] Formula fixtures and provenance-bearing historical fixtures have separate authority, and neither is accepted as live external market evidence.
...
- [Scope 01 foundation evidence](scopes/01-capability-foundation/report.md)
- [Scope 02 technique evidence](scopes/02-technique-engine/report.md)
- [Scope 03 lifecycle evidence](scopes/03-setup-lifecycle/report.md)
- [Scope 04 gate evidence](scopes/04-five-gate-synthesis/report.md)
- [Scope 05 owner evidence](scopes/05-owner-publication/report.md)
- [Scope 06 comparison evidence](scopes/06-comparison-optional-evidence/report.md)
- [Scope 07 validation evidence](scopes/07-validation-risk-process/report.md)
- [Scope 08 experience evidence](scopes/08-experience-publication/report.md)
- [Scope 09 regression evidence](scopes/09-regression-closure/report.md)
