...
identities. They also produce equal rank identity and canonical order.
...
- Semantic anti-inflation governs relevance. Equal semantic identities collapse
  ...
  signal-identity, rank-identity, or ordering derivation.
- Rank behavior remains deterministic on a frozen basis. Adding occurrences
  without adding a semantic identity cannot change rank identity or canonical
  order.
...
stores an exact repeat. It also fails if a repeated semantic identity changes
any relevance or rank projection on the same frozen basis. Storing new
sensitive data or leaving occurrence or ranking influence after owner clear
also fails.
...
identities MUST collapse to one canonical completion contribution. Additional
occurrences of that identity may increase retained audit cardinality, but MUST
NOT change:
...
| AC-4 | FR-B004-005 holds for every projection it names | `tests/portfolio-behavior-occurrence.unit.mjs` case `a repeated same-day occurrence cannot buy relevance it did not earn`, including its baseline-versus-augmented score, identity, and order comparison | Met by `report.md#validate-focused-carrier`; packet status remains `in_progress` pending validation restart. |
