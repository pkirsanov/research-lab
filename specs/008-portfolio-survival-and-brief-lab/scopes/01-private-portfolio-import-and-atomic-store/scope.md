...
- [ ] SCN-008-001 holds exactly as written: when a user imports a valid portfolio without credentials — an import carrying recognized holding fields and no secret or account-identity field — and reviews and confirms the import preview, one new local portfolio revision becomes current, its holdings, quantities, optional cost fields, and derived values remain local-only, and the Portfolio Brief and portfolio analyses reference the new revision.
  ...
  > **What was attempted:** TP-01-03 and TP-01-05 were executed together in a prior session via `npx --no-install playwright test tests/portfolio-survival-foundation.spec.mjs --config=playwright.config.mjs --project=system-chrome --reporter=list`, exit code 0, 3 passed. Every clause was then traced to the specific assertion carrying it.
  > **What was observed:** three of four clauses are confirmed. `one new current revision` is carried by `generation=1`, `revisions=1`, and the post-reload identity re-read. `local-only retention` is carried by the exact two-key `localKeys` set, the empty `sessionKeys`, the negative URL and request-log regexes, the all-same-origin request assertion, and `remoteRequests=0`. The `Portfolio Brief` half of the final clause is carried by the `#brief` route, the visible `Portfolio Brief` heading, and the post-reload `#currentRevision` identity match inside `section.brief`. Under the clause attribution above, every clause Scope 01 owns is therefore carried by a named assertion.
  > **Why this is still uncertain:** two reasons remain, and neither is the analyses conjunct. First, the recorded run is from a prior session, so the Execution Evidence Standard requires TP-01-03 and TP-01-05 to be re-executed and re-read against these clauses before this box is checked. Second, both rows instantiate the claim from an empty workspace, so the `one existing revision` precondition allowed by this scope's UI Scenario Matrix is still not demonstrated; that is a matrix-coverage gap rather than a Gherkin clause, since the scenario's `Given` does not constrain prior state.
  > **What would resolve this:** re-execute the two named rows in the resolving session and re-read each Scope 01 clause against their output, then either extend TP-01-03 to commit a second revision over an existing one or record the empty-workspace-only instantiation as an accepted matrix limitation. Rewriting this item's text to drop `portfolio analyses` is the exact inversion Gate G068 detects and is refused here; the conjunct is delegated with its verifying scope and row named, not deleted. Evidence: [Scenario Behavioral Claim Verification](report.md#scn-008-001-clause-ledger).
- [ ] SCN-008-002 holds exactly as written: a malformed or secret-bearing import cannot partially replace the portfolio — given a current valid portfolio exists, an import carrying malformed rows, credential-shaped fields, or unresolved required identities is rejected with row and field reasons, the prior portfolio remains current and unchanged, and no rejected value enters storage, logs, URLs, telemetry, or committed artifacts.
  ...
  > **Uncertainty Declaration**
  > **What was attempted:** TP-01-04 and TP-01-05 were executed together in the current session via `npx --no-install playwright test tests/portfolio-survival-foundation.spec.mjs --config=playwright.config.mjs --project=system-chrome --reporter=list`, exit code 0, 3 passed. Each named sink and each atomicity sub-claim was traced to the specific assertion carrying it.
  > **What was observed:** the two atomicity sub-claims are confirmed. `no partial replacement in any persistence mode` is carried by TP-01-05's per-mode identity, generation, storage-mode, and durability re-reads across `durable`, `session`, and `memory` (`modes=durable:1:durable,session:1:session,memory:1:memory`, `priorRevisionPreserved=true`). `prior revision identity unchanged after rejection` is carried by TP-01-04's `currentPortfolioId` and `generation` equality against the pre-rejection snapshot plus the `Current portfolio unchanged` state message. Of the five named sinks, storage, logs, and URLs are directly asserted; telemetry is confirmed by construction because the route loads only `rlcontracts.js` and `rlportfolio.js` and has no beacon, XHR, WebSocket, or analytics client to assert against.
  > **Why this is uncertain:** the `committed artifacts` sink has zero coverage. No named row reads the working tree, the git index, or any tracked file, and TP-01-04's probe value is built at run time as `'SCOPE01-E2E-PRIVATE-' + Date.now()`, so the row is structurally incapable of testing that sink. Two narrower gaps also stand: the `row and field reasons` clause is only asserted at the error-code level (`P008-IMPORT-SECRET`) even though `safeErrorCopy()` renders the `row N` and `field X` segments, so a regression dropping them would not fail; and sink absence is proven in durable mode only, because TP-01-04 never calls `blockStorage` while TP-01-05, which does cover `session` and `memory`, performs no sentinel scan.
  > **What would resolve this:** a new or extended verifying row that scans a committed surface for a rejected value, asserts the row and field segments of the rejection reason, and repeats the sink scan under blocked-storage modes. That is a Test Plan addition and is planning-owned. Evidence: [Scenario Behavioral Claim Verification](report.md#scn-008-002-clause-ledger).
