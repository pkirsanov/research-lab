...
- [ ] SCN-008-001 holds exactly as written: when a user imports a valid portfolio without credentials — an import carrying recognized holding fields and no secret or account-identity field — and reviews and confirms the import preview, one new local portfolio revision becomes current, its holdings, quantities, optional cost fields, and derived values remain local-only, and the Portfolio Brief and portfolio analyses reference the new revision.
  ...
  > **What was attempted:** TP-01-03 and TP-01-05 were executed together in the current session via `npx --no-install playwright test tests/portfolio-survival-foundation.spec.mjs --config=playwright.config.mjs --project=system-chrome --reporter=list`, exit code 0, 3 passed. Every clause was then traced to the specific assertion carrying it.
  > **What was observed:** three of four clauses are confirmed. `one new current revision` is carried by `generation=1`, `revisions=1`, and the post-reload identity re-read. `local-only retention` is carried by the exact two-key `localKeys` set, the empty `sessionKeys`, the negative URL and request-log regexes, the all-same-origin request assertion, and `remoteRequests=0`. The `Portfolio Brief` half of the final clause is carried by the `#brief` route, the visible `Portfolio Brief` heading, and the post-reload `#currentRevision` identity match inside `section.brief`.
  > **Why this is uncertain:** the final clause is a conjunction, and its `portfolio analyses reference the new revision` half is asserted by no named row. `portfolio-survival-allocation-lab.html` renders `Risk X-Ray`, `Path Lab`, `Diversification`, `Allocation Comparison`, and `Research Dossier` as `disabled`, and the document has no analysis panel element; those surfaces belong to Scopes 07-15, all `Not Started`. A related precision note: both rows instantiate the claim from an empty workspace, so the `one existing revision` precondition allowed by this scope's UI Scenario Matrix is not demonstrated.
  > **What would resolve this:** either a later scope's rows carry the analyses conjunct once those surfaces exist, or planning re-scopes which surfaces Scope 01 is accountable for. Rewriting this item's text to drop `portfolio analyses` is the exact inversion Gate G068 detects and is refused here. Evidence: [Scenario Behavioral Claim Verification](report.md#scn-008-001-clause-ledger).
