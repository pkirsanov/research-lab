...
1. Inventory the authored carriers for SCN-008-001 through SCN-008-054 against each owning scope plan's exact producer, consumer call chain, observable result, Test Plan title, file, assertion, and adversarial discriminator. Treat every v1 `testCarriers` value as a non-authoritative migration candidate and reject it when it conflicts with that plan-owned causal mapping. Verify that SCN-008-055 retains one separate Scope 29 plan whose authored functional and E2E carriers are not Scope 28 execution prerequisites.
...
| Feature 008 producers, consumers, and test carriers | Every required declaration forms one resolved producer-to-use-to-observable-result-to-assertion graph sourced from the owning scope plan, with no v1-carrier default, zero-match, or disconnected-substring success. Scope 04 resolves `ensureBarCoverage` → `holdingEvidence` → `renderHoldingTruth` → `#truthSummary` / `#truthRows` → TP-04-06; TP-04-05 remains its separate privacy-boundary carrier. |
...
The plan-owned `scopes/_boundary-attribution.md` Scope 28 table is the exact path-and-identity attribution authority. The implementation-owned `scripts/spec008-scope-claims.json` remains the closed v2 claim manifest for the 41 generic rows named below. It must match the authoritative registry rows and cannot define, expand, or repair Scope 28 ownership. It contains exactly 17 `boundary` entries and exactly 24 `consumer` entries. The verifier derives that pair set from the authoritative scope artifacts and the registry before it reads the manifest entries. The manifest cannot define its own completeness. During v1-to-v2 migration, v1 producer, consumer, and test-carrier fields are authoring candidates only. The owning scope plan selects the causal carrier and observable result; a stale v1 test carrier cannot override that mapping or be accepted merely because its title resolves. V1 consumer candidates are migration hints, not authority: when an explicit causal row selects a different current-source consumer, the plan-owned identity wins and the verifier resolves it directly instead of requiring equality with the v1 token.
...
The owning scope plan declares exactly one explicit Consumer Impact Sweep causal row for each real connected graph. A scope with independent producer-to-observable graphs retains one grounded row and one derived `causalBinding` per graph. The generator must not collapse those graphs into one row or impose an item-wide single-row limit. Each row independently names one inline-code function chain in actual causal order, at least one inline-code `#observable`, exactly one Test Plan ID, the phrase `exact title`, and that Test Plan row's exact executable title.
...
The analyzer must resolve one connected producer-to-consumer-to-observable-result-to-assertion graph. The assertion must consume or observe a result that the declared consumer produces. Three independent substring matches are not a binding. A disconnected producer, unused import, unreachable title, assertion outside the named test, or assertion unrelated to the consumer result fails.

For `SCOPE-04-CONSUMER-CLAIM`, the authoritative connected graph is `rldata.js::ensureBarCoverage` → route-local `holdingEvidence` → `renderHoldingTruth` → DOM results `truthSummary` and `truthRows` → exact TP-04-06 title `Regression: SCN-008-035 partial data corrupt schema and localStorage disabled preserve truth`. Its assertions read the summary and rendered row attributes/text after import, cache mutation, reload, corruption, and storage degradation. `barAlignmentStates` remains an attributed Scope 04 source identity with its own unit coverage; it is not the producer used by `renderHoldingTruth`. TP-04-05 proves the distinct SCN-008-005 privacy boundary and is not a causal carrier for the renderer.
...
- TP-28-01 has an authored current reachability carrier. TP-16-14 now has its exact declared title at the named current path. This plan grants neither row execution credit; any rerun and receipt belong to bubbles.test.
...
- TP-28-04 is authored in `tests/portfolio-test-integrity.unit.mjs`, with test-owned in-memory substitution support in `tests/portfolio-defect-injector.cjs`. The exact title is present in the named file. Execution and refusal-coverage verdicts remain test/validation-owned.
...
- The v2 plan retains the six existing TP-28 IDs and commands. TP-28-01, TP-28-02, and TP-28-04 have authored current carriers but remain `planned-not-executed` in plan metadata. Existing receipts are not imported as current plan execution credit.
...
- [ ] TP-28-01 declaration reachability passes with zero unresolved or zero-match titles. → **Resolution condition:** bubbles.test reruns the unchanged TP-28-01 command against the expanded structured plan and records a current receipt. The exact TP-16-14 title is authored at its declared path, but file/title presence does not satisfy this execution row.
