...
These stable IDs are authoritative manifest entries. Each has one owning remediation scope, exact planned test bindings, scenario-derived obligations, and a scope-local evidence target. Authoring and execution state is recorded per owning scope rather than inferred from this index. Scope 27 retains TP-27-01 through TP-27-04 as authored and TP-27-05 as an existing shared carrier, with its checked DoD items linked to the test-owned report. This planning reconciliation mirrors that scope-level `Done` status without creating or restating execution evidence.
