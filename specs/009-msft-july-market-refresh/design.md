...
| Daily bars | retrieval age `<= 5 calendar days` and cutoff age `<= 4 calendar days` | Either valid age exceeds its bound | Invalid envelope/rows/symbol/interval, last-row UTC date differs from cutoff, absolute retrieval more than 5 minutes after evaluation, or cutoff after evaluation UTC date |
...
The four-calendar-day bar window tolerates a normal weekend without claiming exchange-calendar knowledge. The cutoff window is the authoritative bar staleness test; the retrieval bound is derived from it as `cutoff window + 1 day` rather than set independently. Daily bars are retrieved once per completed session, so their retrieval age tracks their cutoff age, and the extra day covers the intra-day offset between the after-hours bar publish and a later evaluation. A retrieval bound tighter than the cutoff window would be unreachable by construction: it would fire every weekend against the latest completed session and report the freshest data that exists as stale, which is the false-staleness claim this policy exists to prevent. The page always renders the actual cutoff, so even an `available` bar resource is never called live or assigned the page date.
