...
13. **Vol targeting holds risk constant, not growth — and that is an assumption, not an identity.** The `targetVol / forecastVol` form scales exposure by `1/σ`. Holding `f·σ` constant holds the volatility-drag term `½·f²·σ²` constant, which is *why* the throttle steadies compounding. But it coincides with growth-optimal sizing only where expected return scales with volatility — i.e. where reward-per-unit-risk is roughly stable across regimes. Growth-optimal sizing under a *known* edge scales by `1/σ²`, not `1/σ`. Because the tool carries no directional input it has no edge estimate and cannot test that condition, so the assumption must be **disclosed in the sizing card** rather than left implicit in the formula. Note that **Cross-Product Applicability** names QF's `f* = μ/σ²` and its `vol_target` mode side by side without observing that the two scaling laws differ; closing that omission is the point of this finding. (Surfaced 2026-07-30 by the cross-cutting volatility-drag review, `notes/volatility-drag-research.md` → RL-3.)
