...
- [ ] `[TP-03-04]` `[BS-013-010]` The credit-quality-pulse, real-assets, and global-rotation ratio-derived sources read through `RLRATIO`, propagate the proxy caveat, and emit `not-comparable` where the comparability predicate fails.
- [ ] `[TP-03-10]` `[BS-013-010]` The declared `fx` slot has no shim host in this feature, so `fx.dollar` composes as `availability: 'unavailable'` with `SOURCE_UNAVAILABLE`, is excluded from `m`, and is named in `excludedFacetIds` — never a neutral vote, never 0, and never a rendered FX value.
...
- [ ] `[TP-03-09]` The facet publication path sustains a repeated high-volume publication run with the Tier 0 facet slot append staying bounded, repeated same-source writes replacing rather than accumulating, and no degraded write throughput across the run.
