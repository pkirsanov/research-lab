...
1. **Author the `brief-recommendation-claim/v1` contract shape** exactly as `design.md` → `## D1` → *Contract*
   ...
   `{ reason, field }` carrying one member of the closed eight-reason mint-refusal set plus the input that caused
...
7. **Implement `RTR-PREDICATE-AMEND` against the hashed terms, never the bytes.** Re-minting an identical claim is a
   ...
   count exactly once.
8. **Implement the closed mint-refusal reason set — eight reasons, each naming the field that caused it.** Each is
   ...
   *which* input was missing instead of showing one opaque bucket. `non-semantic-subject` when `subject` or `family`
   ...
   `thesisFamily`), `no-authored-predicate` (absent predicate). Plus, added 2026-08-20 under `F-015-03-01`,
   `no-authored-flat-band` (`magnitude.flatBand` absent, `null`, non-finite, negative, or zero) — a degenerate band
   coerces to `null`, and `|outcome| <= null` collapses to `<= 0`, which makes **HC-7 vacuous** rather than strict.
   It is the eighth member and it names its offending field exactly as the other members do.
   **`unresolvable-owning-tool` is retired** — an unmatched `deepLink` now yields `citedToolId: null` rather than a
   refusal, so the code has no trigger left and carrying it forward would refuse a mint D1 requires to succeed.
...
- [x] The closed eight-reason mint-refusal set (`non-semantic-subject`, `no-committed-series`, `neutral-direction-no-magnitude`, `no-authored-subject`, `no-authored-horizon`, `no-authored-thesis-family`, `no-authored-predicate`, `no-authored-flat-band`) is implemented, each refusal names the field that caused it, and the retired `unresolvable-owning-tool` code is absent from the source. — **re-baselined from seven on 2026-08-20 (`F-015-03-01`); the tick is not disturbed, see the *Refusal-set re-baseline* note below.**
...
>
> **Refusal-set re-baseline — 2026-08-20, `F-015-03-01`.** The mint-refusal core item above read *seven* reasons
> until today and now reads **eight**. **Old:** `non-semantic-subject`, `no-committed-series`,
> `neutral-direction-no-magnitude`, `no-authored-subject`, `no-authored-horizon`, `no-authored-thesis-family`,
> `no-authored-predicate`. **New:** the same seven **plus** `no-authored-flat-band` (field `magnitude.flatBand`).
> **Why it changed:** routed defect `F-015-03-01` was fixed in `rlclaims.js`, so a degenerate band — absent, `null`,
> `NaN`, `Infinity`, negative, or zero — now refuses at mint instead of coercing to `null`, where
> `|outcome| <= null` collapses to `<= 0` and makes **HC-7 vacuous**. The routing ruling is scope 03's, at
> [`../03-resolved-flat-sentinel-and-resolution-record/report.md`](../03-resolved-flat-sentinel-and-resolution-record/report.md)
> → *RULING — recorded 2026-08-20*, commit `7198f22d8`.
>
> **The tick is not disturbed, and this is deliberate.** The item's substance — *a **closed** refusal set in which
> **every** member names its offending field* — is exactly what was verified and it still holds: the set is still
> closed (`REASONS_OUTSIDE_DECLARED_SET=[]`), and the new member names `magnitude.flatBand` like every other. Only
> the **cardinality** moved, and cardinality was never the property under test. Unticking here would assert that a
> verified property had failed, which is not what happened. The tally line above is therefore also unchanged at
> **17 of 17**: no item was added, removed, ticked, or unticked by this re-baseline. Full old → new record and the
> measured `MINT_REFUSALS` output: [report.md → Refusal set re-baselined to eight](report.md#refusal-set-re-baselined-to-eight).
...
| **Eight**-reason refusal set, each naming its field; `unresolvable-owning-tool` **absent from the source** — re-baselined from seven on 2026-08-20 (`F-015-03-01`), tick undisturbed | [`#t-01-r1`](report.md#t-01-r1) (the set, per-trigger isolation — the assertion deep-equals against `MINT_REFUSALS`, so it tracked the eighth member rather than being re-pinned; the run recorded there observed the set at seven), [`#sweep-b--unresolvable-owning-tool`](report.md#sweep-b--unresolvable-owning-tool) (zero occurrences in `rlclaims.js`, exit `1`), [`#refusal-set-re-baselined-to-eight`](report.md#refusal-set-re-baselined-to-eight) (the measured eight-member set) |
...
| Core — **eight**-reason refusal set, each naming its field, **and** `unresolvable-owning-tool` absent from the source | **Re-baselined from seven on 2026-08-20 under `F-015-03-01`; the row's standing is otherwise unchanged.** The refusal set and the per-trigger isolation are proven by [`#t-01-r1`](report.md#t-01-r1) — whose assertion deep-equals the observed reasons against `MINT_REFUSALS` rather than a literal, so the eighth member `no-authored-flat-band` (field `magnitude.flatBand`) is carried without re-pinning — and the retirement is proven **behaviourally** by [`#t-01-f2`](report.md#t-01-f2), where the input the retired code would refuse mints instead. *Absent from the source* is textual and unswept. | `bubbles.implement` — record the source sweep in `report.md` |
