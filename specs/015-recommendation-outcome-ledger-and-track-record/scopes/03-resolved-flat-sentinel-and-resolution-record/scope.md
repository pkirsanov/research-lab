...
- [x] The denominator contract is declared: `resolvedDirectional = wins + losses` is the fed array's length and therefore the published denominator, and the rate it produces is labelled *directional hit rate*. → evidence recorded in [`report.md#core-denominator-contract`](report.md#core-denominator-contract)
...
- [x] T-03-F1 passes: `resolutionHash` is content-only and the content-addressed write is a byte-identical no-op on repeat → evidence recorded in `report.md#t-03-f1`.
- [x] T-03-F2 passes: the partition identity holds and fails when a class is dropped → evidence recorded in `report.md#t-03-f2`.
