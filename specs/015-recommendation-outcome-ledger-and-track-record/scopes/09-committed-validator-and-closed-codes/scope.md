...
into the build-free baseline, exactly as `scripts/selftest.mjs` imports `validateBriefPayload` at `#L18`
...
11. **Carry `RTR-SESSION-PREDICATE` with both its committed companions.** `design.md#L1037` and `#L1836` declare it
    ...
    `data/calendars/xnys/calendar.json` (on 2026-08-18 that decomposition read `regular` 249 + `early-close` 2, a
    dated observation of the committed calendar rather than a constant), never a literal typed into a fixture.
...
14. **Wire the validator into the baseline as a second entry point, not a second implementation.** `selftest.mjs`
    ...
    the baseline then reports `baseline + N passed, 0 failed` — where `baseline` is the total captured immediately
    before this scope's first change and recorded in `report.md` — with **no pre-existing assertion count
    decreasing**, stated as arithmetic because *"selftest still passes"* is satisfiable by deleting an assertion.
...
| The baseline arithmetic against the total **captured at scope start** and recorded in `report.md` | AC-018, and every `T-NN-S1` row across scopes 01 – 10 | The baseline may grow additively to `baseline + N`; it may not shrink, and no pre-existing group's count may fall. The baseline is **captured, never pinned**: planning recorded `952 passed, 0 failed` on 2026-07-29 and re-measuring on 2026-08-18 returned `2487 passed, 0 failed`, exit 0, so a literal written into this row would fail for a reason having nothing to do with this scope. A new import that throws at load takes the baseline to **zero** rather than to the captured figure — a failure a broad rerun reports late and confusingly. |
...
that one file returns the baseline to exactly the total captured at scope start while the new validator stays fully usable
...
| T-09-S1 | Project check | project check | — | `scripts/selftest.mjs` (unmodified) | The repo self-test is green after the validator, the fixtures and the test cases land, at `baseline + N passed, 0 failed`, where `baseline` is the total captured immediately before this scope's first change and recorded in `report.md`, with no pre-existing assertion count decreasing. | `node scripts/selftest.mjs` | No | `report.md#t-09-s1` |
...
- [ ] T-09-S1 passes: `node scripts/selftest.mjs` reports `baseline + N passed, 0 failed` against the scope-start baseline captured in `report.md`, with no pre-existing assertion count decreasing → evidence recorded in `report.md#t-09-s1`.
