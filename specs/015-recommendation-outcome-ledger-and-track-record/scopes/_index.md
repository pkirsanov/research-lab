...
| **F-015-D4-02** | Reducer idempotence is not self-enforcing; FR-006 rests entirely on the due-set gate. | **Resolved. Planning decision text CORRECTED 2026-08-23 under Ruling R-04-10 — see the struck clause below.** | The two-case pair is a **named DoD item pair** in scope 04, not an implied one: `T-04-I2` asserts pass 2 emits zero closures with a byte-identical `indexFingerprint`, and `T-04-I3` asserts — as an **acceptance**, not a refusal — that the reducer *accepts* a second closure for an already-closed entry when the gate is bypassed, that a duplicate event **is appended**, and that `indexFingerprint` stays **byte-identical** across that append. **Corrected:** this row previously read *"and that the fingerprint changes"*. That was **wrong**. `rlcontracts.js:1318` fingerprints `{ contractVersion, entries }` only, and a repeat closure of the same type moves neither `state` nor `lastEventType`, so the appended duplicate is invisible to it — measured at `tests/recommendation-track-record.integration.mjs:837-841`. The correction **strengthens** the finding rather than retiring it: because no downstream detector sees the duplicate, the due-set gate is the **sole** enforcement point for FR-006, and `indexFingerprint` is an index-state oracle, not an event-append oracle. Case 2 is what pins the invariant to its actual location; case 1 alone would not distinguish "the gate held" from "the reducer protected us", and case 1 could not lean on the fingerprint either — its append oracle is `:767`. |
