...
|---|---|
| 01 | Contract `brief-recommendation-claim/v1`; `claimHash` term list and the content-only hashing rule; content-addressed store `briefs/objects/claims/<hex>.json`; the closed mint-refusal reason set; the unhashed `lifecycleTerms` provenance block; `RTR-PREDICATE-AMEND` |
...
|---|---|
| 01 | `node --test tests/recommendation-track-record.unit.mjs` and `node --test tests/recommendation-track-record.functional.mjs` prove content-only hashing, that any hashed-field change yields a different `claimHash`, and that `RTR-PREDICATE-AMEND` fires on a byte-changing write at an existing path; `node scripts/selftest.mjs` still reports `952 + N passed, 0 failed`. |
...
|---|---|---|---|
| **F-015-D4-01** | Key-space bridge — the reducer keys on `originRecommendationKey` over `{originToolId, thesisFamily, subjects, actionFamily, horizon}`; D1 binds to the publisher key over `{subject, family}`. | **Resolved in part; remainder escalated as P-015-03.** | **Placement resolved:** `originToolId` and `thesisFamily` enter the claim as **unhashed provenance** in a `lifecycleTerms` block, alongside `proposalRunId` / `proposalEventId` / `proposedAt`, which D1 already excludes from `claimHash`. The `claimHash` term list is therefore unchanged and **D1 needs no revisit**. The bridge stays *derived, never authored*: scope 04 calls `deriveRecommendationKeys` (`rlcontracts.js#L1034`) on terms assembled from `lifecycleTerms` plus the claim's hashed `subject` / `actionFamily` / `horizon`, per the foundation's own rule at `rlcontracts.js#L1031` (*"Authors never own identity"*). **`originToolId` resolved:** derived at mint from the authored action's `deepLink` mapped through `tools.json` `file` → `id`; an absent or unmatched `deepLink` refuses the mint rather than defaulting. Verified live `deepLink` values on `market-brief.payload.json`: `etf-momentum-lab.html`, `sector-research-lab.html`, `msft-july-print-model.html`, `gamma-trading-lab.html`. **`thesisFamily` escalated** — see P-015-03; scope 04 carries `routed:P-015-03`. |
...
| **F-015-D5-02** | The legacy count is a moving target and must not be hardcoded. | **Resolved.** | The count is **derived** (`rows lacking a claimRef`), **asserted once** at contract activation into `briefs/history/record-start.json`, and **guarded** by `RTR-LEGACY-GROWTH`. No literal ships in any 015-authored source; scope 09's source scan makes that checkable. Independently recounted this planning run: `briefs/history/recommendations/2026-07.jsonl` holds **165 rows** — recorded here as a dated observation, **not** as a constant, and deliberately not carried into any scope, test, or DoD as a literal. Owned by scope 05; the activation-sequencing rule is a cross-cutting rule below. |
| **F-015-D6-01** | `metrics` is not rendered by the Center; registration gates whether the read is looked up at all. | **Resolved, with the ordering inverted and justified.** | The finding's literal instruction — *register before publish* — is **not** adopted, and the reason is mechanical. `putToolRead` writes `d.toolReads[id]` and returns the stored object regardless of registration (`rldata.js#L433`–`#L453`), so scope 06 can prove FR-018 contract compliance standalone, including the non-`null` return check. What registration actually gates is `RLBRIEF.renderToolReads`' **by-id lookup** (`rlbrief.js#L689`) — i.e. **UI-35 only**. Registration is therefore deferred to scope 10 because it mutates five counted registries with hard count assertions verified this run at `scripts/validate-tool-experience.mjs#L493`–`#L496` (`ordinaryTools === 22`, `centerGoals === 4`, `totalGoals === 48`, `definitionCount === 48`) plus a per-ordinary-tool `journeyDefinitionIds.length >= 2` rule at `#L501`; registering early would leave a live nav entry pointing at an unfinished surface. **Consequence recorded in the UI map: UI-35 is owned by scope 10, not scope 06.** |
...
**No D11 finding is left unaddressed.** The single escalation is the `thesisFamily` half of F-015-D4-01, carried
below as **P-015-03**.
...
| **P-015-04** | **`design.md` → `D6`'s registration table is incomplete.** It lists `tools.json`, `index.html` `TOOLS`, `rlnav.js` `TOOLS` and `notes/` as the registration surface. Registering a 24th tool additionally requires `journeys.json` `definitions` (currently 48) and an `experience.journeyDefinitionIds` block with **at least two** goals, and it trips four hard count assertions in `scripts/validate-tool-experience.mjs`. The coupling is not named in `design.md`, so a scope written from that table alone would fail the repo check. | `bubbles.design` | Advisory | Scope 10 (`counted-registry`) | `scripts/validate-tool-experience.mjs#L493`–`#L496` and `#L501` verified this run; `journeys.json` `definitions` length 48; `tools.json` 23 tools / 22 non-Center |
...
4. **No literal count anywhere.** The unresolvable-legacy figure, the family count, and the trial count are derived
   at render time. The observation recorded above (165 rows) exists to size the problem and is **not** carried into
   any scope, fixture, test name, or DoD item as a constant.
...
**Baseline measured this planning run:** `node scripts/selftest.mjs` → `952 passed, 0 failed`, exit 0. Per AC-018
every scope asserts `952 + N passed, 0 failed` with **no pre-existing assertion count decreasing** — stated as
arithmetic because *"selftest still passes"* is satisfiable by deleting an assertion.

**`node --test` is a real surface**, not an assumption: 78 committed files under `tests/` import `node:test`, and
`node --test tests/market-action.unit.mjs` was run this planning run and reported `# pass 13 / # fail 0`.
