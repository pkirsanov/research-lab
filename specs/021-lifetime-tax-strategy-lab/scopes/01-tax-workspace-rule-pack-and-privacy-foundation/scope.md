...
- [ ] The zero-network canary passes: the route issues no request, and a
      ...
      committed artifact.
  - **Uncertainty Declaration (F-REG-02):** unticked. The second arm is proven; the first arm is not, and cannot be — it is false by measurement. The route issues 24 requests at first paint (the document, 14 declared modules, the configuration document, the rule packs it names, and the browser's own favicon), through one `window.fetch` primitive at `lifetime-tax-strategy-lab.html:5649`. The cited evidence is `expect(unexpected).toEqual([])`, which proves no **undeclared** request, a different proposition from "issues no request". `NFR-021-009` was corrected to the measured truth, so this row's text now contradicts its own requirement. Re-ticking requires the row to be restated against the corrected requirement, which is `bubbles.plan`-owned; it is not something delivery may rewrite to match what shipped.
