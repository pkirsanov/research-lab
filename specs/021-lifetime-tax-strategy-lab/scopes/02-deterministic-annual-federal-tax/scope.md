...
- [ ] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-021-004, SCN-021-005 and SCN-021-006 pass under the exact persistent titles this scope's Test Plan names, and each of those titles is present in the spec file rather than merely selected by `--grep`. Adversarial case: renaming or deleting one of those persistent titles must fail this row, so an empty grep selection can never be read as a pass.
