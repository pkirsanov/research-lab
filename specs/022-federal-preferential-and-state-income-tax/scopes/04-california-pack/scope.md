...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-022-010, SCN-022-011 and SCN-022-012 pass under the exact persistent titles this scope's Test Plan names, and each of those titles is present in the spec file rather than merely selected by `--grep`. Adversarial case: renaming or deleting one of those persistent titles must fail this row, so an empty grep selection can never be read as a pass.
  - **Phase:** implement · **Command:** three `grep -c -F` presence checks plus each Test Plan `--list` and `--reporter=list` run at `--project=chromium` · **Evidence:** `report.md#dod--scenario-specific-e2e-regression-for-scn-022-010--011-and--012`
  - **Evidence:** each title present exactly once as a `test()` declaration (`1`, exit 0, three times); each Test Plan command listing `Total: 1 test in 1 file` and running `1 passed` at exit 0; renaming one title probed to `red-exit 1 / Error: No tests found` against `green-exit 0 / 1 passed`.
- [x] Broader E2E regression suite passes across the whole lifetime-tax browser family, not this scope's own spec file alone. Adversarial case: a change made inside this scope that reddens a sibling scope's persistent title must fail this row even while this scope's own rows stay green.
  ...
  - **Evidence:** `88 passed` at exit 0 grep-selected, `94 passed` at exit 0 path-selected across all twenty family spec files. One earlier path-selected run failed at `93 passed, 1 failed`; the cause is recorded as a measured pre-existing ephemeral-port collision in `tests/lifetime-tax-use.spec.mjs`, an excluded file owned by `SCN-023-010`, not a regression from this scope. The sibling-blast-radius clause is proven by one mutation reading `94 → 91 passed` on the family and `3 → 3 passed` on this scope's own selection.
- [x] Change Boundary is respected and zero excluded file families were changed, proven by a path-scoped `git status --porcelain` over the excluded surfaces plus an mtime comparison for any untracked excluded directory. Adversarial case: touching one excluded path must produce a row and fail this item; `git diff --quiet` alone is not accepted, because it reports an untracked path as unchanged.
  - **Phase:** implement · **Command:** `git status --porcelain` over 28 positive plus one negative pathspec, `git ls-files --others --exclude-standard` over the same set · **Evidence:** `report.md#dod--change-boundary-respected-zero-excluded-file-families-changed`
  - **Evidence:** zero porcelain rows across 7,897 tracked excluded files; zero untracked files in the excluded set, so the mtime limb has a measured empty domain; the negative pathspec proven non-vacuous at `20 → 19`; `git diff --quiet` shown returning exit 0 for an untracked path that porcelain reports as `?? err.txt`; touching one excluded path probed to `red-exit 1` against `green-exit 0`.
