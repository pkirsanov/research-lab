...
This scope fixes the claim-age module name, the mortality pack path and the
comparison stage identifiers. Any rename, move or removal of one of those
...
| --- | --- | --- |
| The page's module `src` list and the mortality pack it reads as an API client | A moved module or pack path turns a declared read into an unresolved request | The declared-read canary fails on any declared read that does not resolve |
| The route's claim-age panel and its anchor ids | A renamed stage leaves the panel unavailable instead of resolved | Every claim age resolves to a rendered comparison row in the browser row |
...
| Sibling scopes that compose the comparison into the route census | A removed identifier drops a leg from the census silently | The census refuses on an unknown leg rather than skipping it |
...
- [x] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in SCN-024-007, SCN-024-008 and SCN-024-009 pass under the exact persistent titles this scope's Test Plan names, and each of those titles is present in the spec file rather than merely selected by `--grep`. Adversarial case: renaming or deleting one of those persistent titles must fail this row, so an empty grep selection can never be read as a pass.
  - **Phase:** implement · **Command:** `npx --no-install playwright test --config=playwright.config.mjs --project=chromium tests/lifetime-tax-claim-age.spec.mjs --reporter=list` → `5 passed (2.9s)`, exit 0 · **Evidence:** `report.md#dod--scenario-specific-e2e-regression-scn-024-007---008---009`
  - Title presence was audited quote-agnostically for all 17 title-bearing rows in scopes 03-05 (`ok=17 bad=0`), and each row's own `--grep` was run through `playwright --list` (`selectedExactlyOne=17 emptyOrError=0`). The adversarial case is proven by probe `S03-TP-03-21-title-rename`: renaming the title yields `red-summary: Error: No tests found`, exit 1, revert hash-verified.
- [x] Broader E2E regression suite passes across the whole lifetime-tax browser family, not this scope's own spec file alone. Adversarial case: a change made inside this scope that reddens a sibling scope's persistent title must fail this row even while this scope's own rows stay green.
  ...
  - Both halves of the adversarial case are measured. Probe `S03-broader-suite-catches-sibling-redness` mutates this scope's *Allowed modified* route file so only the sibling medicare spec reddens: exit 1, `18 passed` versus `19 passed`. Probe `S03-narrow-row-CANNOT-catch-sibling-redness` runs the identical mutation against the narrow command and returns harness exit 7, `5 passed` both sides — the narrow row is blind to it, so the broader row is not redundant.
- [x] Change Boundary is respected and zero excluded file families were changed, proven by a path-scoped `git status --porcelain` over the excluded surfaces plus an mtime comparison for any untracked excluded directory. Adversarial case: touching one excluded path must produce a row and fail this item; `git diff --quiet` alone is not accepted, because it reports an untracked path as unchanged.
  - **Phase:** implement · **Command:** `git status --porcelain -- <34 excluded pathspecs + 19 sibling spec files + 13 market-brief files>` → `EXCLUDED_ROWS=0`, exit 0 · **Evidence:** `report.md#dod--change-boundary-respected-zero-excluded-file-families-changed`
  - `untracked_excluded_dirs=0`, so mtime is nowhere the sole evidence. One excluded file carries an mtime newer than `HEAD` — the boundary probe's own target — and is byte-identical to the committed blob (`worktree_blob == committed_blob`), which is recorded rather than suppressed. The adversarial case is proven by probe `S03-change-boundary-detects-excluded-touch`: `red-summary: M tests/lifetime-tax.support.mjs`, exit 1. `git diff --quiet` is shown insufficient against a real untracked path: exit 0 while `git status` reports `?? err.txt`.
- [x] The Consumer Impact Sweep is complete for every renamed, moved or removed route, path, contract, identifier and UI target in this scope, and zero stale first-party references remain. Adversarial case: one stale reference left in navigation, a breadcrumb, a redirect, a deep link, an API client read or a doc must fail this row, and the proof must be a repository-wide stale-reference scan rather than a spot check.
  - **Phase:** implement · **Command:** three-rule repository-wide stale-reference sweep → `R1 declared-module-reads=14 unresolved=0`, `R2 declared-pack-reads=7 unresolved=0`, `R3 claim-age-ids-targeted=15 unresolved=0`, exit 0 · **Evidence:** `report.md#dod--consumer-impact-sweep-zero-stale-first-party-references`
  - The rename/move/removal set for this scope is EMPTY (`rename_or_delete_commits=0`; both surfaces were added in `b9d92a3f1`), so the row is vacuously satisfiable as written and is instead earned against rules with real populations. Two candidate rules were rejected as unfailable: the route emits `href_hash_anchors=0`, and it contains zero literal `"tax-rules/…"` strings. R3's `byId` fallback was measured (`inMarkup=15 onlyEmittedFallback=0`) and removed as dead leniency. Each surviving rule is probed and discriminates: `S03-sweep-R1-detects-stale-module-read`, `S03-sweep-R2-detects-moved-pack-path`, `S03-sweep-R3-detects-renamed-ui-target`, all exit 0 with hash-verified reverts.
