...
- [ ] Every Test Plan row has intended RED and same-command GREEN evidence recorded,
  ...
  - **Not satisfied.** Every row except TP-04-29 is now authored and green on its own command, and the per-row result table is recorded in `report.md#test-evidence`. Four rows carry an observed intended-RED from this session on the identical command that later shows green: TP-04-13, TP-04-18, TP-04-19 and browser row TP-04-27, plus TP-04-22's second arm. The remaining rows do not. TP-04-01 through TP-04-11, TP-04-15, TP-04-17, TP-04-20 and browser rows TP-04-24 through TP-04-26 and TP-04-28 were delivered by an earlier dispatch and this session did not re-derive their individual intended-RED; TP-04-14, TP-04-16, TP-04-21 and TP-04-23's second arm were authored this session and passed on first execution, so no RED was observed for them either. TP-04-29 was not run as its own `--grep "SCN-02"` command — the cumulative `tests/lifetime-tax-*.spec.mjs` run over the same route is green at 56 passed, which exercises the same rows but is not the command the row names. The row therefore stays `[ ]`. **Selector correction after this note was written (planning note, no evidence claimed):** TP-04-29's command is now `--grep "SCN-02[1-4]"`, pinned to the four owning spec numbers so a concurrent session's `SCN-025-*` and `SCN-026-*` can no longer make a failure unattributable. The outstanding run is unchanged in kind and is now owed against the corrected command.
