...
| Field | Value |
| --- | --- |
| Status | [x] Complete — all 26 Definition of Done items checked with inline evidence. The module, the policy, the measurement, the fail-closed gate, the payload version stamp and `selectDefaultVisible`'s production consumer all landed. Finding R-5 corrected the Change Boundary to name `scripts/brief-narrative-parallel.mjs` as the file that carries them, and the three items that were blocked on it or on the Scope 4 browser suite closed on 2026-08-19 once the suite existed and the selftest reached `3019 passed, 0 failed` |
| Priority | P1 |
| Depends On | none |
| Tag | foundation:true |
| Increment | A |
...
| Field | Value |
| --- | --- |
| Status | [ ] Implemented, DoD 25 of 27 — the corrected R-6 leg set, the dark state, the Tier-A measurement and the fail-closed validator checks landed and are evidenced. Two of the four originally-open items closed on 2026-08-19 when the selftest reached `3019 passed, 0 failed`. The two that remain are not test failures: the derived-artifact boundary overrun (R-7, open with the plan owner) and the three-line cross-spec selftest repair (R-9, open with the spec 024 owner). Each names its own reason below |
| Priority | P1 |
...
| Field | Value |
| --- | --- |
| Status | [ ] Implemented, DoD 20 of 23 — the memory row v2, the closed change vocabulary, delta-only publishing and the balancing roll-up all landed in commit `3855ee75d` and every Test Plan row 3.1 through 3.19 executes and passes. Three items are open and none is a test failure: the commit carried a derived artifact plus 397 lines of foreign Feature 022 selftest content (R-10), it modified 2 pre-existing lines against a required zero, and `notes/market-brief.md` §5 was never updated with the vocabulary (R-13) |
| Priority | P1 |
...
- [ ] No file outside the Allowed file families table changed. → Verify with `git status --short` and record the full listing. **NOT SATISFIED and deliberately left unchecked, on two independent counts, both named rather than absorbed.** Commit `3855ee75d` touched eight non-spec files. Six are admitted by the table for Scope 3. Two are not. **(a) `brief-history.recent.jsonl`** is a DERIVED artifact regenerated by `scripts/shard-brief-history.mjs`; the table admits the generator but not its output, which is precisely the open R-7 defect Scope 2 recorded against `market-brief.config.page.json`. **(b) The commit also carried 397 added lines of FOREIGN Feature 022 selftest content** — the groups `Feature 022 Scope 01: preferential breakpoints beyond \`single\`` and `Feature 022 Scope 01: no bracket edge is shadowed in ANY rltax module` — against 453 lines of genuine Feature 026 Scope 3 content, out of 861 total insertions. The Change Boundary admits `scripts/selftest.mjs` for "appended marker-bounded **Feature 026** groups only", so foreign groups breach it even though the file itself is admitted. This is a scope-isolation breach, almost certainly a concurrent session's work swept in by a broad `git add`, and it is routed as new finding **R-10** rather than repaired here: rewriting a pushed commit to excise another session's assertions would destroy work this scope does not own. **Claim Source:** executed. **Evidence:** report.md E10-4 and E10-5.
...
- [ ] The appended selftest group removed zero pre-existing lines. → Verify with `git diff --stat scripts/selftest.mjs` and a deletion count of 0. **NOT SATISFIED and deliberately left unchecked. The observed stat is `861 insertions(+), 2 deletions(-)`, and the two deletions are named below.** Both are the same physical line and its comment, at the head of the Feature 026 **Scope 1** budget group: the `otherLines26` error filter. The change is additive in effect — it extends `(line) => line.indexOf('outputBudget: ') !== 0 && line.indexOf('crossAsset: ') !== 0` with a third clause `&& line.indexOf('delta: ') !== 0`, so the budget assertions keep excluding only the prefixes owned by other Feature 026 gates. Scope 2 modified this identical line for the identical reason and recorded it, so the precedent is this feature's own. Neither deleted line belongs to a foreign spec, which distinguishes this from the Scope 2 finding R-9. It is nonetheless a deletion count of 2 against a required 0, so the box stays unchecked. **Claim Source:** executed. **Evidence:** report.md E10-5.
...
| Field | Value |
| --- | --- |
| Status | [ ] Implemented, DoD 22 of 25 — the disclosure-first default view, the dark banner, the cross-asset strip, the changed list, the roll-up, the track-record line and the 14-test browser suite all landed in commit `6b00105c7`, which is one of only two scopes that met the zero-deletion append rule cleanly and the only one that touched no file outside its Allowed families. Three items are open: the unrecorded value-to-explanation pairing count, the 9 new `innerHTML` assignments against a literal no-new-sink clause (R-12), and the missing runbook disclosure subsection (R-13) |
| Priority | P1 |
...
| Field | Value |
| --- | --- |
| Status | [ ] Implemented but materially incomplete, DoD 8 of 19 — the weakest scope in the feature and the one a validator should look at first. What landed and is evidenced: `resolvedThisRun`, `notEvaluableShare`, the two-rate rendering, the attention builder wired onto both publication paths with its required `--as-of` argument, and the producer-existence assertion that is the BUG-009 lesson made mechanical. What did not: **seven of the fifteen declared Test Plan rows were never authored** — 5.1, 5.8, 5.9, 5.12, 5.13, 5.14 and 5.15, including both publication-path canaries (R-14); **FR-026-031 is undelivered**, the composer records no claim-plus-resolving-observation and the memory row's claims block carries hard `null` placeholders; and **`notes/market-brief.md` §10a now contradicts the shipped code**, still calling the attention builder a manual CLI (R-13) |
| Priority | P2 |
...
- [ ] Every Test Plan row above ran, and each recorded exit code is a real observed exit code. → Verify by recording each transcript in report.md. **NOT SATISFIED and deliberately left unchecked. Seven of the fifteen declared rows have no assertion anywhere in the suite.** The `market brief — closed loop on the path` group at `scripts/selftest.mjs` lines 23101–23184 contains exactly eight assertions, covering rows **5.2** (three assertions, one of which subsumes row 5.10 by asserting every closed-loop producer in both paths, and one of which is row 5.11's adversarial), **5.3**, **5.4**, **5.5**, **5.6** and **5.7**. **Absent with no assertion under any marker: 5.1, 5.8, 5.9, 5.12, 5.13, 5.14 and 5.15.** Measured directly: `grep -c "TP-026-5.N "` returns 0 for each of those seven, and `SCN-026-CANARY-05` and `SCN-026-CANARY-05B` return no match anywhere in the file, so rows 5.14 and 5.15 were never authored. This is a genuine test-coverage gap in the scope as delivered, not a recording gap, and it is routed as **R-14**. **Claim Source:** executed. **Evidence:** report.md E10-7.
- [ ] No file outside the Allowed file families table changed. → Verify with `git status --short` and record the full listing. **NOT SATISFIED and deliberately left unchecked, on two counts.** Commit `ec7d24b31` touched six non-spec files. Four are admitted for Scope 5: `.github/workflows/tier-a.yml` and `scripts/brief-refresh-and-push.sh` ("Publication path"), `scripts/build-scorecard.mjs` ("Scorecard builders") and `scripts/selftest.mjs` ("Shared selftest"). Two are not. **(a) `market-brief.scorecard.json`** is a DERIVED artifact produced by `scripts/build-scorecard.mjs`; the table admits the generator but not its output, the third instance of the open R-7 derived-artifact defect. **(b) `tests/market-brief-cockpit.spec.mjs`** is admitted by the table for **scope 4 only**, and this is scope 5. The edit itself is defensible — the suite needed to cover the track-record line this scope renders — but the table assigns the file to one scope, so the boundary as written is breached. Both are routed with R-7 rather than repaired here. **Claim Source:** executed. **Evidence:** report.md E10-4.
