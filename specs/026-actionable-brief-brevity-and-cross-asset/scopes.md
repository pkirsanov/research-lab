...
- [ ] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in this scope are present and pass. → Verify with Test Plan rows 2.20 and 2.21 and `npx --no-install playwright test tests/market-brief-cockpit.spec.mjs --config=playwright.config.mjs --reporter=list` exiting 0. **NOT SATISFIED as written and deliberately left unchecked, because "EVERY" is not met and naming the uncovered part is worth more than a tick.** The covered part is real and was observed in this session: `28 passed (17.7s)`, `PLAYWRIGHT_EXIT=0`, including the two dark-state tests row 2.20 names and the default-view test row 2.21 names. The uncovered part is the resolution arithmetic itself — the five-session `changePct`, the `sessions` actually-spanned count, the `partial` state, the carried credit leg's `confirmation.state` and the reachability refusal. None of those has a browser surface: the committed payload carries no `market-brief-payload/v2` stamp, so no live artifact renders a v2 cross-asset block for a page to assert against. Their persistent scenario-specific regression coverage is the marker-bounded selftest group `rlcockpit.js — cross-asset legs` together with `Regression: SCN-026-CANARY-02`, which is unit-category coverage, not E2E. **Phase:** implement. **Claim Source:** executed. **Evidence:** report.md E13-1 and E13-2.
- [ ] Broader E2E regression suite passes. → Verify with `npx --no-install playwright test --config=playwright.config.mjs --reporter=line` exiting 0 over the repository-wide inventory. **NOT SATISFIED and deliberately left unchecked, on the same evidence Scope 1 records.** The repository-wide run available to this feature neither completed nor was green: `Total: 1242 tests in 68 files`, then `333 passed`, `12 failed`, `897 did not run`, exit 130, with all twelve failures inside `tests/chaos-company-intel-probe.spec.mjs`, a file driving a page a concurrent session has modified in the working tree and which this feature neither owns nor touched. This scope's own suite is green at 28 of 28; that is not a repository-wide pass and is not offered as one. **Phase:** implement. **Claim Source:** executed. **Evidence:** report.md E12-3 for the repository-wide attempt and E13-2 for this session's feature-suite run.
...
- [ ] Scenario-specific E2E regression tests for EVERY new/changed/fixed behavior in this scope are present and pass. → Verify with Test Plan rows 3.20 and 3.21 and `npx --no-install playwright test tests/market-brief-cockpit.spec.mjs --config=playwright.config.mjs --reporter=list` exiting 0. **NOT SATISFIED as written and deliberately left unchecked, because "EVERY" is not met.** The covered part was observed in this session: `28 passed (17.7s)`, `PLAYWRIGHT_EXIT=0`, including the two default-view tests rows 3.20 and 3.21 name, which is where the changed list and the roll-up line reach a reader. The uncovered part is the change vocabulary itself — the five kinds, the precedence order, the distinct-market-bar dedupe, the persistence gate and the `brief-history-recent-row/v2` projection. None of those has a browser surface, because the page renders the composed result and not the two memory rows the detector compared. Their persistent scenario-specific regression coverage is the marker-bounded selftest group `rlcockpit.js — change vocabulary` together with `Regression: SCN-026-CANARY-03`, which is unit-category coverage, not E2E. **Phase:** implement. **Claim Source:** executed. **Evidence:** report.md E13-1 and E13-2.
- [ ] Broader E2E regression suite passes. → Verify with `npx --no-install playwright test --config=playwright.config.mjs --reporter=line` exiting 0 over the repository-wide inventory. **NOT SATISFIED and deliberately left unchecked, on the same evidence Scopes 1 and 2 record.** The repository-wide run neither completed nor was green: `333 passed`, `12 failed`, `897 did not run`, exit 130, with all twelve failures inside `tests/chaos-company-intel-probe.spec.mjs`, which drives a page a concurrent session has modified in the working tree and which this feature neither owns nor touched. A partial, interrupted run is not a verdict on the 1242-test inventory and is not offered as one. **Phase:** implement. **Claim Source:** executed. **Evidence:** report.md E12-3 and E13-2.
...
- [ ] Broader E2E regression suite passes. → Verify with `npx --no-install playwright test --config=playwright.config.mjs --reporter=line` exiting 0 over the repository-wide inventory. **NOT SATISFIED and deliberately left unchecked, on the same evidence every other scope records.** The repository-wide run neither completed nor was green: `333 passed`, `12 failed`, `897 did not run`, exit 130, with all twelve failures inside `tests/chaos-company-intel-probe.spec.mjs`, which drives a page a concurrent session has modified in the working tree and which this feature neither owns nor touched. This scope's own suite being green at 28 of 28 is a different claim and is recorded as that. **Phase:** implement. **Claim Source:** executed. **Evidence:** report.md E12-3 and E13-2.
