...
## Scope 2 Sub-pass 3 Of 3 (FINAL) — Local Hypothetical And Reset Engine With Sink-Isolation Integration Proof — 2026-09-06

**Phase:** implement/test
**Claim Source:** executed
**Scope status:** This is the final of 3 planned Scope 2 sub-passes. Every Scope 2 Test Plan row (TP-02-01 through TP-02-12) now has real, executed evidence, and Scope 2's `Status:` field in `scopes.md` is updated from **Not Started** to **In Progress** (not **Done** — see Disposition below for the exact, honest reason it does not reach Done in this pass).

### Scope Of This Sub-pass

Per the investigation that split Scope 2 into 3 sub-passes, sub-pass 3 covers: the local, non-persistable hypothetical/reset engine (`shock-transmission/hypothetical/v1`, design.md §13), the `projectViewState(baselineViewState, definition, projectionInput)` contract that produces both a labelled same-topic comparison and an exact reset (design.md §6.5.4, §14.7), and a real sink-isolation integration test proving no dossier, history, pointer, payload, tool-read, storage, or network path is ever touched by that engine (SCN-031-024, TP-02-11, TP-02-12).

### Design Judgment Recorded

Design §6.5.4's sequence diagram shows `RLSHOCK.projectViewState(snapshot, shockDefinition, projectionInput)` invoked by the (out-of-scope, Scope 4/5-owned) `RLAGENDA` resolver for both the baseline view (from a real composed `snapshot`) and a hypothetical comparison (from a raw, uncomposed `adapterOutput`). Full snapshot-to-view assembly (mapping every snapshot field into `graph`, `policyRows`, `scenarioRows`, `calibrationRows`, `findings`, `ownerLinks`, etc.) is not specified field-by-field anywhere in design.md and is not exercised by any existing test — Scope 1's own `TP-01-09`/`TP-01-10` precedent (`tests/shock-transmission.reader.unit.mjs`) already takes a pre-built `viewState` from the test fixture (`makeViewState(snapshot, definition)`) rather than requiring `rlshock.js` itself to assemble one. Building a from-scratch, correct, closed-shape view assembler from a bare snapshot was judged out of reach for one sub-pass without a dedicated design section to build it against, and is not required by TP-02-11/TP-02-12's literal wording (which speaks only of `projectViewState()` labelling a comparison, setting `persistable: false`, clearing local values, and reproducing the baseline digest). `projectViewState` was therefore implemented to take an already-loaded baseline `viewState` (exactly the shape `makeViewState()` already produces, consistent with the Scope 1 precedent) plus a `shock-transmission/projection-input/v1` carrying either `{hypothetical: null, adapterOutput: null}` (reset) or a validated `hypothetical` plus a raw `adapterOutput.{claims,edges}` section (comparison). This is a genuine, safely-scoped subset of the full design intent — the hypothetical contract, its non-persistence, and its exact reset — without overreaching into unspecified full view-assembly logic. This scoping decision is disclosed here rather than silently narrowed.

### Implementation

Added to `rlshock.js`, inserted between the existing `validateViewEnvelope` and `projectClaimRows` functions (no existing function body was modified); all additions are exported from the UMD surface:

- `CONTRACT_VERSIONS.hypothetical` (`"shock-transmission/hypothetical/v1"`) and `CONTRACT_VERSIONS.projectionInput` (`"shock-transmission/projection-input/v1"`) — `rlshock.js:23-24`.
- `HYPOTHETICAL_FIELDS` and `PROJECTION_INPUT_FIELDS` closed-shape constants — `rlshock.js:197-202`, matching design.md §13's exact eleven-member hypothetical contract (`contractVersion, topicId, baseSnapshotId, baseSnapshotDigest, definitionDigest, baselineViewDigest, leverValues, changedLeverIds, projectionClass, persistable, createdInMemoryAt`).
- `validateBaselineSectionShape(value, path, context)` — `rlshock.js:1923-1949`. Shape/claim/edge validation for a `{claims, edges}` section, factored so the hypothetical comparison section is validated by the identical rules `projectEdgeRows` already applies to the baseline (same range, sign, and non-empty evidence/limitation/refuter checks, same `RLSHOCK-PROJECTION-LOSSY` codes) — no separate, weaker path exists for hypothetical content.
- `validateLeverValues(value, leverRegistry, path, context)` — `rlshock.js:1951-1967`. Closed lever-id map validation: every declared lever must be present, no undeclared lever id is accepted, and every value must be a finite number within its declared `[minimum, maximum]` bound.
- `validateHypothetical(value, definition, baselineViewState, context)` — `rlshock.js:1969-1994`. Validates the complete `shock-transmission/hypothetical/v1` contract: closed shape; `contractVersion` equality; `topicId` and `baseSnapshotId` must equal the loaded baseline's own `topicId`/`snapshotId` (`RLSHOCK-REFERENCE` on mismatch); `definitionDigest` must equal the loaded definition's digest; `baselineViewDigest` must equal `digest(baselineViewState)` **exactly** (`RLSHOCK-DIGEST` on any mismatch — this is the mechanism that refuses a hypothetical built against a stale or different baseline); every `changedLeverIds` entry must reference a lever the definition actually declares (`RLSHOCK-UNKNOWN-MEMBER`); `projectionClass` must literally be `"user-hypothetical"`; `persistable` must literally be `false` (`RLSHOCK-HYPOTHETICAL-PERSIST` otherwise — a hypothetical cannot even declare itself persistable, let alone act that way).
- `validatePersistenceCandidate(value, path)` — `rlshock.js:1996-2009`. The production guard function any canonical sink (dossier writer, history appender, pointer updater, payload builder, tool-read cache) is meant to call before writing. Refuses (`RLSHOCK-HYPOTHETICAL-PERSIST`) any candidate whose `contractVersion` is the hypothetical or projection-input contract, whose `projectionClass` is `"user-hypothetical"`, or whose own `persistable` field is `false`. Admits an ordinary canonical-shaped candidate.
- `projectViewState(baselineViewState, definition, projectionInput)` — `rlshock.js:2011-2041`. Requires a validated baseline view (`projectionClass === "baseline"`, `persistable === false`) and a validated definition. With `projectionInput.hypothetical === null` (reset), returns a clone of the baseline view with `comparison: null`, `changedLeverIds: []`, `persistable: false` — content-identical to the original baseline, so its digest reproduces the baseline digest exactly. With a validated `hypothetical` present, validates it via `validateHypothetical`, validates `projectionInput.adapterOutput` via `validateBaselineSectionShape`, and returns a clone of the baseline view with `projectionClass: "user-hypothetical"`, `comparison` set to the validated adapter-output section, `changedLeverIds` taken from the hypothetical, and `persistable: false`. The original `baseline` section is never mutated by a comparison.

### Test Evidence

TP-02-11 — new test appended to `tests/shock-transmission.reader.unit.mjs`:

```text
node --test --test-name-pattern='^Regression: SCN-031-024 hypothetical projection is nonpersistable and reset is exact$' tests/shock-transmission.reader.unit.mjs
```
Result: `tests 1, pass 1, fail 0`. The test proves: (1) a valid same-topic lever change produces a `user-hypothetical`, `persistable: false` comparison whose `comparison` section is genuinely distinct from the baseline claim it overrides, whose `baseline` section is byte-identical to the loaded baseline (never mutated), and whose digest differs from the baseline digest; (2) a hypothetical carrying a `baselineViewDigest` that does not match the currently loaded baseline is refused at exactly `$.baselineViewDigest` with `RLSHOCK-DIGEST` — proving a stale or foreign hypothetical cannot silently attach to a different baseline; (3) a hypothetical that declares `persistable: true` is refused with `RLSHOCK-HYPOTHETICAL-PERSIST`; (4) reset (`{hypothetical: null, adapterOutput: null}`) produces a result that deep-equals the originally loaded baseline view and whose digest equals the original baseline digest exactly; (5) `RLSHOCK.validatePersistenceCandidate()` refuses both the raw hypothetical object and the labelled comparison view by exact code, and admits an ordinary canonical-shaped candidate; (6) a mismatched topic id and an undeclared lever id in `changedLeverIds` each refuse by exact path/code.

Full `tests/shock-transmission.reader.unit.mjs` run (all 3 tests, including the two pre-existing Scope 1 rows): `tests 3, pass 3, fail 0`.

TP-02-12 — new dedicated integration suite `tests/shock-transmission.hypothetical.integration.mjs` — `Regression: SCN-031-024 every canonical sink refuses local hypothetical state`:

```text
node --test --test-name-pattern='^Regression: SCN-031-024 every canonical sink refuses local hypothetical state$' tests/shock-transmission.hypothetical.integration.mjs
```
Result: `tests 1, pass 1, fail 0`.

This is a real integration test against an owned ephemeral repository, following the repo's established isolation-proof convention (the same `mkdtempSync`/byte-snapshot-before-and-after pattern used by `tests/distributed-briefs.git-isolation.integration.mjs`), not a mock:

1. A disposable temp directory (`mkdtempSync`) is seeded with eight real files standing in for the named canonical sinks: `dossier/current.json`, `history/2026-09-events.jsonl`, `pointer/research-agenda-current.json`, `payload/market-brief.payload.json`, `tool-reads/shock-transmission-read.json`, `storage/local-storage.json`, `network/outbound-request-log.json`, and `immutable/shock-transmission-snapshot.json`. Every file's exact byte content is snapshotted before the run.
2. `globalThis.fetch` is monkey-patched to `throw` if called at all, so any accidental network call fails the test loudly rather than silently succeeding — a real, executable proof rather than an assertion of absence.
3. A validated published baseline is built exactly as the dedicated Scope 1/2 tests do, the operator changes a definition-owned lever, and `RLSHOCK.projectViewState()` produces the `user-hypothetical`, `persistable: false` comparison.
4. Both the raw hypothetical contract and the labelled comparison view are routed through a real sink-write helper (`attemptSinkWrite`) that calls `RLSHOCK.validatePersistenceCandidate()` before ever touching the filesystem — exactly the production contract design.md requires ("Hypothetical persistence attempts refuse before artifact creation"). Every one of the 7 write attempts (dossier, history, pointer, payload, tool-read, storage, immutable) is asserted to refuse with exactly `RLSHOCK-HYPOTHETICAL-PERSIST` **before** any byte is written.
5. `fetchCallCount` is asserted to be `0` across the entire workflow.
6. Reset is exercised and its digest is asserted equal to the baseline digest; the reset/baseline view itself is also asserted to refuse the persistence guard (proving no local-hypothetical-shaped exemption creeps into an ordinary session projection either).
7. The ephemeral directory tree is re-snapshotted and asserted **byte-for-byte identical** to the pre-run snapshot, and the sink-file inventory itself is asserted unchanged (no new file, no deleted file).

**Negative-control verification (to prove this integration test is not vacuous):** a scratch copy of the test with `attemptSinkWrite`'s guard call removed (so it writes unconditionally, as a real production defect would) was run against the same fixture. It failed exactly as expected, on the "every canonical sink byte is unchanged" assertion, with the diff showing the hypothetical contract's own bytes written into `dossier/current.json`. This confirms the byte-snapshot comparison is a genuine, load-bearing proof and not one that would pass regardless of what the guard does. The scratch file was deleted after verification; it is not part of this commit.

### Regression Check

```text
node --test tests/shock-transmission.contracts.unit.mjs tests/shock-transmission.validation.functional.mjs tests/shock-transmission.resource.functional.mjs tests/shock-transmission.composition.unit.mjs tests/shock-transmission.lifecycle.functional.mjs tests/shock-transmission.reader.unit.mjs tests/shock-transmission.hypothetical.integration.mjs tests/shock-transmission.canary.functional.mjs
```
Every dedicated Feature 031 suite passes: contracts 5/5, validation 1/1, resource 1/1, composition 10/10, lifecycle 9/9, reader 3/3, hypothetical integration 1/1, canary 2/2.

```text
node scripts/selftest.mjs
```
Exit `1`. `Research-Lab self-test: 3503 passed, 3 failed`. The three failures are the same three pre-existing, unrelated findings recorded in every prior Scope 1/2 entry above (`committed surface carries no personal identifier`, the deferred-scorecard byte-budget check, and the BUG-016/BUG-017 acceptance-record finding). The `Feature 031 shock-transmission foundation` group in the selftest output is entirely green (3/3). No new failure was introduced by this sub-pass. The `BASELINE_SELFTEST_SHA256` sentinel in `tests/shock-transmission.canary.functional.mjs` (`6aa3964641cdb099fda27d85a64555567e2505c63f0d7b615c39cf2f38ea0cd6`, set during sub-pass 2) required **no update** this pass — both canary tests pass against it unchanged, so `scripts/selftest.mjs` did not drift outside its Feature 031 sentinel region between sub-pass 2 and sub-pass 3. `volatility-sizing-lab.html` and `tests/volatility-sizing-lab.spec.mjs` carry pre-existing uncommitted changes from unrelated, concurrent work in this working tree; those files were not touched by this sub-pass and are excluded from its commit.

### Change Boundary Verification

`git diff --stat` against this sub-pass's own changes (excluding the concurrent, unrelated `volatility-sizing-lab.*` files) touches exactly: `rlshock.js` (hypothetical-section additions only, no existing function body modified), `tests/shock-transmission.reader.unit.mjs` (TP-02-11 test appended), `tests/shock-transmission.hypothetical.integration.mjs` (new, TP-02-12), `tests/fixtures/shock-transmission/foundation-fixture.mjs` (three new hypothetical-fixture helper functions appended, no existing export changed), and this feature's own `report.md`/`scopes.md`. Every one of these is inside Scope 2's declared Allowed Product Files. No adapter, resolver, publisher, topic, Research Agenda HTML, registry, or owner-module file was touched.

### Disposition

TP-02-11 and TP-02-12 are genuinely closed by this sub-pass with real, executed evidence, including a verified-non-vacuous negative control for the sink-isolation proof. `DOD-02-TP-02-11` and `DOD-02-TP-02-12` are checked in `scopes.md`. Combined with sub-passes 1 and 2, every Scope 2 Test Plan row (TP-02-01 through TP-02-12) now has real, executed, honestly-labelled evidence. `DOD-02-C01` (design match, including the local hypothetical behavior scoped and disclosed above), `DOD-02-C02` (no predecessor input into `composeSnapshot()`, proven by sub-pass 2's arity assertions; local hypothetical state cannot enter any canonical path, proven by this sub-pass's guard function and integration test), and `DOD-02-C03` (change-boundary verification above) are checked.

`DOD-02-BQ` (the Scope 2 build-quality gate) remains **unchecked**. It requires a zero-exit `node scripts/selftest.mjs`, and the repository currently exits `1` with the same 3 pre-existing, unrelated findings that also block `DOD-01-BQ` for Scope 1 — a personal-identifier surface check, a deferred-scorecard byte-budget check, and the BUG-016/BUG-017 acceptance-record finding. None of the 3 is inside Scope 2's (or Scope 1's) allowed file families; repairing any of them here would violate this scope's declared change boundary. This is the same honest gap already recorded against `DOD-01-BQ`, carried forward rather than silently resolved.

**Scope 2 as a whole is genuinely substantially complete but not Done.** All 12 Test Plan rows and 3 of 4 Core DoD items are checked with real, executed evidence; only the shared build-quality gate item remains blocked by findings entirely outside this scope's boundary. Per the same honest-disclosure standard Scope 1 already established for its own identical `DOD-01-BQ` gap, Scope 2's `Status:` field in `scopes.md` is updated to **In Progress** (from **Not Started**) rather than **Done**. Flipping it to Done would require either (a) the 3 unrelated findings being repaired by their owning packets and a clean `node scripts/selftest.mjs` re-run, or (b) an explicit repository convention establishing that `DOD-0N-BQ` items may pass conditionally when 100% of the failures are pre-existing, documented, and outside the scope's own boundary — no such convention is recorded in this repository today, so this report does not assume one.

Spec 033 as a whole: with Scope 2 now In Progress (was Not Started) and Scope 1 already In Progress, 0 of the 5 declared scopes has reached a genuine, evidence-backed **Done** status yet — both open scopes share the identical `DOD-0N-BQ` blocker. Scope 3 ("Dynamic Definitions And Immutable V1 V2 Publication") is next; per `scopes.md`, it depends on both Scope 1 and Scope 2 and covers the Agenda resolver's dynamic-definition registries, cutoff/vintage handling, dual-read/single-write publication, and migration/history tests (`RLAGENDA` v1/v2 dispatch, generation-cutoff equality ordering, and immutable v2 publication) — none of which this sub-pass touched.

## Scope 3 Sub-pass 1 (Partial) — `resolveDefinitionRegistries(definition)` Pure Export — 2026-09-06

**Phase:** implement/test
**Claim Source:** executed
**Scope status:** This is a genuine but partial sub-pass. Only the `rlshock.js`-owned `resolveDefinitionRegistries(definition)` export and its two directly-attributable Test Plan rows (TP-03-01, TP-03-09) are closed with real, executed evidence. Every other Scope 3 Test Plan row (TP-03-02 through TP-03-08, TP-03-10 through TP-03-13) remains untouched. Scope 3's `Status:` field in `scopes.md` is updated from **Not Started** to **In Progress** — not Done, and not claimed as a larger fraction of Done than this sub-pass actually delivered.

### Scope Of This Sub-pass

Scope 3 is a large scope: it spans a new `rlagenda.js` v1/v2 dual-read and definition-selected single-write contract, `research-situation/v2`/`research-review/v2`/`research-dossier/v2` composition inside the existing generation transaction (`scripts/research-agenda-generation.mjs`), definition-selected adapter/input preparation (`scripts/research-agenda-refresh.mjs`), content-addressed immutable publication of the real geopolitical and food-input v2 definition families under `research/agenda/definitions/<topicId>/<digest>.json`, scenario-curve tolerance/provenance validation, calibration-threshold enforcement, predecessor-lineage checks for every revised primitive, a canonical-byte-owner boundary test at 262143/262144/262145 bytes, a mixed-version-refusal migration/atomicity integration suite, a `LEGACY-031-001` gate on geopolitical production selection, and a Playwright e2e regression proving the live v1 Research Agenda route stays readable beside additive v2 publication.

None of that transaction, publication, or gating machinery was investigated deeply enough in this pass to implement it correctly and prove it with real evidence — each of those pieces individually carries its own nontrivial design surface (atomic multi-file transactions with pointer-last rollback, content-addressed digest identity and predecessor-linkage rules, scenario-curve tolerance arithmetic, calibration boundary semantics) and attempting all of it in one pass risked exactly the kind of fabricated-evidence outcome this task was explicitly warned against. This sub-pass instead delivers the one piece of Scope 3 that is: (a) fully specified in isolation (design.md §4.2's export table: `resolveDefinitionRegistries(definition)` — validated selected definition in, distinct frozen horizon and lever registries out), (b) scoped to a single allowed product file with a narrow declared boundary (`rlshock.js` only for `resolveDefinitionRegistries(definition)` and its closed frozen registry result), and (c) directly testable without any new transaction, filesystem, or publication infrastructure — and proves it with two real, executed Test Plan rows.

### Implementation

Added to `rlshock.js`, inserted immediately after the existing `validateDefinition` function (no existing function body was modified):

- `resolveDefinitionRegistries(definition)` — `rlshock.js:829-865`. Takes an already-validated `shock-transmission/definition/v1` value. Defensively checks it is a plain object with the correct `contractVersion` and array-typed `horizonRegistry`/`leverRegistry` (refusing by exact path/code otherwise, consistent with every other exported function in this module), then builds two new plain-object arrays — one horizon row per declared horizon (`horizonId, label, order, durationBasis, startExclusive, endInclusive, scenarioSetId, calibrationPolicyId`), one lever row per declared lever (`leverId, label, description, unitId, minimum, maximum, step, baselinePath, targetIds, ownerAdapterId`) — and returns them through the module's standard `success()` wrapper (which `deepFreeze`s the result) alongside `definitionId`, `definitionDigest`, and a `digest()` of each registry (`horizonRegistryDigest`, `leverRegistryDigest`). The function performs no topic switch, no clearing, no route claim, and does not mutate its input; it is a pure read-projection over an already-validated definition, matching design.md §4.2's row exactly.
- Exported from the UMD surface — `rlshock.js:2168` (`resolveDefinitionRegistries: resolveDefinitionRegistries` added to the frozen export object, alongside the existing `validateDefinition` entry).

### Test Evidence

TP-03-01 — new dedicated suite `tests/shock-transmission.definitions.unit.mjs` — `Regression: SCN-031-014 definitions retain independent ordered horizons`:

```text
node --test --test-name-pattern='^Regression: SCN-031-014 definitions retain independent ordered horizons$' tests/shock-transmission.definitions.unit.mjs
```
Result: `tests 1, pass 1, fail 0`. The test builds two independently-shaped definitions from the existing fixture (`makeDefinition({horizonCount: 1})` and `makeDefinition({horizonCount: 3})`), validates both through the existing `RLSHOCK.validateDefinition()`, and resolves both through `resolveDefinitionRegistries()`. It proves: each resolved horizon registry is byte-identical to its own definition's declared horizons (not the other's); the three-horizon definition's horizons retain contiguous declared order, stable ids, labels, duration basis, and interval bounds with no coercion toward the legacy `structural`/`swing`/`tactical` vocabulary; the two definitions' resolved registries carry distinct lengths and distinct `horizonRegistryDigest`/`definitionDigest` values (proving independence, not aliasing); and the returned horizon registry and its rows are frozen (`Object.isFrozen`) and are not the same array reference as the source definition's own `horizonRegistry`.

TP-03-09 — new test appended to `tests/shock-transmission.reader.unit.mjs` — `Regression: SCN-031-026 selected definitions expose independent ordered lever models`:

```text
node --test --test-name-pattern='^Regression: SCN-031-026 selected definitions expose independent ordered lever models$' tests/shock-transmission.reader.unit.mjs
```
Result: `tests 1, pass 1, fail 0`. The test resolves the Scope 1/2 fixture's own definition through `resolveDefinitionRegistries()` and proves: the result carries exactly the six documented keys (`definitionId, definitionDigest, horizonRegistry, horizonRegistryDigest, leverRegistry, leverRegistryDigest`) and no others; the horizon registry retains contiguous `order` and is deep-equal to the source definition's declared horizons; the lever registry retains stable `leverId`, `unitId`, `minimum`, `maximum`, `step`, `baselinePath`, and `targetIds` per row, matching the source definition exactly; both registries are frozen and carry distinct digests from each other, and each digest equals `RLSHOCK.digest()` of its own registry independently computed by the test (not merely echoed back); calling the resolver twice on the same definition is idempotent (`deepEqual` on both results) and leaves the source definition's own `horizonRegistry` unchanged; and no topic-switch, clearing, or route-claim side effect is asserted or exercised anywhere in the call — the function is proven pure.

Full `tests/shock-transmission.definitions.unit.mjs` and `tests/shock-transmission.reader.unit.mjs` run together: `tests 5, pass 5, fail 0`.

### Regression Check

```text
node --test tests/shock-transmission.contracts.unit.mjs tests/shock-transmission.validation.functional.mjs tests/shock-transmission.resource.functional.mjs tests/shock-transmission.composition.unit.mjs tests/shock-transmission.lifecycle.functional.mjs tests/shock-transmission.reader.unit.mjs tests/shock-transmission.definitions.unit.mjs tests/shock-transmission.hypothetical.integration.mjs tests/shock-transmission.canary.functional.mjs
```
Result: `tests 35, pass 34, fail 1`. The one failure is `tests/shock-transmission.e2e.spec.mjs`, which is a Playwright spec file being invoked through Node's own `--test` runner rather than `npx playwright test` (it throws `Playwright Test did not expect test.beforeAll() to be called here` — a harness-invocation mismatch, not a code failure); this file was not touched by this sub-pass and is excluded from the dedicated-suite command above in every prior Scope 1/2 report entry for the same reason. Every other dedicated Feature 031 suite passes, including both canary tests unchanged against their existing `BASELINE_SELFTEST_SHA256` sentinel.

```text
node scripts/selftest.mjs
```
Exit `1`. `Research-Lab self-test: 3504 passed, 2 failed`. The two failures are: `the deferred scorecard is a real 11982-byte artifact whose inclusion would exceed the blocking first-load budget`, and `the real BUG-016/BUG-017 pair is cleared as ONE declared acceptance act rather than by a new baseline entry`. Both are unrelated, pre-existing findings entirely outside Feature 031's file families (a deferred-scorecard byte-budget check and a Human Acceptance Record baseline check). The `Feature 031 shock-transmission foundation` group in the selftest output is entirely green (3/3), and no shock-transmission-owned test anywhere in the full run failed. Note: the prior Scope 2 sub-pass 3 entry recorded 3 pre-existing failures at that point (including a `committed surface carries no personal identifier` finding); this run shows only 2, meaning one of the three has since been repaired by the concurrent, unrelated selftest-infra agent referenced in this task's own instructions — consistent with "no new regression," since the count went down, not up.

### Change Boundary Verification

`git diff --stat` against this sub-pass's own changes (excluding concurrent, unrelated changes already present in the working tree before this sub-pass started — `.github/bubbles/scripts/measured-budget-runtime-v2-selftest.py`, the `specs/031-volatility-roughness-and-model-assumption-diagnostic/*` files, `tests/rlvol-roughness.unit.mjs`, `tests/volatility-sizing-lab.spec.mjs`, and `volatility-sizing-lab.html`, none of which this sub-pass touched or staged) covers exactly: `rlshock.js` (the `resolveDefinitionRegistries` addition only; no existing function body modified), `tests/shock-transmission.reader.unit.mjs` (TP-03-09 test appended), `tests/shock-transmission.definitions.unit.mjs` (new, TP-03-01), and this feature's own `report.md`/`scopes.md`/`state.json`. Every one of these is inside Scope 3's declared Allowed Product Files (`rlshock.js only for resolveDefinitionRegistries(definition)...`; `tests/shock-transmission.reader.unit.mjs for the pure selected-definition registry model`; the planned `tests/shock-transmission.definitions.unit.mjs`). No file under `rlagenda.js`, `scripts/research-agenda-generation.mjs`, `scripts/research-agenda-refresh.mjs`, `research-agenda.json`, or the excluded surfaces (adapters, route HTML, registries, owner modules, Horizon Ladder) was touched.

### Disposition — Honest Remainder

**What is genuinely Done in this sub-pass, with real executed evidence:**
- `resolveDefinitionRegistries(definition)` matches design.md §4.2's export contract (part of `DOD-03-C01`, but not the whole item — see below).
- TP-03-01 and TP-03-09 (`DOD-03-TP-03-01`, `DOD-03-TP-03-09`) are checked in `scopes.md` with executed evidence.

**What remains explicitly undone — every other Scope 3 Test Plan row and DoD item:**
- `DOD-03-C01` (compound item) — only the `resolveDefinitionRegistries` clause is proven; the "definition-owned horizons, levers, units, scenario sets, calibration policies, adapter selection... match the design" clause as a whole is not re-verified beyond what Scope 1 already covers, so this item is left **unchecked**.
- `DOD-03-C02`, `DOD-03-C03`, `DOD-03-C04`, `DOD-03-C05` — none of the v1/v2 dual-read contract, single-write publication, cutoff/vintage/predecessor-isolation machinery, `LEGACY-031-001` gating, or consumer/shared-infrastructure sweep was implemented. These require new work in `rlagenda.js`, `scripts/research-agenda-generation.mjs`, `scripts/research-agenda-refresh.mjs`, and `research-agenda.json` that this sub-pass did not attempt.
- `DOD-03-TP-03-02` through `DOD-03-TP-03-08`, `DOD-03-TP-03-10` through `DOD-03-TP-03-13` — all remain unchecked. In particular: no scenario-curve tolerance/provenance validation exists yet (TP-03-02, TP-03-03); no content-addressed geopolitical/food-input v2 definition files have been created or published (TP-03-12, and the "Create and publish... exactly once" Implementation Plan step 9); no migration/atomicity integration suite (`tests/shock-transmission.migration.integration.mjs`) exists (TP-03-04, TP-03-05, TP-03-06, TP-03-10, TP-03-12); no canonical-byte-owner-boundary test at 262143/262144/262145 bytes exists (TP-03-10); no predecessor-lineage functional suite exists (TP-03-11); no Playwright e2e regression for live v1/v2 coexistence exists (TP-03-13); the planned `tests/shock-transmission.canary.functional.mjs` addition for TP-03-08 was not added.
- `DOD-03-BQ` remains unchecked, for the same reason recorded against `DOD-01-BQ`/`DOD-02-BQ`: `node scripts/selftest.mjs` currently exits `1` on 2 pre-existing, unrelated findings outside this scope's boundary.

**Recommendation for continuing Scope 3:** the next sub-pass should tackle the `rlagenda.js` v1/v2 dual-read contract and the generation-transaction v2 composition together (Implementation Plan steps 1-7), since TP-03-04/05/06/10/12 (the migration integration suite) all depend on that transaction existing before any of them can be proven with real evidence; scenario-curve tolerance and calibration-threshold validation (TP-03-02, TP-03-03, TP-03-07, TP-03-11) is a separable, independently-testable piece of `rlshock.js`/`scripts/research-agenda-generation.mjs` work that could reasonably be its own sub-pass; the geopolitical/food-input content-addressed publication (TP-03-12) and the `LEGACY-031-001` gate should be sequenced after the transaction exists, since the publication path is exercised through it; TP-03-13 (Playwright e2e) should be last, once the underlying v1/v2 coexistence it is asserting actually exists to observe.

### Scope 4 Recommendation

Per `scopes.md`, Scope 4 ("Three Adapters And Lossless Production Consumption") depends on Scopes 1-3 and owns the sole `RLAGENDA.resolveAgendaConsumerState(request, dependencies)` parent resolver export, its callers, and the three production topic adapters plus lossless consumption. Scope 4 cannot meaningfully start until Scope 3's v1/v2 dual-read/single-write contract exists for it to bind against (Scope 3's own Implementation Plan step 1 explicitly reserves that resolver export for Scope 4, and its Consumer Impact Sweep table repeatedly notes "Scope 4 owns the sole parent resolver before activation" / "Scope 4 owns the sole browser resolver call"). This sub-pass does not change that sequencing: Scope 4 still depends on a materially more complete Scope 3 than exists today.
