...
- [x] **SCN-011B-001** — Every test in `tests/causal-rotation-consumers.spec.mjs` declares `test.setTimeout(180_000)` as its first statement, and the value is an existing in-repo magnitude
...
- [x] **SCN-011B-002** — The full committed suite passes with zero failures in `tests/causal-rotation-consumers.spec.mjs`, run at the four-worker parallelism that produced the red
...
- [x] **SCN-011B-003** — The committed diff is additive only, introduces no `retries`, marks no test `.skip`/`.fixme`, deletes no assertion, and leaves `enterOwnerView` unchanged
  - **Phase:** implement · **Claim Source:** executed, this run
...
- [x] **SCN-011B-004** — `node scripts/validate-playwright-timeout-budgets.mjs` exits 0
  - **Phase:** implement · **Claim Source:** executed, this run
  - ```
...
- [x] **SCN-011B-005** — The BUG-011 fix removed, renamed and skipped no test — asserted over the fix commit and the delivered file, not as a pinned suite total
