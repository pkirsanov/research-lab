...
const SUPERSEDED_VALIDATOR_NOTE_BLOCK_SHA256 = '4ae9f32d42ec5ff2837b2f84da90b812e5f88346c9e84c8b337a400f6923486a';
...
const DURABLE_EVIDENCE_BLOCK_SHA256 = 'c5ed7a110a2a743d2aef3b32c0655a2fd3c20c7ca6c9ee2ecef2716654ef7268';
const CURRENT_IDENTITY_V4_BLOCK_SHA256 = '64ddfd9bb8a2adbaa218eb1a7f0efbe62746a51b7fcdc0ad2444debf297da30c';
const CURRENT_IDENTITY_V5_BLOCK_SHA256 = 'c3a487a6e6d2f45084635a1fe6a8269a1efb4096ff76acebec3b419c36551780';
const FOREIGN_ROADMAP_V6_BLOCK_SHA256 = 'c2f6fc88b147e8de6501a7c87d41aaf3aa8bd9537ac49d06da61e49976b246f9';
const FOREIGN_SET_V7_BLOCK_SHA256 = '851dca31caded6498c1a84e68e9e9372489c3cb39f3663ea8b254b8cbb35c121';
const POST_COMMIT_V9_BLOCK_SHA256 = 'fc6367325829099019f47966f54da67a33a88db13c49404c0416f51511f5a921';
const POST_COMMIT_V10_BLOCK_SHA256 = '7ec73108b5159174f341ddeb50dcadac1d9ccc769ccec3484b0e34c6c3d4f9d8';
const POST_COMMIT_V11_BLOCK_SHA256 = 'b33e1323bafb4ad7da3b8846ba825fe67a6b7778e16a86cc6a4af6f832a62d9e';
...
const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256 = '4c361cdac1998882cc6dc408c99158ea20efa08eb757e8e5f31ea671810cd6cc';
...
function parseCollisionContractsWithPostCommitV11() {
  ...
  assert.equal(Buffer.byteLength(v9Block.raw), 23091, 'post-commit v9 marker-inclusive byte length is exact');
  assertPinnedReportBlock(v10Block.raw, POST_COMMIT_V10_BLOCK_SHA256, 'post-commit v10 block');
  assert.equal(Buffer.byteLength(v10Block.raw), 39470, 'post-commit v10 marker-inclusive byte length is exact');
  ...
  if (!postCommitV11ValidationLogged) {
    console.log(`FEATURE004_V10_HISTORY_VALIDATED marker=${POST_COMMIT_V10_MARKER} sha256=${POST_COMMIT_V10_BLOCK_SHA256} bytes=39470 requiredHead=${POST_COMMIT_V10_REQUIRED_HEAD} liveComparison=false historicalTests=3/3`);
...
const POST_COMMIT_V12_BLOCK_BYTE_LENGTH = 58977;
const POST_COMMIT_V12_REPORT_PREFIX_BYTE_LENGTH = 520131;
const POST_COMMIT_V12_REPORT_PREFIX_SHA256 = '647c8208a34d4852430da49f1674ba8036f158fb3f287f9959fb34e667105863';
...
const POST_COMMIT_V12_CLOSURE_SHA256 = 'bae952e125487d83e472316f52eb634de5db51b4a44da8997705f8113baaf879';
...
function parseCollisionContractsWithPostCommitV12() {
  ...
  for (const [block, pin, label, byteLength] of [
    ...
    [v10Block, POST_COMMIT_V10_BLOCK_SHA256, 'post-commit v10 block', 39470],
...
const POST_COMMIT_V13_REPORT_PREFIX_BYTE_LENGTH = 580597;
const POST_COMMIT_V13_REPORT_PREFIX_SHA256 = '2a36893cdab2bd945bd16e6fde8deeb3fafdfbc71ef3c399164b9a39c0a8fb44';
...
function postCommitV13HistoricalBlocks() {
  ...
  const v12Block = parseReportBlock(report, POST_COMMIT_V12_MARKER);
  for (const [block, pin, label, byteLength] of [
    [v10Block, POST_COMMIT_V10_BLOCK_SHA256, 'historical v10 block', 39470],
...
/* FEATURE-004-COLLISION-HERMETIC-HISTORICAL-V13-V16-BEGIN */
const FEATURE004_V16_MARKER = 'feature004-dirty-collision-hermetic-v13-v16';
const FEATURE004_V16_BEGIN = '/* FEATURE-004-COLLISION-HERMETIC-HISTORICAL-V13-V16-BEGIN */';
const FEATURE004_V16_END = '/* FEATURE-004-COLLISION-HERMETIC-HISTORICAL-V13-V16-END */';
const FEATURE004_V16_ALLOWED_PATHS = [REPORT_PATH, COLLISION_PARSER_PATH];
const FEATURE004_V16_PLAN_COMMIT = '1e3abdd9c2012c7b0bd5e2ecddefbd04bc1c3ca8';
const FEATURE004_V16_CAPTURED_AT = '2026-08-07T01:01:08.000Z';
const FEATURE004_V16_PAYLOAD_FIELDS = [
  'contractVersion',
  'findingId',
  'capturedAt',
  'predecessorAuthority',
  'hermeticAdapterPolicy',
  'redactedForeignAuthority',
  'noCurrentAuthority',
  'commitPolicy',
  'canonicalBindingReuse'
];
const FEATURE004_V16_V12_REDACTED_ROOT = '/home/redacted/research-lab';
const FEATURE004_V16_V12_CANONICAL_ROOT = '/home/<owner>/research-lab';
const FEATURE004_V16_OPERATOR_ROOT_PATTERN = /^\/home\/[^/]+\/research-lab$/;

// The immutable v13 payload is Brotli-compressed, so the repository-wide de-personalization
// pass that redacted every plaintext operator path could not reach the operator root recorded
// inside it. The payload stays byte-immutable and its raw, compressed and decoded hashes stay
// pinned; this projection maps that single foreign field onto the committed redaction so the
// expectation never has to carry an operator home path this repository forbids committing.
// (F004-V16-HERMETIC-HISTORICAL-V13-001)
function feature004V16RedactOperatorRoot(binding) {
  assert.match(binding.repositoryRoot, FEATURE004_V16_OPERATOR_ROOT_PATTERN,
    'v16 historical v13 recorded repository root is an operator home path');
  assert.notEqual(binding.repositoryRoot, FEATURE004_V16_V12_REDACTED_ROOT,
    'v16 records the compressed operator root as unredacted inside the immutable payload');
  return { ...binding, repositoryRoot: FEATURE004_V16_V12_REDACTED_ROOT };
}
const FEATURE004_V16_V12_COMMITTED_RAW_SHA256 =
  '8bb5363e0a14b24a7101f030be024d11598fcdabd8563068eabf627b867ef1d9';
const FEATURE004_V16_V12_COMMITTED_BYTE_LENGTH = 58978;
const FEATURE004_V16_V12_PROJECTED_RAW_SHA256 =
  'b1fc6d9c58002ce07c4eb48f55ff245c19c6c6d99f00d33a18534c9931e03e0b';
const FEATURE004_V16_V13_PAYLOAD_FIELDS = [
  'contractVersion',
  'findingId',
  'successorRevision',
  'capturedAt',
...
const FEATURE004_V16_CONTRACT = {
  contractVersion: 'feature004-dirty-collision-hermetic-v13-v16/v1',
  findingId: 'F004-V16-HERMETIC-HISTORICAL-V13-001',
  capturedAt: FEATURE004_V16_CAPTURED_AT,
  predecessorAuthority: {
    marker: FEATURE004_V15_MARKER,
    rawBlockSha256: POST_COMMIT_V15_BLOCK_SHA256,
  ...
  hermeticAdapterPolicy: {
    caller: 'feature004V14RunInheritedV13AdversarialCases',
    adapter: 'feature004V16RunHermeticHistoricalV13AdversarialCases',
    v13Authority: 'feature004V14HistoricalV13Authority-decoded-immutable-payload',
    v12Authority: 'immutable-v13-bounded-history',
    recordConverter: 'feature004V16V13RecordToV12FullRecord',
    inventoryConverter: 'porcelainEntries-rawStatus-path-projection',
    inheritedRunner: 'runPostCommitV13AdversarialCases-unchanged',
    providerOverrides: [
      'postCommitV12CurrentRecords',
      'postCommitV12InventoryEntries',
      'capturePostCommitV12AuthorityObservations'
    ],
    providerResults: 'structured-clones-of-immutable-projections',
    forbiddenCurrentCaptureFunctions: [
      'captureV13Snapshot',
      'captureV13ValidatePayloadAgainstRecapture',
      'captureV13FullRecord'
    ],
    functionIdentityRestoration: 'finally-asserted',
    currentForeignReads: 0,
    v14CurrentAuthorityMutationRejection: 'separate-and-unchanged'
  },
  redactedForeignAuthority: {
    marker: POST_COMMIT_V12_MARKER,
    field: 'repositoryBinding.repositoryRoot',
    supersededRawBlockSha256: POST_COMMIT_V12_BLOCK_SHA256,
    supersededMarkerInclusiveByteLength: POST_COMMIT_V12_BLOCK_BYTE_LENGTH,
    committedRawBlockSha256: FEATURE004_V16_V12_COMMITTED_RAW_SHA256,
    committedMarkerInclusiveByteLength: FEATURE004_V16_V12_COMMITTED_BYTE_LENGTH,
    redactedRoot: FEATURE004_V16_V12_REDACTED_ROOT,
    canonicalRoot: FEATURE004_V16_V12_CANONICAL_ROOT,
    projectedRawBlockSha256: FEATURE004_V16_V12_PROJECTED_RAW_SHA256,
    preRedactionBytesRecoverableFromCommittedTree: false,
    retainedRecordAuthority: {
      matrixSha256: POST_COMMIT_V12_MATRIX_SHA256,
      inventorySha256: POST_COMMIT_V12_INVENTORY_SHA256,
      closureSha256: POST_COMMIT_V12_CLOSURE_SHA256
    }
  },
  noCurrentAuthority: true,
  commitPolicy: {
    exactAllowedPaths: FEATURE004_V16_ALLOWED_PATHS,
    additionalPathsAllowed: false
  },
  canonicalBindingReuse: {
    repositoryRoot: FEATURE004_V16_V12_REDACTED_ROOT,
    repositoryAlias: 'research-lab',
    sessionId: 'vscode-a855981ff9231b3a8f1b780a6192a1da',
    decisionId: 'rb:vscode-a855981ff9231b3a8f1b780a6192a1da:1',
    controlRevision: 1,
    controlPathDigest: 'sha256:45d65b0e481c5f6d7d5eed2fa26b3ba5c55488f131d66828fe6024fc1eadddd1',
...
// The committed v12 block redacts exactly one field - repositoryBinding.repositoryRoot,
// an operator home path - and that redaction landed after the v12 raw pin was taken. It
// widened the block by one byte, so POST_COMMIT_V12_BLOCK_SHA256 and its byte length now
// describe bytes no committed tree can hold: reproducing them would require committing the
// operator path this repository forbids. v16 supersedes those two raw-byte authorities with
// a fixed-width canonical projection of the same foreign field, and keeps every record
// authority the block itself pins, none of which ever contained the redacted field.
function feature004V16ProjectImmutableV12Raw(raw) {
  assert.equal(typeof raw, 'string', 'v16 immutable v12 raw block is a string');
  assert.equal(sha256(raw), FEATURE004_V16_V12_COMMITTED_RAW_SHA256,
    'v16 immutable v12 committed raw block is exactly pinned');
  assert.equal(Buffer.byteLength(raw), FEATURE004_V16_V12_COMMITTED_BYTE_LENGTH,
    'v16 immutable v12 committed marker-inclusive byte length is exact');
  assert.notEqual(sha256(raw), POST_COMMIT_V12_BLOCK_SHA256,
    'v16 records the pre-redaction v12 raw pin as unreachable from committed bytes');
  assert.equal(raw.split(FEATURE004_V16_V12_REDACTED_ROOT).length - 1, 1,
    'v16 immutable v12 block redacts exactly one foreign repository root');
  assert.equal(raw.includes(FEATURE004_V16_V12_CANONICAL_ROOT), false,
    'v16 immutable v12 committed block carries no canonical projection token');
  const projected = raw.split(FEATURE004_V16_V12_REDACTED_ROOT)
    .join(FEATURE004_V16_V12_CANONICAL_ROOT);
  assert.equal(Buffer.byteLength(projected), POST_COMMIT_V12_BLOCK_BYTE_LENGTH,
    'v16 canonical v12 projection restores the immutable marker-inclusive byte length');
  assert.equal(sha256(projected), FEATURE004_V16_V12_PROJECTED_RAW_SHA256,
    'v16 canonical v12 projection is exactly pinned');
  return projected;
}

function feature004V16AssertImmutableV12RecordAuthority(value) {
  assert.equal(value.repositoryBinding.repositoryRoot, FEATURE004_V16_V12_REDACTED_ROOT,
    'v16 immutable v12 foreign binding root is the committed redaction');
  assert.equal(sha256(JSON.stringify(value.inventoryProof.porcelainEntries)),
    POST_COMMIT_V12_INVENTORY_SHA256,
  'v16 immutable v12 inventory authority survives the foreign redaction');
  assert.equal(value.inventoryProof.inventorySha256, POST_COMMIT_V12_INVENTORY_SHA256,
    'v16 immutable v12 recorded inventory hash is exact');
  assert.equal(value.currentMatrix.matrixSha256, POST_COMMIT_V12_MATRIX_SHA256,
    'v16 immutable v12 recorded record-matrix hash is exact');
  assert.equal(sha256(JSON.stringify(value.protectedAuthorityClosure.orderedEntries)),
    POST_COMMIT_V12_CLOSURE_SHA256,
  'v16 immutable v12 closure authority survives the foreign redaction');
  assert.equal(value.protectedAuthorityClosure.closureSha256, POST_COMMIT_V12_CLOSURE_SHA256,
    'v16 immutable v12 recorded closure hash is exact');
}

const parseReportBlockBeforeV16 = parseReportBlock;
parseReportBlock = (report, marker) => {
  const block = parseReportBlockBeforeV16(report, marker);
  if (marker !== POST_COMMIT_V12_MARKER) return block;
  feature004V16AssertImmutableV12RecordAuthority(block.value);
  return { raw: feature004V16ProjectImmutableV12Raw(block.raw), value: block.value };
};

const assertPinnedReportBlockBeforeV16 = assertPinnedReportBlock;
assertPinnedReportBlock = (raw, expectedSha256, label) => {
  if (expectedSha256 !== POST_COMMIT_V12_BLOCK_SHA256) {
    assertPinnedReportBlockBeforeV16(raw, expectedSha256, label);
    return;
  }
  assertPinnedReportBlockBeforeV16(raw, FEATURE004_V16_V12_PROJECTED_RAW_SHA256,
    `${label} superseded by the v16 canonical redaction-invariant projection`);
};

function feature004V16RenderBlock(payload) {
  return [
    `<!-- ${FEATURE004_V16_MARKER}:start -->`,
    '```json',
    JSON.stringify(payload),
    '```',
    `<!-- ${FEATURE004_V16_MARKER}:end -->`
  ].join('\n');
}

function feature004V16BoundedV15Bytes(reportBytes) {
  const v15EndMarker = Buffer.from(`<!-- ${FEATURE004_V15_MARKER}:end -->`, 'utf8');
  assert.equal(captureV13BufferOccurrenceCount(reportBytes, v15EndMarker), 1,
    'v16 history contains exactly one v15 end marker');
  const v15EndByteExclusive = reportBytes.indexOf(v15EndMarker) + v15EndMarker.length;
  assert.deepEqual(reportBytes.subarray(v15EndByteExclusive, v15EndByteExclusive + 2),
    Buffer.from('\n\n'), 'v16 history preserves the exact v15 two-LF separator');
  return reportBytes.subarray(0, v15EndByteExclusive + 2);
}

const feature004V15ReportContextBeforeV16 = feature004V15ReportContext;

function feature004V16ReportContext(reportBytes) {
  assert.ok(Buffer.isBuffer(reportBytes), 'v16 report input is binary-safe');
  const boundedV15 = feature004V16BoundedV15Bytes(reportBytes);
  const v15 = feature004V15ReportContextBeforeV16(boundedV15);
  assert.equal(v15.blockBytes.length, 2888,
    'v16 retains the exact v15 marker-inclusive byte length');
  assert.equal(sha256(v15.blockBytes), POST_COMMIT_V15_BLOCK_SHA256,
    'v16 retains the exact v15 marker-inclusive raw pin');
  const startMarker = Buffer.from(`<!-- ${FEATURE004_V16_MARKER}:start -->`, 'utf8');
  const endMarker = Buffer.from(`<!-- ${FEATURE004_V16_MARKER}:end -->`, 'utf8');
  assert.equal(captureV13BufferOccurrenceCount(reportBytes, startMarker), 1,
    'report contains exactly one v16 start marker');
  assert.equal(captureV13BufferOccurrenceCount(reportBytes, endMarker), 1,
    'report contains exactly one v16 end marker');
  const startByte = reportBytes.indexOf(startMarker);
  assert.equal(startByte, boundedV15.length,
    'v16 begins immediately after the exact v15 two-LF separator');
  const endMarkerStartByte = reportBytes.indexOf(endMarker, startByte);
  assert.ok(endMarkerStartByte > startByte, 'v16 marker order is exact');
  const endByteExclusive = endMarkerStartByte + endMarker.length;
  assert.deepEqual(reportBytes.subarray(endByteExclusive), Buffer.from('\n\n'),
    'v16 is report-final with exactly two LF suffix bytes');
  const blockBytes = reportBytes.subarray(startByte, endByteExclusive);
  const block = blockBytes.toString('utf8');
  assert.deepEqual(Buffer.from(block, 'utf8'), blockBytes,
    'v16 marker-inclusive bytes are lossless UTF-8');
  return { v15, startByte, endMarkerStartByte, endByteExclusive, blockBytes, block };
}

feature004V15ReportContext = (reportBytes) => {
  const marker = Buffer.from(`<!-- ${FEATURE004_V16_MARKER}:start -->`, 'utf8');
  if (captureV13BufferOccurrenceCount(reportBytes, marker) === 0) {
    return feature004V15ReportContextBeforeV16(reportBytes);
  }
  return feature004V16ReportContext(reportBytes).v15;
};

function feature004V16AssertPin(source) {
  const familyAssignments = [...source.matchAll(
    /^const [A-Z0-9_]*HERMETIC_HISTORICAL_V13_V16[A-Z0-9_]*BLOCK_SHA256 = '[^']*';$/gm)];
  assert.equal(familyAssignments.length, 1,
    'v16 parser has exactly one v16 raw-pin family member');
  const assignments = [...source.matchAll(
    /^const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256 = '([^']*)';$/gm)];
  assert.equal(assignments.length, 1, 'v16 parser has exactly one raw pin literal');
  assert.match(assignments[0][1], /^[a-f0-9]{64}$/,
    'v16 parser raw pin is canonical lowercase SHA-256');
  assert.equal(assignments[0][1], HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256,
    'v16 parser raw pin has the declared value');
  const v15Line = `const POST_COMMIT_V15_BLOCK_SHA256 = '${POST_COMMIT_V15_BLOCK_SHA256}';\n`;
  const v16Line = `const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256 = '${HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256}';\n`;
  assert.equal(source.indexOf(v16Line), source.indexOf(v15Line) + v15Line.length,
    'v16 parser pin immediately follows the immutable v15 pin');
  return assignments[0][1];
}

function feature004V16StripSource(source) {
  const startNeedle = `\n${FEATURE004_V16_BEGIN}\n`;
  const endNeedle = `${FEATURE004_V16_END}\n\n`;
  assert.equal(source.split(startNeedle).length - 1, 1,
    'v16 source has exactly one additive branch start');
  assert.equal(source.split(endNeedle).length - 1, 1,
    'v16 source has exactly one additive branch end');
  const start = source.indexOf(startNeedle);
  const end = source.indexOf(endNeedle, start);
  let historical = source.slice(0, start) + '\n' + source.slice(end + endNeedle.length);
  const pinLine = `const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256 = '${HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256}';\n`;
  assert.equal(countExact(historical, pinLine), 1,
    'v16 source has exactly one raw pin addition');
  historical = historical.replace(pinLine, '');
  const interceptableFsImport = [
    'import {',
    '  existsSync,',
    '  lstatSync,',
  ...
  const historicalFsImport =
    "import { existsSync, lstatSync, readFileSync, readlinkSync } from 'node:fs';\n";
  assert.equal(countExact(historical, interceptableFsImport), 1,
    'v16 source has exactly one interceptable filesystem import');
  historical = historical.replace(interceptableFsImport, historicalFsImport);
  const interceptableReadBinding = 'let readFileSync = nodeReadFileSync;\n\n';
  assert.equal(countExact(historical, interceptableReadBinding), 1,
    'v16 source has exactly one interceptable read binding');
  historical = historical.replace(interceptableReadBinding, '');
  assert.equal(countExact(historical, 'nodeReadFileSync'), 0,
    'v16 strip leaves no interceptable read alias');
  return historical;
}

function feature004V16StripSourceIfPresent(source) {
  const starts = source.split(FEATURE004_V16_BEGIN).length - 1;
  const ends = source.split(FEATURE004_V16_END).length - 1;
  assert.equal(starts, ends, 'v16 historical source has balanced branch markers');
  return starts === 0 ? source : feature004V16StripSource(source);
}

const feature004V15NormalizedParserBytesBeforeV16 = feature004V15NormalizedParserBytes;
feature004V15NormalizedParserBytes = (source) =>
  feature004V15NormalizedParserBytesBeforeV16(feature004V16StripSourceIfPresent(source));

const stripPostCommitV13AdoptionSourceBeforeV16 = stripPostCommitV13AdoptionSource;
stripPostCommitV13AdoptionSource = (source) =>
  stripPostCommitV13AdoptionSourceBeforeV16(feature004V16StripSourceIfPresent(source));

function feature004V16ValidateHistoricalV13Payload(payload, canonicalPayload = payload) {
  captureV13AssertExact(Object.keys(payload), FEATURE004_V16_V13_PAYLOAD_FIELDS,
    'v16 historical v13 payload top-level field order');
  assert.equal(payload.contractVersion, 'feature004-dirty-collision-post-commit/v13');
  assert.equal(payload.findingId, 'F004-V13-CAPTURE-SHAPE-001');
  assert.equal(payload.successorRevision, 'v13');
  assertUtcTimestamp(payload.capturedAt, 'v16 historical v13 capturedAt');
  assert.match(payload.capturedHead, /^[a-f0-9]{40,64}$/,
    'v16 historical v13 captured HEAD is canonical');
  assertExactOrderedKeys(payload.headPolicy, [
    'capturedHeadRole', 'globalHeadEqualityRequired',
    'replacementObjectsDisabledForLineageAndTreeQueries',
    'renameOrCopyLineageInferenceAllowed'
  ], 'v16 historical v13 head policy');
  captureV13AssertExact(payload.successorOf, {
    marker: POST_COMMIT_V12_MARKER,
    rawBlockSha256: POST_COMMIT_V12_BLOCK_SHA256,
    disposition: 'mandatory-byte-identical-history'
  }, 'v16 historical v13 predecessor link');
  assertExactOrderedKeys(payload.captureSchema, [
    'inventoryCommand', 'requiredScope1PathsRemainExact',
    'requiredScope1PathsGainPromotionTolerance', 'foreignPathCoverage',
    'excludedPaths', 'additionalExclusionsAllowed', 'fullRecordOrderedFields',
    'treeEntryShape', 'indexEntryShape', 'indexEntryRecordShape',
    'worktreeLstatShape', 'numstatShape', 'hunkShape', 'indexFlagsShape',
    'missingObservationPolicy'
  ], 'v16 historical v13 capture schema');
  captureV13AssertExact(payload.captureSchema.fullRecordOrderedFields,
    FEATURE004_CAPTURE_V13_FULL_RECORD_FIELDS,
    'v16 historical v13 full-record schema');
  captureV13AssertExact(payload.captureSchema.excludedPaths,
    FEATURE004_CAPTURE_V13_EXCLUDED_PATHS,
    'v16 historical v13 excluded-path schema');
  captureV13AssertMatrixIntegrity(payload.currentMatrix,
    'v16 historical v13 currentMatrix');
  captureV13AssertInventoryIntegrity(payload.inventoryProof,
    payload.currentMatrix, 'v16 historical v13 inventoryProof');
  captureV13AssertExact(payload.currentMatrix.requiredRecords.map(({ path }) => path),
    REQUIRED_SCOPE_ONE_PATHS, 'v16 historical v13 required path order');
  const allMatrixRecords = [
    ...payload.currentMatrix.requiredRecords,
    ...payload.currentMatrix.foreignRecords
  ];
  assert.equal(new Set(allMatrixRecords.map(({ path }) => path)).size,
    allMatrixRecords.length, 'v16 historical v13 matrix paths are unique');
  captureV13AssertClosureIntegrity(payload.protectedAuthorityClosure,
    'v16 historical v13 protectedAuthorityClosure');
  assertExactOrderedKeys(payload.reportPrefix, [
    'startByte', 'endByteExclusive', 'byteLength', 'contentSha256', 'boundary'
  ], 'v16 historical v13 report prefix');
  captureV13AssertExact(payload.reportPrefix, {
    startByte: 0,
    endByteExclusive: POST_COMMIT_V13_REPORT_PREFIX_BYTE_LENGTH,
    byteLength: POST_COMMIT_V13_REPORT_PREFIX_BYTE_LENGTH,
    contentSha256: POST_COMMIT_V13_REPORT_PREFIX_SHA256,
    boundary: 'byte-immediately-before-v13-start-marker'
  }, 'v16 historical v13 report prefix authority');
  captureV13AssertExact(payload.semanticLedgerSelectors,
    payload.protectedAuthorityClosure.semanticLedgerSelectors,
    'v16 historical v13 semantic selector cross-link');
  assertExactOrderedKeys(payload.semanticLedgerSelectors, [
    'selectorContract', 'resolvedSource', 'selectedReceiptCount',
    'semanticSelectorSha256'
  ], 'v16 historical v13 semantic selectors');
  assertSha256(payload.semanticLedgerSelectors.semanticSelectorSha256,
    'v16 historical v13 semantic selector hash');
  const matrixByPath = new Map(allMatrixRecords.map((record) => [record.path, record]));
  payload.protectedAuthorityClosure.orderedEntries.forEach((entry, index) => {
    const label = `v16 historical v13 closure entry ${index}`;
    assert.ok([
      'clean-git-tree',
      'path-scoped-live-parser-source',
      'append-prefix',
      'append-only-ledger-selector'
    ].includes(entry.kind), `${label} kind is closed`);
    if (Object.hasOwn(entry, 'fullRecord')) {
      captureV13AssertExact(Object.keys(entry.fullRecord),
        FEATURE004_CAPTURE_V13_FULL_RECORD_FIELDS, `${label} full-record field order`);
      if (matrixByPath.has(entry.path)) {
        captureV13AssertExact(entry.fullRecord, matrixByPath.get(entry.path),
          `${label} matrix full-record cross-link`);
      }
    }
    if (entry.kind === 'append-prefix') {
      captureV13AssertExact(entry.prefix, payload.reportPrefix,
        `${label} report-prefix cross-link`);
    }
    if (entry.kind === 'append-only-ledger-selector') {
      captureV13AssertExact(entry.semanticLedgerSelectors,
        payload.semanticLedgerSelectors, `${label} semantic-selector cross-link`);
    }
  });
  captureV13AssertExact(payload.adversarialCases,
    FEATURE004_CAPTURE_V13_ADVERSARIAL_CASES,
    'v16 historical v13 nine planned adversaries');
  assertExactOrderedKeys(payload.performanceHandoff, [
    'optimizedHelper', 'referenceHelper', 'equivalenceFixtureCount',
    'requiredEqualOutputs', 'canonicalSourceDigestCheckedBeforeAndAfterCandidateGeneration',
    'changedPathAncestorsCopied', 'untouchedContainerIdentityPreserved',
    'exhaustiveValidationGenerator', 'settledPathsReuse',
    'additionalParserInvocationsForFiveTransitionChecks',
    'assertionCaseOrderOrFailureConditionRemovalAllowed'
  ], 'v16 historical v13 performance handoff');
  assert.equal(payload.performanceHandoff.optimizedHelper,
    'pathCopyClosedSchemaMutationCandidates');
  assert.equal(payload.performanceHandoff.referenceHelper,
    'deepCloneClosedSchemaMutationCandidates');
  assert.equal(payload.performanceHandoff.settledPathsReuse,
    'one parseCollisionContracts result reused by all five scriptTransitionCases checks');
  assertExactOrderedKeys(payload.parserHandoff, [
    'owner', 'path', 'futurePinLiteral', 'futureNormalizedPinFamilyName',
    'futureNormalizedPinFamilyOrder', 'currentImplementationState',
    'payloadEncoding', 'canonicalDecodeReencodeIdentityRequired',
    'futureParserChecksRequired', 'completeDecodedCanonicalPayloadIsAuthority',
    'outerNormalizedSelfHash'
  ], 'v16 historical v13 parser handoff');
  captureV13AssertExact(payload.parserHandoff.futureNormalizedPinFamilyOrder,
    NORMALIZED_SELF_PIN_NAMES_V9,
    'v16 historical v13 normalized pin family order');
  captureV13AssertExact(feature004V16RedactOperatorRoot(payload.repositoryBinding),
    FEATURE004_CAPTURE_V13_BINDING, 'v16 historical v13 repository binding');
  captureV13AssertExact(payload, canonicalPayload,
    'v16 exact pinned historical v13 payload');
  return {
    requiredRecordCount: payload.currentMatrix.requiredRecords.length,
    foreignRecordCount: payload.currentMatrix.foreignRecords.length,
    closureCount: payload.protectedAuthorityClosure.orderedEntryCount,
    selectorCount: payload.semanticLedgerSelectors.selectedReceiptCount
  };
}

function feature004V16HistoricalV13Authority() {
  const historical = feature004V14HistoricalV13Authority();
  const { reportContext, decoded } = historical;
  assert.equal(decoded.outer.markerInclusiveByteLength,
    POST_COMMIT_V13_BLOCK_BYTE_LENGTH,
  'v16 historical v13 envelope raw length is pinned');
  assert.equal(decoded.outer.compressedPayloadSha256.join(''),
    POST_COMMIT_V13_COMPRESSED_PAYLOAD_SHA256,
  'v16 historical v13 compressed hash is pinned');
  assert.equal(decoded.outer.compressedPayloadByteLength,
    POST_COMMIT_V13_COMPRESSED_PAYLOAD_BYTE_LENGTH,
  'v16 historical v13 compressed length is pinned');
  assert.equal(decoded.outer.payloadSha256.join(''), POST_COMMIT_V13_PAYLOAD_SHA256,
    'v16 historical v13 decoded hash is pinned');
  assert.equal(decoded.outer.payloadByteLength, POST_COMMIT_V13_PAYLOAD_BYTE_LENGTH,
    'v16 historical v13 decoded length is pinned');
  assert.equal(sha256(reportContext.blockBytes), POST_COMMIT_V13_BLOCK_SHA256,
    'v16 historical v13 raw pin is exact');
  const summary = feature004V16ValidateHistoricalV13Payload(decoded.payload);
  return { ...historical, summary };
}

function feature004V16AssertNullableGitOid(value, label) {
  if (value === null) return;
  assert.match(value, /^[a-f0-9]{40,64}$/, `${label} is a canonical Git object ID`);
}

function feature004V16AssertNumstat(numstat, path, label) {
  assertExactOrderedKeys(numstat,
    ['present', 'additions', 'deletions', 'binary', 'path'], label);
  assert.equal(numstat.present, true, `${label}.present is exact`);
  assert.equal(typeof numstat.path, 'string', `${label}.path is a string`);
  assert.ok(numstat.path.length > 0, `${label}.path is nonempty`);
  if (numstat.binary) {
    assert.equal(numstat.additions, null, `${label}.additions preserves the binary null`);
    assert.equal(numstat.deletions, null, `${label}.deletions preserves the binary null`);
  } else {
    assert.ok(Number.isSafeInteger(numstat.additions) && numstat.additions >= 0,
      `${label}.additions is a non-negative safe integer`);
    assert.ok(Number.isSafeInteger(numstat.deletions) && numstat.deletions >= 0,
      `${label}.deletions is a non-negative safe integer`);
  }
  if (!numstat.path.includes('\0')) {
    assert.equal(numstat.path, path, `${label}.path is exact`);
  }
}

function feature004V16AssertHunks(hunks, label) {
  assert.ok(Array.isArray(hunks), `${label} is an array`);
  hunks.forEach((hunk, index) => {
    assertExactOrderedKeys(hunk,
      ['header', 'additionCount', 'deletionCount', 'bodySha256'],
      `${label}[${index}]`);
    assertNonemptyString(hunk.header, `${label}[${index}].header`);
    assert.ok(Number.isSafeInteger(hunk.additionCount) && hunk.additionCount >= 0,
      `${label}[${index}].additionCount is a non-negative safe integer`);
    assert.ok(Number.isSafeInteger(hunk.deletionCount) && hunk.deletionCount >= 0,
      `${label}[${index}].deletionCount is a non-negative safe integer`);
    assertSha256(hunk.bodySha256, `${label}[${index}].bodySha256`);
  });
}

function feature004V16V13NumstatToV12(record, field) {
  const value = record.unstagedNumstat[field];
  if (record.pathKind !== 'untracked') return value;
  assert.equal(value, 0,
    `${record.path} immutable v13 untracked ${field} stores the no-diff zero`);
  return null;
}

function feature004V16V13RecordToV12FullRecord(record) {
  captureV13AssertExact(Object.keys(record), FEATURE004_CAPTURE_V13_FULL_RECORD_FIELDS,
    `${record.path} immutable v13 full-record field order`);
  assertNonemptyString(record.path, 'v16 immutable v13 record path');
  assert.ok(record.pathKind === 'tracked' || record.pathKind === 'untracked',
    `${record.path} v13 pathKind is convertible`);
  assert.ok(record.rawStatus === '' || record.rawStatus === '??'
    || /^[ MADRCU?!]{2}$/.test(record.rawStatus),
  `${record.path} v13 rawStatus is exact-width porcelain status`);
  assert.equal(record.staged,
    record.rawStatus !== '' && record.rawStatus !== '??' && record.rawStatus[0] !== ' ',
  `${record.path} v13 staged flag is derived exactly`);
  assert.equal(record.unstaged,
    record.rawStatus === '??' || (record.rawStatus !== '' && record.rawStatus[1] !== ' '),
  `${record.path} v13 unstaged flag is derived exactly`);

  assertExactOrderedKeys(record.headEntry, ['present', 'mode', 'type', 'oid'],
    `${record.path} v13 headEntry`);
  let headOid = null;
  if (record.headEntry.present) {
    assert.equal(record.headEntry.type, 'blob', `${record.path} v13 HEAD entry is a blob`);
    feature004V16AssertNullableGitOid(record.headEntry.oid,
      `${record.path} v13 headEntry.oid`);
    headOid = record.headEntry.oid;
  } else {
    captureV13AssertExact(record.headEntry,
      { present: false, mode: null, type: null, oid: null },
      `${record.path} absent v13 headEntry`);
  }

  assertExactOrderedKeys(record.indexEntry, ['present', 'entries'],
    `${record.path} v13 indexEntry`);
  assert.ok(Array.isArray(record.indexEntry.entries),
    `${record.path} v13 index entries are an array`);
  let indexOid = null;
  if (record.indexEntry.present) {
    assert.equal(record.indexEntry.entries.length, 1,
      `${record.path} v13 index has exactly one entry`);
    const [entry] = record.indexEntry.entries;
    assertExactOrderedKeys(entry, ['mode', 'type', 'oid', 'stage', 'path'],
      `${record.path} v13 stage-zero index entry`);
    assert.equal(entry.stage, 0, `${record.path} v13 index entry is stage zero`);
    assert.equal(entry.type, 'blob', `${record.path} v13 index entry is a blob`);
    assert.equal(entry.path, record.path, `${record.path} v13 index path is exact`);
    feature004V16AssertNullableGitOid(entry.oid, `${record.path} v13 index oid`);
    indexOid = entry.oid;
  } else {
    assert.deepEqual(record.indexEntry.entries, [],
      `${record.path} absent v13 index has no entries`);
  }

  assertExactOrderedKeys(record.worktreeLstat,
    ['present', 'kind', 'mode', 'executable', 'symlink'],
    `${record.path} v13 worktree representation`);
  assert.equal(record.worktreeLstat.present, true,
    `${record.path} v13 worktree representation is present`);
  assertNonemptyString(record.worktreeLstat.kind,
    `${record.path} v13 worktree representation kind`);
  assert.match(record.worktreeLstat.mode, /^[0-7]{6}$/,
    `${record.path} v13 worktree mode is canonical`);
  feature004V16AssertNullableGitOid(record.worktreeGitBlobOid,
    `${record.path} v13 worktree Git blob`);
  assertSha256(record.contentSha256, `${record.path} v13 content SHA-256`);
  assert.ok(Number.isSafeInteger(record.byteLength) && record.byteLength >= 0,
    `${record.path} v13 byte length is a non-negative safe integer`);
  feature004V16AssertNumstat(record.stagedNumstat, record.path,
    `${record.path} v13 staged numstat`);
  feature004V16AssertNumstat(record.unstagedNumstat, record.path,
    `${record.path} v13 unstaged numstat`);
  feature004V16AssertHunks(record.stagedHunks, `${record.path} v13 staged hunks`);
  feature004V16AssertHunks(record.unstagedHunks, `${record.path} v13 unstaged hunks`);
  feature004V16AssertNullableGitOid(record.lastCommit,
    `${record.path} v13 lastCommit`);

  const converted = {
    path: record.path,
    pathKind: record.pathKind,
    classification: record.classification,
    ownerAttribution: record.ownerAttribution,
    feature004OwnershipClaim: record.feature004OwnershipClaim,
    transitionClass: record.transitionClass,
    ...
    headOid,
    indexOid,
    worktreeGitOid: record.worktreeGitBlobOid,
    worktreeSha256: record.contentSha256,
    byteLength: record.byteLength,
    additions: feature004V16V13NumstatToV12(record, 'additions'),
    deletions: feature004V16V13NumstatToV12(record, 'deletions'),
    hunkCount: record.unstagedHunks.length,
    hunkBodySha256: record.unstagedHunks.map(({ bodySha256 }) => bodySha256),
    lastCommit: record.lastCommit
  };
  assertExactOrderedKeys(converted, POST_COMMIT_V9_FULL_RECORD_FIELDS,
    `${record.path} converted immutable v12 full record`);
  feature004V16AssertNullableGitOid(converted.headOid, `${record.path} converted headOid`);
  feature004V16AssertNullableGitOid(converted.indexOid, `${record.path} converted indexOid`);
  feature004V16AssertNullableGitOid(converted.worktreeGitOid,
    `${record.path} converted worktreeGitOid`);
  assertSha256(converted.worktreeSha256, `${record.path} converted worktreeSha256`);
  converted.hunkBodySha256.forEach((value, index) =>
    assertSha256(value, `${record.path} converted hunkBodySha256[${index}]`));
  feature004V16AssertNullableGitOid(converted.lastCommit,
    `${record.path} converted lastCommit`);
  return converted;
}

function feature004V16WithHistoricalCurrentCaptureBlocked(run) {
  const activeSnapshot = captureV13Snapshot;
  const activeValidate = captureV13ValidatePayloadAgainstRecapture;
  const activeFullRecord = captureV13FullRecord;
  let forbiddenCallCount = 0;
  const forbidden = (name) => {
    forbiddenCallCount += 1;
    throw new Error(`v16 historical replay called forbidden ${name}`);
  };
  captureV13Snapshot = () => forbidden('captureV13Snapshot');
  captureV13ValidatePayloadAgainstRecapture = () =>
    forbidden('captureV13ValidatePayloadAgainstRecapture');
  captureV13FullRecord = () => forbidden('captureV13FullRecord');
  let result;
  try {
    result = run();
  } finally {
    captureV13Snapshot = activeSnapshot;
    captureV13ValidatePayloadAgainstRecapture = activeValidate;
    captureV13FullRecord = activeFullRecord;
    assert.equal(captureV13Snapshot, activeSnapshot,
      'v16 restores captureV13Snapshot identity');
    assert.equal(captureV13ValidatePayloadAgainstRecapture, activeValidate,
      'v16 restores captureV13ValidatePayloadAgainstRecapture identity');
    assert.equal(captureV13FullRecord, activeFullRecord,
      'v16 restores captureV13FullRecord identity');
  }
  assert.equal(forbiddenCallCount, 0,
    'v16 inherited historical replay calls no current v13 capture function');
  return { result, currentForeignReads: forbiddenCallCount };
}

function feature004V16HistoricalV12Authority(historicalV13) {
  const boundedHistory = historicalV13.reportContext.prefixBytes.toString('utf8');
  const v12Block = parseReportBlock(boundedHistory, POST_COMMIT_V12_MARKER);
  assertPinnedReportBlock(v12Block.raw, POST_COMMIT_V12_BLOCK_SHA256,
    'v16 bounded immutable v12 block');
  assert.equal(Buffer.byteLength(v12Block.raw), POST_COMMIT_V12_BLOCK_BYTE_LENGTH,
    'v16 bounded immutable v12 byte length is exact');
  return v12Block;
}

function feature004V16AssertV12IdentityCommitments(records, commitments, label) {
  assert.equal(records.length, commitments.length,
    `${label} cardinality matches immutable v12 commitments`);
  records.forEach((record, index) => {
    const commitment = commitments[index];
    assert.equal(record.path, commitment.path,
      `${label}[${index}] path order is exact`);
    assertSha256(commitment.identitySha256,
      `${label}[${index}] immutable v12 identity`);
    const actualIdentity = sha256(JSON.stringify(record));
    if (actualIdentity !== commitment.identitySha256) {
      const predecessorUntracked = record.pathKind === 'untracked'
        ? { ...record, additions: null, deletions: null } : null;
      const predecessorIdentity = predecessorUntracked === null
        ? null : sha256(JSON.stringify(predecessorUntracked));
      const fieldMismatch = predecessorIdentity === commitment.identitySha256
        ? ` fields=additions,deletions v13=${JSON.stringify([record.additions, record.deletions])} v12=[null,null]`
        : '';
      throw new Error(`${label}[${index}] first identity mismatch path=${record.path} actual=${actualIdentity} expected=${commitment.identitySha256}${fieldMismatch}`);
    }
  });
}

const FEATURE004_V16_V12_PARSER_METADATA_FIELDS = [
  'path',
  'pathKind',
  'classification',
  'ownerAttribution',
...
// The v13 payload captured the parser under a later normalization epoch, so its
// bytes can never reproduce the v12 stripped-parser identity by field conversion.
function feature004V16HermeticV12ParserRecord(record, v12) {
  const converted = feature004V16V13RecordToV12FullRecord(record);
  const closure = v12.protectedAuthorityClosure.orderedEntries
    .find(({ kind }) => kind === 'path-scoped-live-parser-source');
  assert.ok(closure,
    'v16 immutable v12 records exactly one stripped-parser source authority');
  assert.equal(closure.path, COLLISION_PARSER_PATH,
    'v16 immutable v12 stripped-parser authority is path-exact');
  assert.equal(closure.normalizedPinFamily, 'normalized-self-pins/v7-pre-v12',
    'v16 immutable v12 stripped-parser normalization contract is exact');
  captureV13AssertExact(closure.normalizedPinNames, NORMALIZED_SELF_PIN_NAMES_V7,
    'v16 immutable v12 stripped-parser normalized pin family');
  assert.match(closure.capturedHead, /^[a-f0-9]{40}$/,
    'v16 immutable v12 stripped-parser captured head is an exact Git commit');
  const authority = {
    path: converted.path,
    pathKind: converted.pathKind,
    status: converted.status,
    staged: converted.staged,
    unstaged: converted.unstaged,
    headOid: converted.headOid,
    indexOid: converted.indexOid,
    lastCommit: converted.lastCommit
  };
  captureV13AssertExact({
    status: authority.status,
    headOid: authority.headOid,
    indexOid: authority.indexOid,
    lastCommit: authority.lastCommit
  }, {
    status: closure.liveStatus,
    headOid: closure.blobOid,
    indexOid: closure.blobOid,
    lastCommit: closure.pathLastCommit
  }, 'v16 immutable v13 parser metadata matches the immutable v12 parser authority');
  const shared = v12.currentMatrix.requiredRecordSharedContract;
  const parserRecord = feature004V15WithHistoricalParserMetadata(authority,
    'v16 hermetic v12 stripped-parser metadata', null, closure.capturedHead,
    () => postCommitV12ParserRecord({
      path: COLLISION_PARSER_PATH,
      classification: shared.classification,
      ownerAttribution: shared.ownerAttribution,
      feature004OwnershipClaim: shared.feature004OwnershipClaim,
      transitionClass: converted.transitionClass
    }));
  assertExactOrderedKeys(parserRecord, POST_COMMIT_V9_FULL_RECORD_FIELDS,
    'v16 hermetic v12 stripped-parser full record');
  captureV13AssertExact({
    worktreeGitOid: parserRecord.worktreeGitOid,
    worktreeSha256: parserRecord.worktreeSha256,
    byteLength: parserRecord.byteLength
  }, {
    worktreeGitOid: closure.normalizedWorktreeGitOid,
    worktreeSha256: closure.normalizedContentSha256,
    byteLength: closure.normalizedByteLength
  }, 'v16 hermetic v12 stripped-parser bytes match the immutable v12 normalization contract');
  captureV13AssertExact(
    Object.fromEntries(FEATURE004_V16_V12_PARSER_METADATA_FIELDS
      .map((field) => [field, parserRecord[field]])),
    Object.fromEntries(FEATURE004_V16_V12_PARSER_METADATA_FIELDS
      .map((field) => [field, converted[field]])),
    'v16 hermetic v12 stripped-parser metadata is the immutable v13 metadata');
  return parserRecord;
}

function feature004V16HermeticV12RequiredRecord(record, v12) {
  if (record.path !== COLLISION_PARSER_PATH) {
    return feature004V16V13RecordToV12FullRecord(record);
  }
  return feature004V16HermeticV12ParserRecord(record, v12);
}

function feature004V16BuildHermeticV12Authority(payload, v12,
  canonicalPayload = payload) {
  captureV13AssertExact(payload.currentMatrix.requiredRecords,
    canonicalPayload.currentMatrix.requiredRecords,
    'v16 immutable v13 required records');
  captureV13AssertExact(payload.currentMatrix.foreignRecords,
    canonicalPayload.currentMatrix.foreignRecords,
    'v16 immutable v13 foreign records');
  captureV13AssertExact(payload.inventoryProof.porcelainEntries,
    canonicalPayload.inventoryProof.porcelainEntries,
    'v16 immutable v13 inventory records');
  feature004V16ValidateHistoricalV13Payload(payload, canonicalPayload);

  const requiredCommitments = v12.currentMatrix.requiredRecordCommitments;
  const foreignCommitments = v12.currentMatrix.foreignRecordCommitments;
  captureV13AssertExact(payload.currentMatrix.requiredRecords.map(({ path }) => path),
    requiredCommitments.map(({ path }) => path),
    'v16 v13-to-v12 required path order and cardinality');
  captureV13AssertExact(payload.currentMatrix.foreignRecords.map(({ path }) => path),
    foreignCommitments.map(({ path }) => path),
    'v16 v13-to-v12 foreign path order and cardinality');
  assert.equal(requiredCommitments.length, 19,
    'v16 immutable v12 has exactly 19 required commitments');
  assert.equal(foreignCommitments.length, 55,
    'v16 immutable v12 has exactly 55 foreign commitments');

  const requiredRecords = payload.currentMatrix.requiredRecords
    .map((record) => feature004V16HermeticV12RequiredRecord(record, v12));
  const foreignRecords = payload.currentMatrix.foreignRecords
    .map(feature004V16V13RecordToV12FullRecord);
  feature004V16AssertV12IdentityCommitments(requiredRecords,
    requiredCommitments, 'v16 hermetic v12 required records');
  feature004V16AssertV12IdentityCommitments(foreignRecords,
    foreignCommitments, 'v16 hermetic v12 foreign records');
  ...
  requiredRecords.forEach((record, index) => {
    captureV13AssertExact({
      ...
      ownerAttribution: record.ownerAttribution,
      feature004OwnershipClaim: record.feature004OwnershipClaim
    }, v12.currentMatrix.requiredRecordSharedContract,
    `v16 hermetic v12 required record ${index} metadata`);
  });
  foreignRecords.forEach((record, index) => {
    const commitment = foreignCommitments[index];
    captureV13AssertExact({
      classification: record.classification,
      ownerAttribution: record.ownerAttribution,
      feature004OwnershipClaim: record.feature004OwnershipClaim
      ...
      ownerAttribution: commitment.ownerAttribution,
      feature004OwnershipClaim: false
    }, `v16 hermetic v12 foreign record ${index} metadata`);
  });

  const inventoryEntries = payload.inventoryProof.porcelainEntries
    .map((entry, index) => {
      assertExactOrderedKeys(entry,
        ['rawStatus', 'path', 'originalPath', 'rawRecordBase64'],
        `v16 immutable v13 inventory entry ${index}`);
      return { status: entry.rawStatus, path: entry.path };
    });
  captureV13AssertExact(inventoryEntries, v12.inventoryProof.porcelainEntries,
    'v16 hermetic v12 inventory projection');
  assert.equal(inventoryEntries.length, 67,
    'v16 hermetic v12 inventory has exactly 67 entries');
  assert.equal(sha256(JSON.stringify(inventoryEntries)),
    POST_COMMIT_V12_INVENTORY_SHA256,
  'v16 hermetic v12 inventory hash is exact');
  assertPostCommitV12RecordCommitments(v12,
    requiredRecords, foreignRecords, inventoryEntries);
  return { requiredRecords, foreignRecords, inventoryEntries };
}

function feature004V16AssertConverterReadsNoForeignPath(payload, v12, authority) {
  const foreignIndex = payload.currentMatrix.foreignRecords.findIndex((record) =>
    !record.headEntry.present && !record.indexEntry.present);
  assert.ok(foreignIndex >= 0,
    'v16 immutable v13 has an untracked foreign record absent from clean HEAD');
  const record = payload.currentMatrix.foreignRecords[foreignIndex];
  const forbiddenPath = resolve(ROOT, record.path);
  const activeReadFileSync = readFileSync;
  let currentForeignReads = 0;
  readFileSync = (path, ...args) => {
    if (typeof path === 'string' && resolve(path) === forbiddenPath) {
      currentForeignReads += 1;
      throw new Error(`v16 converter attempted current foreign read ${record.path}`);
    }
    return activeReadFileSync(path, ...args);
  };
  let converted;
  try {
    converted = feature004V16V13RecordToV12FullRecord(record);
  } finally {
    readFileSync = activeReadFileSync;
    assert.equal(readFileSync, activeReadFileSync,
      'v16 restores readFileSync identity after foreign-read proof');
  }
  assert.equal(currentForeignReads, 0,
    'v16 converter performs zero current foreign-path reads');
  assert.equal(record.pathKind, 'untracked',
    'v16 absent clean-HEAD foreign proof uses an immutable untracked record');
  assert.equal(record.headEntry.present, false,
    'v16 absent clean-HEAD foreign proof has no immutable HEAD entry');
  assert.equal(record.indexEntry.present, false,
    'v16 absent clean-HEAD foreign proof has no immutable index entry');
  captureV13AssertExact(converted, authority.foreignRecords[foreignIndex],
    'v16 absent current foreign path is irrelevant to immutable conversion');
  assert.equal(sha256(JSON.stringify(converted)),
    v12.currentMatrix.foreignRecordCommitments[foreignIndex].identitySha256,
  'v16 absent current foreign path retains its immutable v12 identity');
  return currentForeignReads;
}

function feature004V16RunHermeticHistoricalV13AdversarialCases() {
  const historicalV13 = feature004V16HistoricalV13Authority();
  const v12Block = feature004V16HistoricalV12Authority(historicalV13);
  const authority = feature004V16BuildHermeticV12Authority(
    historicalV13.decoded.payload, v12Block.value);
  const activeV12Observations = capturePostCommitV12AuthorityObservations;
  const activeV12CurrentRecords = postCommitV12CurrentRecords;
  const activeV12InventoryEntries = postCommitV12InventoryEntries;
  capturePostCommitV12AuthorityObservations = feature004V14HistoricalV12Observations;
  postCommitV12CurrentRecords = (v12) => {
    captureV13AssertExact(v12, v12Block.value,
      'v16 inherited v12 current-record provider receives immutable v12');
    return {
      requiredRecords: structuredClone(authority.requiredRecords),
      foreignRecords: structuredClone(authority.foreignRecords)
    };
  };
  postCommitV12InventoryEntries = () => structuredClone(authority.inventoryEntries);
  let guarded;
  try {
    assert.equal(runPostCommitV13AdversarialCases,
      runPostCommitV13AdversarialCasesBeforeV14,
    'v16 invokes the original inherited v13 adversarial runner unchanged');
    guarded = feature004V16WithHistoricalCurrentCaptureBlocked(() =>
      runPostCommitV13AdversarialCases());
  } finally {
    capturePostCommitV12AuthorityObservations = activeV12Observations;
    postCommitV12CurrentRecords = activeV12CurrentRecords;
    postCommitV12InventoryEntries = activeV12InventoryEntries;
    assert.equal(capturePostCommitV12AuthorityObservations, activeV12Observations,
      'v16 restores v12 authority observation identity');
    assert.equal(postCommitV12CurrentRecords, activeV12CurrentRecords,
      'v16 restores v12 current-record provider identity');
    assert.equal(postCommitV12InventoryEntries, activeV12InventoryEntries,
      'v16 restores v12 inventory provider identity');
  }
  return {
    requiredRecordCount: authority.requiredRecords.length,
    foreignRecordCount: authority.foreignRecords.length,
    inventoryCount: authority.inventoryEntries.length,
    currentForeignReads: guarded.currentForeignReads
  };
}

function feature004V16RunConverterProviderAdversaries(historicalV13, v12Block) {
  const canonical = historicalV13.decoded.payload;
  const firstForeignPath = canonical.currentMatrix.foreignRecords[0].path;
  const hunkRecord = [
    ...canonical.currentMatrix.requiredRecords,
    ...canonical.currentMatrix.foreignRecords
  ].find(({ unstagedHunks }) => unstagedHunks.length > 0);
  assert.ok(hunkRecord, 'v16 immutable v13 contains an unstaged hunk commitment');
  const cases = [
    ['deleted immutable v13 foreign record', (payload) => {
      payload.currentMatrix.foreignRecords.shift();
    }],
    ['duplicated immutable v13 foreign record', (payload) => {
      payload.currentMatrix.foreignRecords.splice(1, 0,
        structuredClone(payload.currentMatrix.foreignRecords[0]));
    }],
    ['reordered immutable v13 foreign records', (payload) => {
      [payload.currentMatrix.foreignRecords[0], payload.currentMatrix.foreignRecords[1]] =
        [payload.currentMatrix.foreignRecords[1], payload.currentMatrix.foreignRecords[0]];
    }],
    ['mutated immutable v13 foreign record', (payload) => {
      payload.currentMatrix.foreignRecords[0].contentSha256 = '0'.repeat(64);
    }],
    ['mutated path metadata', (payload) => {
      payload.currentMatrix.foreignRecords[0].path = `${firstForeignPath}.mutated`;
    }],
    ['mutated status metadata', (payload) => {
      const record = payload.currentMatrix.foreignRecords[0];
      record.rawStatus = record.rawStatus === '??' ? ' M' : '??';
    }],
    ['mutated representation metadata', (payload) => {
      payload.currentMatrix.foreignRecords[0].worktreeLstat.mode = '100755';
    }],
    ['mutated hunk metadata', (payload) => {
      const records = [
        ...payload.currentMatrix.requiredRecords,
        ...payload.currentMatrix.foreignRecords
      ];
      records.find(({ path }) => path === hunkRecord.path)
        .unstagedHunks[0].bodySha256 = '0'.repeat(64);
    }],
    ['mutated content metadata', (payload) => {
      payload.currentMatrix.foreignRecords[0].byteLength += 1;
    }],
    ['mutated identity metadata', (payload) => {
      payload.currentMatrix.foreignRecords[0].ownerAttribution = 'mutated-owner';
    }]
  ];
  for (const [label, mutate] of cases) {
    const candidate = structuredClone(canonical);
    mutate(candidate);
    assert.throws(() => feature004V16BuildHermeticV12Authority(
      candidate, v12Block.value, canonical),
      `v16 rejects ${label}`);
  }
  const authority = feature004V16BuildHermeticV12Authority(
    canonical, v12Block.value);
  return feature004V16AssertConverterReadsNoForeignPath(
    canonical, v12Block.value, authority);
}

function feature004V16AssertV14CurrentAuthorityStillRejects() {
  const v14 = feature004V14ParseHistoricalAuthority();
  const activeStagedPaths = feature004V14StagedPaths;
  const activeScopesProjection = feature004V14ScopesProjection;
  const activeCurrentRecord = feature004V14CurrentAuthorityRecord;
  const historicalScopesText = captureV13GitText(['show',
    `${v14.decoded.payload.captureTransaction.startHead}:${FEATURE004_V15_SCOPES_PATH}`]);
  let mutated = false;
  feature004V14StagedPaths = () => [];
  feature004V14ScopesProjection = () => activeScopesProjection(historicalScopesText);
  feature004V14CurrentAuthorityRecord = (record) => structuredClone(record);
  try {
    assert.doesNotThrow(() => feature004V14ValidateCurrent(
      v14.decoded.payload, v14.reportContext),
    'v16 leaves v14 current Scope 1 authority validation unchanged');
    feature004V14CurrentAuthorityRecord = (captured) => {
    ...
    assert.throws(() => feature004V14ValidateCurrent(
      v14.decoded.payload, v14.reportContext),
    'v16 retains v14 rejection of a mutated current Scope 1 path');
  } finally {
    feature004V14StagedPaths = activeStagedPaths;
    feature004V14ScopesProjection = activeScopesProjection;
    feature004V14CurrentAuthorityRecord = activeCurrentRecord;
  }
  assert.equal(mutated, true, 'v16 adversary reached one v14 current authority record');
}

function feature004V16AssertPlanningCapturedAt() {
  const resolvedCommit = captureV13GitText([
    'rev-parse', '--verify', `${FEATURE004_V16_PLAN_COMMIT}^{commit}`
  ]).trim();
  assert.equal(resolvedCommit, FEATURE004_V16_PLAN_COMMIT,
    'v16 planning commit resolves to the exact canonical commit');
  const capturedAt = new Date(captureV13GitText([
    'show', '-s', '--format=%cI', FEATURE004_V16_PLAN_COMMIT
  ]).trim()).toISOString();
  assertUtcTimestamp(capturedAt, 'v16 planning capturedAt');
  assert.equal(capturedAt, FEATURE004_V16_CAPTURED_AT,
    'v16 capturedAt is the exact planning commit committer timestamp');
}

function feature004V16ParseAuthority() {
  const reportBytes = readFileSync(resolve(ROOT, REPORT_PATH));
  const reportContext = feature004V16ReportContext(reportBytes);
  assert.equal(sha256(reportContext.blockBytes),
    HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256,
  'v16 raw report block hash is independently pinned');
  const parsed = parseReportBlock(reportBytes.toString('utf8'), FEATURE004_V16_MARKER);
  assert.equal(parsed.raw, reportContext.block,
    'v16 parser selects the exact marker-inclusive report bytes');
  captureV13AssertExact(parsed.value, FEATURE004_V16_CONTRACT,
    'v16 closed report contract');
  captureV13AssertExact(Object.keys(parsed.value), FEATURE004_V16_PAYLOAD_FIELDS,
    'v16 report payload field order');
  assert.equal(feature004V16RenderBlock(parsed.value), reportContext.block,
    'v16 report block is the exact canonical renderer identity');
  feature004V16AssertPlanningCapturedAt();
  assert.equal(parsed.value.noCurrentAuthority, true,
    'v16 declares no current authority');
  assert.equal(JSON.stringify(parsed.value).includes('specs/010-'), false,
    'v16 stores no Feature 010 path');
  captureV13AssertExact(parsed.value.commitPolicy.exactAllowedPaths,
    FEATURE004_V16_ALLOWED_PATHS, 'v16 exact commit allowlist');
  feature004V16AssertPin(readFileSync(resolve(ROOT, COLLISION_PARSER_PATH), 'utf8'));
  return { reportContext, payload: parsed.value };
}

function feature004V16ValidateV15History() {
  const reportBytes = readFileSync(resolve(ROOT, REPORT_PATH));
  const boundedV15 = feature004V16BoundedV15Bytes(reportBytes);
  const reportContext = feature004V15ReportContextBeforeV16(boundedV15);
  assert.equal(sha256(reportContext.blockBytes), POST_COMMIT_V15_BLOCK_SHA256,
    'v16 focused validation preserves the exact v15 raw block');
  const decoded = feature004V15DecodeBlock(reportContext.block);
  feature004V15ValidatePayloadSchema(decoded.payload);
  const parserBytes = feature004V15NormalizedParserBytes(
    readFileSync(resolve(ROOT, COLLISION_PARSER_PATH), 'utf8'));
  assert.equal(sha256(parserBytes),
    decoded.payload.implementationAuthority.contentSha256,
  'v16 focused validation preserves the v15 normalized parser hash');
  assert.equal(parserBytes.length,
    decoded.payload.implementationAuthority.byteLength,
  'v16 focused validation preserves the v15 normalized parser length');
  return decoded.payload;
}

function feature004V16ParseAuthorityFromCandidate(report) {
  const reportBytes = Buffer.from(report, 'utf8');
  const reportContext = feature004V16ReportContext(reportBytes);
  const parsed = parseReportBlock(report, FEATURE004_V16_MARKER);
  captureV13AssertExact(parsed.value, FEATURE004_V16_CONTRACT,
    'v16 candidate closed report contract');
  assert.equal(feature004V16RenderBlock(parsed.value), reportContext.block,
    'v16 candidate report canonical rendering');
  return parsed.value;
}

function feature004V16RunAdversarialCases() {
  const parsed = feature004V16ParseAuthority();
  const historical = feature004V16HistoricalV13Authority();
  const v12Block = feature004V16HistoricalV12Authority(historical);
  assertEveryClosedSchemaMutationFails(historical.decoded.payload,
    (candidate) => feature004V16ValidateHistoricalV13Payload(
      candidate, historical.decoded.payload),
    'v16 historical v13 closed payload');
  assert.equal(feature004V16RunConverterProviderAdversaries(
    historical, v12Block), 0,
  'v16 converter/provider adversaries perform zero current foreign reads');
  feature004V16AssertV14CurrentAuthorityStillRejects();
  const report = readFileSync(resolve(ROOT, REPORT_PATH), 'utf8');
  assert.throws(() => feature004V16ReportContext(Buffer.from(
    `${report}\n${parsed.reportContext.block}\n\n`, 'utf8')),
  'v16 rejects a duplicate report block');
  assert.throws(() => feature004V16ReportContext(Buffer.from(
    report.replace(parsed.reportContext.block, ''), 'utf8')),
  'v16 rejects a missing report block');
  const malformedV16 = parsed.reportContext.block.replace('```json\n{',
    '```json\n{ malformed');
  assert.throws(() => feature004V16ParseAuthorityFromCandidate(
    report.replace(parsed.reportContext.block, malformedV16)),
  'v16 rejects malformed report JSON');
  assert.throws(() => assertPinnedReportBlock(
    `${parsed.reportContext.block} `,
    HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256, 'mutated v16 block'),
  'v16 rejects raw marker-inclusive byte drift');
  const separatorMutation = Buffer.from(report, 'utf8');
  separatorMutation[parsed.reportContext.v15.endByteExclusive] = 0x20;
  assert.throws(() => feature004V16ReportContext(separatorMutation),
    'v16 rejects a v15-to-v16 separator mutation');
  assert.throws(() => feature004V16ReportContext(Buffer.concat([
    Buffer.from(report, 'utf8'), Buffer.from('x', 'utf8')
  ])), 'v16 rejects a non-two-LF final suffix');

  const parserSource = readFileSync(resolve(ROOT, COLLISION_PARSER_PATH), 'utf8');
  const v16PinCases = [
    ['missing', (value) => value.replace(
      /^const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256.*\n/m, '')],
    ['duplicate', (value) => value.replace(
      /^const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256.*$/m,
      (line) => `${line}\n${line}`)],
    ['renamed', (value) => value.replace(
      'HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256',
      'HERMETIC_HISTORICAL_V13_CURRENT_BLOCK_SHA256')],
    ['nonhex', (value) => value.replace(
      HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256, 'g'.repeat(64))],
    ['reordered', (value) => {
      const lines = value.split('\n');
      const v15Index = lines.findIndex((line) => line.startsWith('const POST_COMMIT_V15_BLOCK_SHA256'));
      const v16Index = lines.findIndex((line) => line.startsWith(
        'const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256'));
      [lines[v15Index], lines[v16Index]] = [lines[v16Index], lines[v15Index]];
      return lines.join('\n');
    }],
    ['extra', (value) => value.replace(
      /^const HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256.*$/m,
      (line) => `${line}\nconst HERMETIC_HISTORICAL_V13_V16_EXTRA_BLOCK_SHA256 = '${'0'.repeat(64)}';`)]
  ];
  v16PinCases.forEach(([label, mutate]) => assert.throws(
    () => {
      const candidate = mutate(parserSource);
      feature004V16AssertPin(candidate);
      feature004V16StripSource(candidate);
    }, `v16 rejects ${label} parser pin`));
  assert.throws(() => feature004V16StripSource(parserSource.replace(
    `\n${FEATURE004_V16_END}\n`, `\n${FEATURE004_V16_BEGIN}\n`)),
  'v16 rejects a parser branch marker mutation');
}

const feature004V14RunInheritedV13AdversarialCasesBeforeV16 =
  feature004V14RunInheritedV13AdversarialCases;
feature004V14RunInheritedV13AdversarialCases =
  feature004V16RunHermeticHistoricalV13AdversarialCases;
const parseCollisionContractsBeforeV16 = parseCollisionContracts;
const runForeignSetV7AdversarialCasesBeforeV16 = runForeignSetV7AdversarialCases;
let feature004V16ValidationLogged = false;

function feature004V16ParseCollisionContracts() {
  const inherited = parseCollisionContractsBeforeV16();
  const v16 = feature004V16ParseAuthority();
  if (!feature004V16ValidationLogged) {
    console.log(`FEATURE004_V16_VALIDATED marker=${FEATURE004_V16_MARKER} sha256=${HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256} predecessor=${POST_COMMIT_V15_BLOCK_SHA256} currentForeignReads=0 v14CurrentAuthority=unchanged`);
    feature004V16ValidationLogged = true;
  }
  return {
    ...inherited,
    postCommitV16: v16.payload,
    postCommitV16Raw: v16.reportContext.block
  };
}

function feature004V16RunAllAdversarialCases() {
  assert.equal(runForeignSetV7AdversarialCasesBeforeV16,
    feature004V15RunAllAdversarialCases,
  'v16 preserves the complete inherited v15 adversarial function identity');
  assert.notEqual(feature004V14RunInheritedV13AdversarialCases,
    feature004V14RunInheritedV13AdversarialCasesBeforeV16,
  'v16 replaces only the post-v14 inherited v13 live-current replay adapter');
  runForeignSetV7AdversarialCasesBeforeV16();
  feature004V16RunAdversarialCases();
}

parseCollisionContracts = feature004V16ParseCollisionContracts;
runForeignSetV7AdversarialCases = feature004V16RunAllAdversarialCases;
assert.equal(parseCollisionContractsBeforeV16, feature004V15ParseCollisionContracts,
  'v16 retains the v15 parser as bounded historical implementation input');

if (process.env.FEATURE004_VALIDATE_V16_HERMETIC === '1') {
  const parsed = feature004V16ParseAuthority();
  const summary = feature004V16RunHermeticHistoricalV13AdversarialCases();
  feature004V16RunAdversarialCases();
  feature004V16ValidateV15History();
  console.log(`FEATURE004_V16_HERMETIC_VALIDATED marker=${FEATURE004_V16_MARKER} rawSha256=${HERMETIC_HISTORICAL_V13_V16_BLOCK_SHA256} required=${summary.requiredRecordCount} foreign=${summary.foreignRecordCount} inventory=${summary.inventoryCount} currentForeignReads=${summary.currentForeignReads}`);
  process.exit(0);
}
/* FEATURE-004-COLLISION-HERMETIC-HISTORICAL-V13-V16-END */
