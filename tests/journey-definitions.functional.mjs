...
test('TP-08-02 SCN-012-032 every registered tool resolves concrete Journey goals through the runtime', () => {
  ...
  assert.equal(summary.ordinaryTools, 27, 'twenty-seven ordinary tools must resolve');
  assert.equal(summary.totalGoals, 58, 'twenty-seven ordinary tools x2 goals plus four Center goals');
  assert.equal(summary.definitionCount, 58, 'the journey registry defines exactly 58 goals');
});
...
test('TP-08-02 SCN-012-032 each ordinary tool has at least two concrete same-tool goals with a mechanism', () => {
  ...
  assert.equal(ordinary.length, 27);
...
test('TP-08-02 every one of the 58 definitions compiles under the runtime schema with a fingerprint', () => {
  const registry = journeyRegistry();
  const compiled = requireValue(RJ.compileRegistry(registry));
  assert.equal(compiled.definitionIds.length, 58);
...
test('TP-08-02 cross-checks that rlexperience.js still validates the same journey registry (single source)', () => {
  ...
  assert.equal(result.value.definitions.length, 58);
  ...
  assert.equal(result.value.steps.length, 80);
