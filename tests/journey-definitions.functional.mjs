...
test('TP-08-02 SCN-012-032 every registered tool resolves concrete Journey goals through the runtime', () => {
  ...
  assert.equal(summary.ordinaryTools, 29, 'twenty-nine ordinary tools must resolve');
  assert.equal(summary.totalGoals, 62, 'twenty-nine ordinary tools x2 goals plus four Center goals');
  assert.equal(summary.definitionCount, 62, 'the journey registry defines exactly 62 goals');
});
...
test('TP-08-02 every one of the 62 definitions compiles under the runtime schema with a fingerprint', () => {
  const registry = journeyRegistry();
  const compiled = requireValue(RJ.compileRegistry(registry));
  assert.equal(compiled.definitionIds.length, 62);
...
test('TP-08-02 cross-checks that rlexperience.js still validates the same journey registry (single source)', () => {
  ...
  assert.equal(result.value.definitions.length, 62);
  ...
  assert.equal(result.value.steps.length, 84);
